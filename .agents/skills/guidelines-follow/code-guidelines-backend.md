...
**CRITICAL REQUIREMENT:** All services must log authentication failures, validation errors, and business logic errors to the admin dashboard.
...
export class LoggerService {
  ...
  constructor(private readonly supabaseFactory: SupabaseClientFactory) {
    this.supabase = this.supabaseFactory.createServiceClient()
  }
  ...
  async error(params: {
    ...
    try {
      const { error } = await this.supabase.from('app_errors').insert({
        app: params.app,
      ...
      console.error('Critical: Failed to log error:', err)
    }
  }
...
- **Admin Dashboard:** `/admin/errors` (frontend)
...
- [ ] Errors appear in admin dashboard
...
- Project architecture: `@.docs/guidelines/development/.project-architecture.md`
