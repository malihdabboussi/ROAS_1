...
**SECURITY & VALIDATION** 7. [Security Requirements (MANDATORY)](#7-security-requirements-mandatory) ............ Line ~800 8. [Input Validation & Error Handling](#8-input-validation--error-handling) ........... Line ~894 9. [Admin Error Logging (MANDATORY)](#9-admin-error-logging-mandatory) ................ Line ~959

**COMMON PATTERNS** 10. [Common Coding Patterns](#10-common-coding-patterns) ............................. Line ~1144 11. [AI/LLM Integration Patterns](#11-aillm-integration-patterns) .................... Line ~1191 12. [Web Scraping with Fallback](#12-web-scraping-with-fallback) .................... Line ~1293 13. [Common Mistakes to Avoid](#13-common-mistakes-to-avoid) ......................... Line ~1389
...
2. **Find ALL Old Code References**
   ...
   grep -r "oldFunctionName" apps/app/src/
   grep -r "OldComponentName" apps/app/src/
   grep -r "import.*oldFile" apps/app/src/
   ```
...
**See `@.project-architecture.md` for directory structure details.**
...
**See `@.project-architecture.md` for feature structure standards.**
...
**For UI error handling patterns, see `@.design_guidelines.md` (Toast, Inline Errors, Error States)**

---

<!-- ============================================================================
   SECTION 9: ADMIN ERROR LOGGING (MANDATORY)
   ============================================================================ -->

## 9. Admin Error Logging (MANDATORY)
...
| ------------ | -------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| `app`        | ✅       | Application identifier              | `'app'`, `'admin'`, `'site'`                                                                      |
| `severity`   | ✅       | Error severity level                | `'error'`, `'warning'`, `'critical'`                                                              |
| `feature`    | ✅       | Feature/route identifier            | `'offers/create'`, `'branding/generate'`, `'crm/leads/delete'`                                    |
| `error_code` | ✅       | Specific error type                 | `'AUTH_FAILED'`, `'VALIDATION_FAILED'`, `'DB_ERROR'`, `'OAUTH_FAILED'`                            |
| `message`    | ✅       | Human-readable description          | `'Authentication failed in offers create'`                                                        |
| `context`    | ❌       | Additional error details (AI-ready) | `{ auth_error: '...', userId: '...', db_table: '...', file_path: '...', possible_causes: [...] }` |

### **AI-Enhanced Context Fields**

The `context` object supports AI-assisted debugging with these optional fields:

**Code Location:**

- `file_path` - Exact file where error occurred
- `line_number` - Line number in file
- `function_name` - Function/method name

**Database Context:**

- `db_table` - Table involved in operation
- `db_query` - Query that failed
- `db_expected` - Expected data/result
- `db_actual` - Actual data/result received

**AI Diagnosis:**

- `possible_causes[]` - Array of potential root causes
- `suggested_fixes[]` - Array of suggested solutions
...
For React components and client-side code, use `logErrorClient()`:

```typescript
import { logErrorClient } from '@/lib/services/error-logger.service'
...
const handleAction = async () => {
  try {
    ...
    await logErrorClient({
      app: 'app',
      severity: 'error',
      feature: 'funnels/list-load',
      error_code: 'FUNNELS_LOAD_FAILED',
      message: error instanceof Error ? error.message : 'Failed to load funnels',
      context: {
        userId: user?.id,
...
**Note:** `logErrorClient()` calls `/api/log/error` endpoint which uses `logErrorServer()` internally.

### **Benefits of Comprehensive Error Logging**

- **🔍 Complete Visibility:** See all auth failures and validation errors in admin dashboard
- **🚨 Proactive Monitoring:** Identify issues before users report them
- **📊 Usage Analytics:** Track which endpoints have the most failures
- **🐛 Easy Debugging:** Rich context for troubleshooting production issues
- **📈 Performance Insights:** Identify bottlenecks and problematic routes
- **🔐 Security Monitoring:** Detect authentication attacks and suspicious activity
- **🤖 AI-Assisted Debugging:** Structured context helps AI diagnose issues faster

### **Admin Dashboard Access**

View logged errors at:

- **Admin Dashboard:** `/admin/errors`
- **API Endpoint:** `GET /api/admin/errors` (admin role required)
- **Database Table:** `app_errors` in Supabase

### **New Route Checklist**

Before deploying any new API route:

- [ ] `logErrorServer` imported from `@/lib/services/error-logger.service`
- [ ] Authentication failures logged with `AUTH_FAILED` error code
- [ ] Validation failures logged with `VALIDATION_FAILED` error code
- [ ] Meaningful `feature` identifier specified (e.g., `'users/profile/update'`)
- [ ] Error context includes relevant debugging information
- [ ] Test route to confirm errors appear in admin dashboard

---

<!-- ============================================================================
   SECTION 10: COMMON CODING PATTERNS
   ============================================================================ -->
...
<!-- ============================================================================
   SECTION 11: AI/LLM INTEGRATION PATTERNS
   ============================================================================ -->
...
<!-- ============================================================================
   SECTION 12: WEB SCRAPING WITH FALLBACK
   ============================================================================ -->

## 12. Web Scraping with Fallback
...
```typescript
// ❌ WRONG - Don't use individual scraping services directly
import { FirecrawlService } from '@/lib/services/firecrawl'

const scraped = await FirecrawlService.scrapeWebsite(url)
```

### **How It Works**

1. **Primary (Firecrawl)**: Fast, comprehensive scraping with retries
2. **Fallback (WebCrawlerAPI)**: Automatic fallback if Firecrawl fails
3. **Error Handling**: Returns detailed error if both services fail
...
```typescript
// ❌ WRONG - Don't pass scraped content directly to AI without validation
const scraped = await ScrapingService.scrapeWebsite(url)
await AIService.generate(scraped.markdown) // May process sign-in pages!
```
...
See `@.project-architecture.md` for complete feature creation guidelines.
...
- [ ] All errors appear in admin dashboard (`/admin/errors`)
...
- File organization & structure: `@.project-architecture.md`
- UI/UX patterns & design tokens: `@.design_guidelines.md`
- Backend architecture (NestJS): `@.backend-architecture.md`
