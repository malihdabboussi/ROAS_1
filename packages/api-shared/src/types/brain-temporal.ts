...
export const BRAIN_TEMPORAL_STATUSES: BrainTemporalStatus[] = [
  'current',
  'historical',
  'superseded',
  'contradicted',
...
export const BRAIN_RETRIEVAL_TIME_MODES: BrainRetrievalTimeMode[] = [
  'default',
  'as_of',
  'timeline',
  'evolution',
...
const TEMPORAL_INPUT_ALIASES: Record<keyof BrainTemporalPayload, string[]> = {
...
export function normalizeTemporalPayload(input: unknown): BrainTemporalPayload {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return {}
  const record = input as Record<string, unknown>
  const result: BrainTemporalPayload = {}

  for (const key of ['episode_id', 'temporal_status', 'temporal_source'] satisfies Array<
    keyof BrainTemporalPayload
  >) {
  ...
  for (const key of [
    ...
    'evidence_started_at',
    'evidence_ended_at',
  ] satisfies Array<keyof BrainTemporalPayload>) {
...
export function temporalInsertFields(input: unknown): Record<string, unknown> {
...
export function temporalCandidateMetadata(row: Record<string, unknown>): BrainTemporalMetadata {
  ...
  const temporalStatus =
    typeof row.temporal_status === 'string' && row.temporal_status.trim()
  ...
  return {
    ...
    temporal_confidence:
      typeof row.temporal_confidence === 'number' ? row.temporal_confidence : null,
    temporal_source:
      typeof row.temporal_source === 'string' && row.temporal_source.trim()
...
export function temporalLabel(input: BrainTemporalPayload): string {
  ...
  if (occurredAt) return `Happened ${formatDateLabel(occurredAt)}`
