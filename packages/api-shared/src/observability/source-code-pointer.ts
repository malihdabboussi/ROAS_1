...
export function resolveReleaseContext(env: NodeJS.ProcessEnv = process.env): ReleaseContext {
  const commitSha =
    textValue(env.VIBEY_COMMIT_SHA, 80) ??
    ...
    null
  const releaseId =
    textValue(env.VIBEY_RELEASE_ID, 160) ??
    ...
    commitSha
  const buildId =
    textValue(env.VIBEY_BUILD_ID, 160) ??
...
export function parseFirstStackFrame(stack?: string | null): SourceCodePointer | null {
...
export function sanitizeSourceCodePointer(input: SourceCodePointer): SourceCodePointer {
...
export function normalizeRequestTraceEvent(input: RequestTraceEventInput): Record<string, unknown> {
