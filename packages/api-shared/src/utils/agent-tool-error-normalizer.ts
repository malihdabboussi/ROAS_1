...
function readRetryPolicy(value: unknown): string | undefined {
  if (typeof value === 'string' && value.trim().length > 0) return truncateText(value)
  if (!isRecord(value)) return undefined
  return readString(value, 'mode') ?? readString(value, 'reason')
}

function readObservability(value: unknown): NormalizedAgentToolFailureFields['observability'] {
  if (!isRecord(value)) return undefined
  const fingerprint = readString(value, 'fingerprint')
  const reportLevel = readString(value, 'report_level')
  if (!fingerprint && !reportLevel) return undefined
  return {
...
function candidateRecords(value: unknown): Array<Record<string, unknown>> {
  ...
  const records: Array<Record<string, unknown>> = [value]
  if (isRecord(value.details)) records.unshift(value.details)
  if (isRecord(value.result)) records.push(value.result)
  if (isRecord(value.data)) records.push(value.data)
  const envelope = extractJsonTextEnvelope(value)
  if (envelope) {
    records.unshift(envelope)
    if (isRecord(envelope.details)) records.unshift(envelope.details)
  }
  return records
}

function looksLikeToolError(record: Record<string, unknown>): boolean {
  return (
...
export function normalizeAgentToolFailureFields(
  value: unknown,
): NormalizedAgentToolFailureFields {
  const fields: NormalizedAgentToolFailureFields = {}
  for (const record of candidateRecords(value)) {
    if (!looksLikeToolError(record)) continue
