/**
 * Legacy integration HTTP routing for agent-api → main API proxy.
 * Stored in integration_capabilities.route_config and used by ComposioService.syncCapabilities.
 */
...
export type IntegrationLegacyRouteConfig = {
  ...
  path: string
  /** Map param key → query string key */
  query_params?: Record<string, string>
  fixed_query?: Record<string, string>
  alt_path?: string
  alt_when?: string
  /** For GET: append remaining scalar params as query (same key names). */
  query_remainder?: boolean
  /** When set, request is POSTed to `path` (GHL proxy) with upstream method/path in body. */
...
function resolvePathParamValue(
  params: Record<string, unknown>,
  name: string,
): { value: unknown; usedKey: string } {
  ...
  throw new Error(
    `Integration path parameter "${name}" is required. Pass integration action inputs inside data.params using the exact parameter names returned by get_integration or search_available_integrations.`,
  )
}

export function buildLegacyIntegrationHttpRoute(
  config: IntegrationLegacyRouteConfig,
  params: Record<string, unknown>,
): { method: LegacyHttpMethod; path: string; body?: Record<string, unknown> } {
  if (config.ghl_proxy) {
    const { gh_method, gh_path: ghPathTemplate, gh_version } = config.ghl_proxy
    const pathParamNames: string[] = []
    const pathParamKeys: string[] = []
    const resolvedGhPath = ghPathTemplate.replace(/:([a-zA-Z0-9_]+)/g, (_, name: string) => {
    ...
    const gh_query: Record<string, string> = {}
    for (const [k, v] of Object.entries(config.fixed_query ?? {})) {
      gh_query[k] = v
    }
    const usedKeys = new Set<string>([...pathParamNames, ...pathParamKeys])
    for (const [paramKey, queryKey] of Object.entries(config.query_params ?? {})) {
      if (params[paramKey] === undefined || params[paramKey] === null) continue
    ...
    if (Object.keys(gh_query).length > 0) proxyBody.gh_query = gh_query
    if (gh_version) proxyBody.gh_version = gh_version

    if (gh_method === 'POST' || gh_method === 'PUT' || gh_method === 'PATCH') {
      const gh_body: Record<string, unknown> = { ...params }
      ...
      if (Object.keys(gh_body).length > 0) proxyBody.gh_body = gh_body
    }
  ...
  const pathParamNames: string[] = []
  const pathParamKeys: string[] = []
  const resolvedPath = pathTemplate.replace(/:([a-zA-Z0-9_]+)/g, (_, name: string) => {
    pathParamNames.push(name)
    const resolved = resolvePathParamValue(params, name)
    pathParamKeys.push(resolved.usedKey)
    return encodeURIComponent(String(resolved.value))
...
const CALENDLY_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const STRIPE_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const PAYPAL_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
  search_transactions: {
    method: 'GET',
...
const FIREFLIES_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const GOHIGHLEVEL_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
  upsert_lead_contact: { method: 'POST', path: `${LHG}/upsert-lead-contact` },
}

const DROPBOX_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const ACTIVE_CAMPAIGN_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const FANBASIS_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
/** ScrapeCreators proxy: GET with query params forwarded to upstream (see apps/api scrapecreators controller). */
const SCRAPECREATORS_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
  tiktok_profile: {
    method: 'GET',
...
const DATAFORSEO_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
/**
 * Slack native agent tools — backed by SlackApiIntegration with the Vibey bot
 * token. Scope is limited to what the Vibey Slack bot can see in the
 * workspace (channels it was added to, DMs with it, etc.).
 */
const SLACK_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
const WORDPRESS_ROUTES: Record<string, IntegrationLegacyRouteConfig> = {
...
/** integration_id → action_slug → config (no youtube: no first-party HTTP route). */
export const LEGACY_INTEGRATION_ROUTE_MAP: Record<
  string,
  Record<string, IntegrationLegacyRouteConfig>
> = {
...
export function getLegacyIntegrationRouteConfig(
  integrationId: string,
  actionSlug: string,
): IntegrationLegacyRouteConfig | null {
  const iid = integrationId.trim().toLowerCase()
  const slug = actionSlug.trim()
  return LEGACY_INTEGRATION_ROUTE_MAP[iid]?.[slug] ?? null
}

/** Flat rows for SQL backfill / tooling. */
export function listLegacyIntegrationRouteRows(): Array<{
  ...
  for (const [integration_id, actions] of Object.entries(LEGACY_INTEGRATION_ROUTE_MAP)) {
