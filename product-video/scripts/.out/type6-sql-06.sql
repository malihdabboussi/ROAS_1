INSERT INTO skill_library_resources (skill_key, file_path, content, content_type, created_at) VALUES ('carousel-designer', $body_fp_41$references/type6-components/MarketingMissionDetailModalMockup.md$body_fp_41$, $body_c_41$# DeliverableVisualPreview

> Artifact preview (ad / funnel / email); use as slide body.

## Location

- Web-library path: `product-video/src/web-library/components/marketing/MarketingMissionDetailModalMockup.tsx`
- Website source: `apps/website/src/components/marketing/MarketingMissionDetailModalMockup.tsx`
- Import alias: `@/components/marketing/MarketingMissionDetailModalMockup`
...
- `body-3`
- `btn-icon-glass`
- `btn-icon-glass-destructive`
- `btn-icon-glass-sm`
- `btn-icon-glass-yellow`
- `button-glass-neutral`
- `card-glass`
- `disabled:opacity-30`
- `group`
- `group-hover:text-app-muted`
- `icon-md`
- `icon-sm`
- `input-glass`
- `italic`
...
```
$body_c_41$, $body_ct_41$text/markdown$body_ct_41$, now()) ON CONFLICT (skill_key, file_path) DO UPDATE SET content = EXCLUDED.content, content_type = EXCLUDED.content_type;
INSERT INTO skill_library_resources (skill_key, file_path, content, content_type, created_at) VALUES ('carousel-designer', $body_fp_42$references/type6-components/MarketingMissionExecutionMockup.md$body_fp_42$, $body_c_42$# MissionQuickCaptureChrome

> Product component; reuse verbatim in the slide TSX for 1:1 website fidelity.

## Location

- Web-library path: `product-video/src/web-library/components/marketing/MarketingMissionExecutionMockup.tsx`
- Website source: `apps/website/src/components/marketing/MarketingMissionExecutionMockup.tsx`
- Import alias: `@/components/marketing/MarketingMissionExecutionMockup`
      ...
      <div className="card-glass w-full overflow-hidden">
        <div className="bg-deep-muted relative flex max-h-56 min-h-[13.5rem] flex-col p-4">
          <div className="border-color-glass mb-3 flex shrink-0 items-center justify-between border-b border-white/10 pb-2">
            <span className="text-app-muted text-[10px] font-semibold uppercase tracking-wider">Preview</span>
            <span className="rounded bg-red-500/90 px-1.5 py-0.5 text-[9px] font-bold text-white">PDF</span>
          </div>
          <div className="text-app-foreground min-h-0 flex-1 select-none overflow-hidden">
            ...
            <p className="body-4 mb-1 leading-snug opacity-90">
              <span className="font-medium text-app-foreground">Email 1 — Problem.</span>{' '}
              <span className="text-app-muted">
                Open on the cost of a leaky funnel: missed follow-ups, long cycles, and demos that never
                convert. Anchor on one metric your ICP already tracks.
              </span>
            </p>
            <p className="body-4 mb-1 leading-snug opacity-90">
              <span className="font-medium text-app-foreground">Email 2 — Reframe.</span>{' '}
              <span className="text-app-muted">
                Show how teams like theirs operationalize nurture without adding headcount—brief proof,
                one customer snapshot, no hype.
              </span>
            </p>
            <p className="body-4 line-clamp-2 leading-snug opacity-90">
              <span className="font-medium text-app-foreground">Email 3 — CTA.</span>{' '}
              <span className="text-app-muted">
                Single ask: book a 20-minute working session. Restate value, remove risk (what to expect),
                link above the fold.
              </span>
            </p>
          </div>
...
function pickThreePortraitUrls(agents: PublicAgentLibraryRow[] | undefined): string[] {
  const list =
    agents != null && agents.length >= 3 ? agents : MARKETING_AGENT_LIBRARY_FALLBACK
...
export function MissionQuickCaptureChrome(props: {
  ...
  return (
    <div className={cn('input-glass rounded-spacing-3 relative flex flex-col', compact && 'rounded-lg')}>
      ...
      <div className={cn('flex items-center justify-between px-3', compact ? 'py-1.5' : 'py-2')}>
        ...
        <div className="flex items-center gap-1">
          <span
            className={cn(
              'text-color-muted tabular-nums',
              compact ? 'typo-caption' : 'body-4',
            )}
          >
          ...
          <span
            className="tooltip"
            data-tooltip={
              phase === 'done' ? 'Completed' : !inputValue.trim() ? 'Choose a campaign first' : 'Send mission'
            }
          >
            <button
              type="button"
              ...
              {isCEOPlanning ? (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40">Breaking brief into specialized subtasks</p>
                  </div>
                </motion.div>
              ) : null}
            </AnimatePresence>
                      ...
                      return (
                        <motion.div
                          ...
                          className={`glass-card relative overflow-hidden rounded-xl border p-4 transition-colors ${
                            isWorking ? 'border-white/20 bg-white/[0.05]' : 'border-white/5 bg-white/[0.02]'
                          }`}
                        >
...
```
$body_c_42$, $body_ct_42$text/markdown$body_ct_42$, now()) ON CONFLICT (skill_key, file_path) DO UPDATE SET content = EXCLUDED.content, content_type = EXCLUDED.content_type;
