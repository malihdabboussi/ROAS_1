...
const FEATURE_CARDS: {
  ...
  title: string
  subtitle: string
  href: 'the-brain' | 'missions' | 'autopilot' | 'your-team'
}[] = [
  {
    id: 'brain',
    title: 'The Brain',
    subtitle:
      'Three layers of memory. Your voice, your audience, your strategy: applied to every output.',
    href: 'the-brain',
  },
  {
    id: 'missions',
    title: 'Missions',
    subtitle: 'Describe the outcome. Your team plans, executes, and delivers: you review.',
    href: 'missions',
  },
  {
    id: 'autopilot',
    title: 'Autopilot',
    subtitle: 'Define your strategy. Close the tab. Wake up to finished work.',
    href: 'autopilot',
  },
  {
    id: 'team',
    title: 'Your Team',
    subtitle:
      'Specialist agents you hire, train, and manage. Each with their own memory, skills, and tools.',
    href: 'your-team',
  },
]
...
function FeatureCardVisual({
  ...
  if (id === 'missions') {
    return (
      <div className="ml-8 flex h-20 w-36 items-end gap-1.5">
        {[
          { label: 'Planning', color: 'bg-purple-500/60', cards: 2 },
          { label: 'In Progress', color: 'bg-blue-500/60', cards: 3 },
          { label: 'Done', color: 'bg-emerald-500/60', cards: 2 },
        ].map((col) => (
  ...
  if (id === 'autopilot') {
    return (
      <div className="ml-8 flex h-20 w-36 flex-col items-center justify-center gap-2">
        <div className="flex items-center gap-2">
          <span className="relative flex h-3 w-3">
            <span className="bg-emerald-accent absolute inline-flex h-full w-full animate-ping rounded-full opacity-40" />
            <span className="bg-emerald-accent relative inline-flex h-3 w-3 rounded-full" />
          </span>
          <span className="text-color-muted text-[8px] font-medium">Vibey is working...</span>
        </div>
        <div className="border-strong bg-color-surface w-full rounded border px-2 py-1.5 shadow-sm">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-color-muted text-[6px]">Campaign progress</span>
            <span className="text-emerald-accent text-[6px]">73%</span>
          </div>
          <div className="bg-mock-white-08 h-1 w-full overflow-hidden rounded-full">
            <div className="bg-emerald-accent h-full rounded-full" style={{ width: '73%' }} />
          </div>
        </div>
      </div>
    )
        ...
        <div className="studio-home-hero-shell flex min-h-0 flex-1 flex-col">
          <div className="site-container relative z-10 flex flex-1 items-center justify-center">
            <div className="p-spacing-8 mx-auto mt-4 w-full max-w-3xl md:mt-6">
              <div className="pointer-events-none mx-auto mb-4 h-[200px] w-[200px] sm:h-[260px] sm:w-[260px]">
                <VibeyHeroDepthOrb />
              </div>
              <Link
