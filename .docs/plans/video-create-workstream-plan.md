...
**P2 addendum — completion ownership (2026-08-12).** The sweeper introduced a race: an agent's late `get_video_status` poll and a sweep could both observe the provider success and each upload assets/posters, debit credits, and mark the job succeeded (the `hasProviderUsageEvent` lookup is check-then-insert, so it does not hold under concurrency). Fixed with a DB-backed ownership protocol on `media_generation_jobs` (migration `20260812150000_media_generation_jobs_completion_claim.sql`): every terminal transition — success completion, provider failure/cancel, and the sweeper's 24h expiry — must first win an atomic claim (`completion_claimed_at`/`completion_claimed_by`; single conditional `UPDATE ... RETURNING`, retakable after a 10-minute stale window for crash recovery), and the credit debit is additionally gated by a never-expiring one-shot `billing_recorded_at` flip so a stale-claim retry can re-upload but never re-bill (at-most-once billing bias: a crash between the flip and the debit loses the debit, never doubles it). Losing callers reload the job and return the canonical terminal result, or `processing` while the winner finishes. Both paths share the protocol because both run through `ArtifactLegacyMediaStatusService.getVideoStatus`; the expiry path in `ArtifactMediaJobsSweeperService.failExpiredJob` acquires the same claim.
