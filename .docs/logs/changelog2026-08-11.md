...
## [2026-08-11 21:35] - [FIX]
What: Fixed pre-existing tsc errors blocking a clean `npx tsc --noEmit` in apps/web: removed the undeclared `spaces` prop passed to SidebarHomeFlyout; implemented and exported the missing `captureDelegationThought` in delegation-desk.service (creates an inbox space item on the desk via createSpaceItem, matching the delegation-intake pattern); named the delegation-desk-view test fixtures so indexed access no longer yields `SpaceItem | undefined`; guarded the possibly-undefined restore row in AgendaCardEventEntry.test (also retargets the restore query to the AgendaMinimizedEventEntry aria-label).
Why: DelegationDeskWorkspace imported a function that was never exported (runtime/build breaker from the Delegation Desk feature), and the remaining errors kept the typecheck red.
Impact: apps/web typecheck is clean; Delegation Desk "Add to desk" capture now has a real service implementation.
Files: apps/web/src/components/layout/sidebar/SidebarHqFlyouts.tsx, apps/web/src/features/spaces/services/delegation-desk.service.ts, apps/web/src/features/spaces/lib/delegation-desk-view.test.ts, apps/web/src/features/home/components/AgendaCardEventEntry.test.tsx
