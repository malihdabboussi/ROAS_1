...
## [2026-08-18 23:05] - [FIX]
What: UI pass (branch `claude/ui-pass-2026-08-18`). Sidebar: collapsed logo mark centred (both theme imgs were rendering side by side inside a 32px clip because `.hub-sidebar-logo-face` display:flex beat Tailwind `.hidden`); wordmark hover glyph left-aligned instead of centred under the wordmark. View tabs: `pinned_to_start` is now a display flag — pinned views render first (leading view → pinned → rest), several views can be pinned, drag-reorder keeps pins (drop into the pinned zone pins, drag out unpins), right-click a tab opens a quick menu (Pin to start / Unpin view / Customize view…); Meetings surface leads with All Meetings and pins Agenda by default (template pins Agenda for new spaces). Route titles: every dashboard route now ends in `| ROAS` (server metadata / thin server layouts for client segments), branded `not-found.tsx`, sidebar loading placeholder reserves the persisted width, `/home` conversations name the browser tab. Lists: name column fills the measured list width (All Tasks no longer truncates titles beside an empty gutter), due/start default width 150, overdue due dates read red in absolute format, Client Campaigns name column absorbs free width, tooltips on truncated client / account-manager / agent names, ops-desk greeting capitalises email-derived names. Meetings: materialize/resolve payload normalises all-day dates and drops attendees/URLs the API rejects (was 400 → "Could not add calendar meetings" toast on every load); all-day entries are skipped. A11y: 96 aria-labels on icon-only buttons + title tooltips on truncated names across sidebar, cells, cards, tabs. Artifacts type chips use the shared purple selected pill; Brain list shows pulse placeholders instead of literal "Loading" / "—".
Why: Browser tour of every screen (Meetings, Home/chat, Inbox, All Tasks, Clients, Client Campaigns, Programs, Team, Brain, Flows, Artifacts, Spaces) plus a code audit of icon buttons, truncation and titles surfaced these; pin semantics were contradictory between the API (append + flag) and the panel (move + clear others).
Impact: Pinned tabs sit first everywhere; Meetings shows All Meetings, Agenda, then the rest; no recurring error toast on Meetings; tabs are distinguishable in the browser; long names are hover-readable; assistive tech gets names for close/sort/more controls.
Files: `apps/web/src/components/layout/sidebar/SidebarHqHubLogoButton.tsx`, `apps/web/src/app/globals.css`, `apps/website/src/app/globals.css`, `apps/web/src/features/spaces/lib/order-views-for-strip.ts`, `apps/web/src/features/spaces/lib/list-grid-template.ts`, `apps/web/src/features/spaces/components/ViewTabContextMenu.tsx`, `ViewSwitcher.tsx`, `containers/SpaceItemsContainer.tsx`, `hooks/use-customize-view-actions.ts`, `components/ListView.tsx`, `DraggableColumnHeaders.tsx`, `views/calendar/SpaceCalendarDayTaskList.tsx`, `apps/web/src/app/(dashboard)/home/meetings/MeetingsUnifiedSurface.tsx`, `apps/api/src/modules/space-templates/data/space-template-catalog-personal-dashboard.ts`, `apps/web/src/app/not-found.tsx`, `apps/web/src/app/(dashboard)/**/layout.tsx|page.tsx` (metadata), `dashboard-sidebar.client.tsx`, `apps/web/src/components/shell/use-shell-conversation-document-title.ts`, `ShellWorkspace.tsx`, `apps/web/src/components/spaces/cells/DueDateCell.tsx`, `apps/web/src/features/agency-clients/ClientCampaignsPage.tsx`, `AgencyClientsTable.tsx`, `apps/web/src/features/home/services/meeting-workspace-api.ts`, `apps/web/src/features/home/hooks/use-meetings-calendar-materialize.ts`, `apps/web/src/features/team-2/lib/ops-desk-summary.ts`, `apps/web/src/features/artifacts/components/GlobalArtifactsPage.tsx`, `apps/web/src/features/brain/components/BrainHomeListView.tsx`, ~80 files with aria-label/title additions.
