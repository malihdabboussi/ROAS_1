-- Append-only message timeline events (Vibey)
-- Purpose: persist a deterministic, ordered UI timeline per assistant turn/message.
...
-- Read: users can only read their own timeline events.
CREATE POLICY "Users can view own message timeline events"
  ON vb_message_timeline_events FOR SELECT
  USING (auth.uid() = user_id);

-- Insert: allow users to insert their own events (service role bypasses RLS anyway).
