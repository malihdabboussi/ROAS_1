-- Pixel (agent_key vibey) live-page QC: require a real browser open plus
-- desktop 1440x900 and mobile 390x844 snapshots. The previous section already
-- authorized a test-lead submit, but Pixel still QC'd via web_fetch because
-- the browser tool stayed denied for vibey and the contract never required
-- both viewports.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id = tools_row.id
      ...
      AND content LIKE '%1440x900%'
      AND content LIKE '%390x844%'
