-- Let owned personal brains be shared to an organization through brain_shares.
-- Org-owned brains still use the brain's org_id. Personal brains use the share's entity_id
-- and require the viewer to be an active member of that organization.
  ...
  WHERE bs.brain_id = p_brain_id
    AND (
      ...
      OR (
        bs.entity_type = 'org'
        AND (
          (v_brain.org_id IS NOT NULL AND bs.entity_id = v_brain.org_id)
          OR (
            v_brain.org_id IS NULL
            AND EXISTS (
              SELECT 1
              FROM public.org_members om
              WHERE om.org_id = bs.entity_id
