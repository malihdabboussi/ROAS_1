-- Task status `inbox`: mission is queued for planning / user retry — show on board as Queue, not archived.
...
BEGIN
  ...
  -- Retry path: linked task stays visible as Queue (inbox) until planning runs again.
  IF NEW.status = 'inbox' AND OLD.status IN ('failed', 'dead_letter', 'blocked', 'error') THEN
    UPDATE tasks
    SET status = 'inbox',
        updated_at = NOW(),
        completed_at = NULL
...
UPDATE tasks t
SET status = 'inbox', updated_at = NOW()
FROM missions m
