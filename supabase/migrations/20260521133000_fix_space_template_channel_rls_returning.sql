-- Avoid channels RLS SELECT checks during template instantiation.
-- `INSERT ... RETURNING` requires the inserted channel to pass SELECT RLS, but
-- channel access depends on the membership row created immediately after it.
