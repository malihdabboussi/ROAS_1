-- Include emerged_from in brain home health connection totals (match graph + legend).
...
AS $$
  ...
  legend_connection_counts AS (
    SELECT a.brain_id, public.brain_legend_connection_counts(a.brain_id) AS counts
    FROM accessible a
    WHERE a.scope <> 'company'
  ),
  legend_connection_totals AS (
    SELECT
      lc.brain_id,
      lc.counts,
      COALESCE(
        (
          ...
          FROM jsonb_each_text(lc.counts)
        ),
        0
      )::bigint AS total
    FROM legend_connection_counts lc
  ),
  ...
  SELECT
    ...
    CASE
      ...
      ELSE COALESCE(lct.counts, '{}'::jsonb)
  ...
  LEFT JOIN legend_connection_totals lct ON lct.brain_id = a.brain_id
