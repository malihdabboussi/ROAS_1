...
CREATE INDEX IF NOT EXISTS idx_missions_completed_at
  ON public.missions(completed_at DESC)
  WHERE completed_at IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_missions_created_at
  ON public.missions(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_channel_messages_sender_created
  ON public.channel_messages(sender_id, created_at DESC)
  WHERE sender_type = 'agent';

CREATE INDEX IF NOT EXISTS idx_space_automation_runs_created
  ON public.space_automation_runs(created_at DESC);
...
REVOKE ALL ON FUNCTION public.get_team_overview(UUID, TIMESTAMPTZ, TIMESTAMPTZ) FROM public;
GRANT EXECUTE ON FUNCTION public.get_team_overview(UUID, TIMESTAMPTZ, TIMESTAMPTZ) TO authenticated;
