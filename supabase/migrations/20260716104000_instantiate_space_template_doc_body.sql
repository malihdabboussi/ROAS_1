-- Backfill template/space docs that only have custom_data.body.
UPDATE public.space_items
SET doc_body = custom_data->>'body'
WHERE custom_data ? 'body'
  AND NULLIF(BTRIM(COALESCE(custom_data->>'body', '')), '') IS NOT NULL
  AND (
    doc_body IS NULL
    OR LENGTH(TRIM(doc_body)) = 0
  );


-- Fix template docs writing only custom_data.body (Doc cards read doc_body).
-- Keep channel RLS RETURNING behavior from 20260521133000.
