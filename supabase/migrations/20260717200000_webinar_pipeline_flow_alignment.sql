...
5. Explicit "gaps / inferred" section for anything not tool-observed$roas_market_research_body$,
  'agency_research',
  now()
)
...
SELECT NULL, NULL, 'designer', 'roas-ad-design', 'assets/samples/sample_dark_bold.png', NULL, 'image/png', NULL
WHERE NOT EXISTS (
...
INSERT INTO public.agent_skill_resources (user_id, org_id, agent_key, skill_key, file_path, content, content_type, storage_url)
SELECT NULL, NULL, 'designer', 'roas-ad-design', 'assets/samples/sample_dark_marker.png', NULL, 'image/png', NULL
WHERE NOT EXISTS (
  SELECT 1 FROM public.agent_skill_resources
   WHERE user_id IS NULL AND org_id IS NULL AND agent_key = 'designer'
...
INSERT INTO public.agent_skill_resources (user_id, org_id, agent_key, skill_key, file_path, content, content_type, storage_url)
SELECT NULL, NULL, 'designer', 'roas-ad-design', 'assets/samples/sample_light_marker.png', NULL, 'image/png', NULL
WHERE NOT EXISTS (
  SELECT 1 FROM public.agent_skill_resources
   WHERE user_id IS NULL AND org_id IS NULL AND agent_key = 'designer'
...
INSERT INTO public.agent_skill_resources (user_id, org_id, agent_key, skill_key, file_path, content, content_type, storage_url)
SELECT NULL, NULL, 'designer', 'roas-ad-design', 'assets/zoom_logo.png', NULL, 'image/png', NULL
WHERE NOT EXISTS (
  SELECT 1 FROM public.agent_skill_resources
   WHERE user_id IS NULL AND org_id IS NULL AND agent_key = 'designer'
...
INSERT INTO public.template_skill_assignments (template_key, skill_key, is_enabled)
VALUES ('designer', 'roas-ad-design', true)
ON CONFLICT (template_key, skill_key) DO UPDATE SET is_enabled = true;
...
-- Backfill roas-ad-design to already-hired agents carrying roas-webinar-deck
INSERT INTO public.agent_skills (
  user_id, org_id, agent_key, skill_key, name, description, markdown_content, is_enabled, source
)
SELECT
...
WHERE marker.skill_key = 'roas-webinar-deck'
  AND library.skill_key = 'roas-ad-design'
  AND (marker.user_id IS NOT NULL OR marker.org_id IS NOT NULL)
  AND marker.source IN ('template', 'system', 'default')
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skills existing
...
FROM public.agent_skills target
JOIN public.skill_library_resources resource
  ON resource.skill_key = target.skill_key
WHERE target.skill_key = 'roas-ad-design'
  AND (target.user_id IS NOT NULL OR target.org_id IS NOT NULL)
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skill_resources existing
    WHERE existing.agent_key = target.agent_key
      AND existing.skill_key = target.skill_key
      AND existing.file_path = resource.file_path
      AND existing.user_id IS NOT DISTINCT FROM target.user_id
      AND existing.org_id IS NOT DISTINCT FROM target.org_id
  );

-- roas-ad-copy (copywriter)
INSERT INTO public.skill_library (skill_key, name, description, markdown_content, category, updated_at)
VALUES (
  'roas-ad-copy',
  $roas_ad_copy_name$Roas Ad Copy$roas_ad_copy_name$,
  $roas_ad_copy_desc$Writes Meta (Facebook and Instagram) ad copy for ROAS client campaigns. Produces the full ad (primary text and hook, image/overlay copy, headline, description, CTA) in the client's voice. Two standing requirements are built in. Every deliverable includes a VALIDATE MESSAGING set, 4-5+ short "if you've / if you are a / if your" identity callouts that each speak to a different audience segment for the same offer. And every new campaign starts with AD LIBRARY RESEARCH, pulling 3-5 competitor and top-performer references from the Meta Ad Library before writing. Load for Meta ad copy, Facebook or Instagram ads, ad creative, hooks, primary text, overlay or image copy, validate messaging sets, identity callouts, or ad library research. Triggers on "write ad copy," "Meta ads," "Facebook ad," "ad creative," "validate messaging," "ad hooks," or a client name paired with an ad campaign. Do NOT load for landing or registration pages, webinar emails (use the webinar skills), or non-ad long-form copy.$roas_ad_copy_desc$,
  $roas_ad_copy_body$# ROAS Ad Copy — Meta Ads with Validate Messaging built in
...
The ad earns the registration click; the webinar skills (`roas-webinar-emails`, `roas-master-webinar`) take it from there. Keep the identity/angle of the winning ads consistent with the webinar's audience and promise so the funnel reads as one message start to finish.$roas_ad_copy_body$,
  'agency_copy',
  now()
)
ON CONFLICT (skill_key) DO UPDATE SET
...
INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-ad-copy', 'references/ad-anatomy.md', $roas_ad_copy_references_ad_anatomy_md$# Meta Ad Anatomy — what you're actually writing
...
$roas_ad_copy_references_ad_anatomy_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-ad-copy', 'references/ad-library-research.md', $roas_ad_copy_references_ad_library_research_md$# Ad Library Research — mine the market before writing a line
...
$roas_ad_copy_references_ad_library_research_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-ad-copy', 'references/validate-messaging.md', $roas_ad_copy_references_validate_messaging_md$# Validate Messaging — the identity-callout testing set
...
$roas_ad_copy_references_validate_messaging_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-ad-copy', 'assets/swipe-meta-ads.md', $roas_ad_copy_assets_swipe_meta_ads_md$# SWIPE — Meta ad references (annotated)
...
$roas_ad_copy_assets_swipe_meta_ads_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.template_skill_assignments (template_key, skill_key, is_enabled)
VALUES ('copywriter', 'roas-ad-copy', true)
ON CONFLICT (template_key, skill_key) DO UPDATE SET is_enabled = true;
...
FROM public.skill_library_resources r
WHERE r.skill_key = 'roas-ad-copy'
ON CONFLICT (agent_key, skill_key, file_path) WHERE user_id IS NULL AND org_id IS NULL DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type,
  updated_at = now();

-- Backfill roas-ad-copy to already-hired agents carrying roas-webinar-copy-package
INSERT INTO public.agent_skills (
  user_id, org_id, agent_key, skill_key, name, description, markdown_content, is_enabled, source
)
SELECT
...
FROM public.agent_skills marker
CROSS JOIN public.skill_library library
WHERE marker.skill_key = 'roas-webinar-copy-package'
  AND library.skill_key = 'roas-ad-copy'
  AND (marker.user_id IS NOT NULL OR marker.org_id IS NOT NULL)
  AND marker.source IN ('template', 'system', 'default')
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skills existing
...
JOIN public.skill_library_resources resource
  ON resource.skill_key = target.skill_key
WHERE target.skill_key = 'roas-ad-copy'
  AND (target.user_id IS NOT NULL OR target.org_id IS NOT NULL)
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skill_resources existing
    WHERE existing.agent_key = target.agent_key
...
- **Live event ticket** → registration link + seats remaining + event date$roas_webinar_emails_body$,
  'agency_copy',
  now()
)
ON CONFLICT (skill_key) DO UPDATE SET
...
INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'references/angles.md', $roas_webinar_emails_references_angles_md$# Winning Angles — the plays that drive the FOMO, and when each one is allowed

These are the transferable angles behind the proven flows in `assets/`. Each one has a condition. Using an angle when its condition isn't met is how a sequence reads as fake and burns trust. Match the angle to what actually happened.

---

## REPLAY-PHASE ANGLES (days 1 through the takedown)

### "I revealed too much" / "I think I messed up"
The day-after pattern interrupt. The presenter half-apologizes for going too deep, says people told them they over-shared, and frames the replay takedown as self-protection ("if my competitors get this I'm in trouble"). This is what makes the replay feel valuable and the deadline feel real.

**CONDITION — only use when the webinar actually went deep.** If the presenter genuinely revealed the system, the frameworks, the real numbers, this lands because it's true. If the webinar just touched the surface and was mostly a pitch, this angle is nonsense and the audience knows it. Don't fake depth. When the content was thin, drop this angle and lead the replay drive with the outcome/proof instead ("here's the replay where I showed [specific result]").

### "did you watch it yet" + social proof of action
Day-2 follow-up. Light accountability ("did you see what I sent?") plus the number that creates FOMO: how many people already took action ("5 booked calls while still on the training, and I barely pitched"). The proof isn't the case studies here, it's that *other people in your exact position already moved.*

**CONDITION** — needs a real action number. If nobody booked/bought yet, use early-watcher reactions or the strongest case study instead. Never invent the number.

### The replay actually comes down
The deadline is only as strong as your willingness to execute it. The replay link must die when you said it would. After takedown the page should say something like "Replay Gone." A deadline you don't honor trains the list to ignore every future one.

**CONDITION** — always. If you're not willing to take it down, don't promise to. Use a different urgency mechanic (a bonus expiry, a price increase).

### "I might pull it even sooner"
Unpredictability multiplies urgency. "Friday at the latest... maybe sooner" stops people from assuming they have until the last minute.

---

## TAKEDOWN-DAY STACK (the 3-send day)

The day the replay comes down is the hinge of the whole flow. It gets three sends, and it's where the offer gets revealed/pushed because everyone who was going to watch has now watched.

- **Morning** — the offer reveal with scarcity. "Replay comes down tonight. Here's what happened after: [action number]. I opened [N] spots. [N-x] left." Both links: replay and offer.
- **Early evening** — "pulling it down tonight. This is the last time I send this link." Heavier on the loss.
- **Night (a few hours before)** — "last call." Two or three lines. Replay link for the stragglers, offer link for the fence-sitters. "This is the last email about this."

For a hard-cart offer instead of a call/application, the cart-close day gets this same 3-send stack, anchored to the cart deadline.

---

## OFFER-PHASE ANGLES (after the replay is gone, the weekend push)

### Diminishing spots countdown
The core FOMO engine for a call/application/limited offer. The number of remaining spots ticks down across the sequence and is stated in every send. Mike's ran 10 offered → 5 booked live → 3 (Fri) → 2 (Sat) → 1 (Sun).

**CONDITION** — the scarcity has to be real or genuinely capped. A countdown that never actually closes is the fastest way to become noise. If the offer is unlimited, switch to a deadline (price increase, bonus expiry, cohort start date) as the urgency instead of spot count.

### "which one are you?"
The Saturday move. After a week of sharing, split the list in the reader's own head: "some people watched and moved on, some people booked and are already getting it built. Which one are you?" Identity-level pressure without insulting anyone.

### "one more thing before I move on" / honest final
The last email. Radical honesty: "most people who read this won't do anything, no hard feelings. But a few people will book and finally get it built." Removes the salesy energy and lets the self-selecting buyer raise their hand. Stack the best case studies here. State the final number ("1 spot left") and the real consequence of waiting ("next round opens after the next training").

### CTA destination depends on the offer
- **Call / application offer** → the booking/scheduling link, with the diminishing-spots countdown.
- **Cart / purchase offer** → straight to checkout, with a cart-close deadline and any fast-action bonus expiry.
- Either way the offer-phase emails point at ONE destination. The replay is gone; there's no second link competing anymore.

---

## REACTIVATION ANGLES (front end — driving registration from an aged list)

### "they told me not to do this"
The pattern interrupt that opens a reactivation push. The presenter is doing something their own team thinks is crazy (giving away the $X system for free, one time). Frames the webinar as a rare event.

### "it's not the tech, it's not the time"
Names the reader's stated excuses out loud, then reframes the real problem as a missing system (not a missing skill). Bridges to "that's what the training shows."

### "I'll be honest with you" (the aged-list call-out)
Acknowledges the reader has been on the list a while and never took the next step, without judgment. "You know better than I do why." Then: the people who launched weren't more ready, they just stopped waiting. Works specifically on cold/aged segments.

### "the room is still small / this goes to the cold list tomorrow"
Manufactured-but-true scarcity on registration. The link is currently only going to warm people; it hits the cold list next. Register before it's crowded.

---

## INTENSITY RULE (applies to every angle)

A reminder is not enough. Each send is a packet built to create FOMO and urgency, not a notification. Before any email or SMS ships, it has to carry at least one of: a real deadline counting down, a real scarcity (spots/seats), loss framing (what disappears and when), or proof that other people already moved. If a send has none of those, it's filler. Cut it or rebuild it.
$roas_webinar_emails_references_angles_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'references/email-craft.md', $roas_webinar_emails_references_email_craft_md$# Email Craft — Subject Lines, Structure, CTAs, Deliverability
...
$roas_webinar_emails_references_email_craft_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'references/post-webinar.md', $roas_webinar_emails_references_post_webinar_md$# Post-Webinar Engine — Two Phases: Replay Drive, then Offer FOMO

This is the proven structure (see `assets/swipe-post-webinar.md` for the real flow that doubled bookings). It is ONE machine that runs to the whole list, not two parallel sequences. Show vs no-show is a tone overlay handled at the bottom of this file, not a separate build.

The shape:

- **Phase 1 — Replay Drive (roughly the first 3 days):** every send pushes the replay. Urgency is the replay coming down, and it actually comes down. The last replay day is the takedown day and gets a multi-send stack where the offer gets revealed.
- **Phase 2 — Offer FOMO (the remaining days, usually the weekend):** replay is gone. Every send pushes the offer directly. Urgency is diminishing spots (for a call/application) or a cart-close deadline (for a purchase).

**One email + one SMS per day is the baseline.** The takedown day breaks the baseline with a 3-send stack. If the offer also has its own hard close, that close day gets a stack too. SMS is not optional. It is half the machine. Pair every email with an SMS that carries the same angle in two or three lines.

---

## PHASE 1 — REPLAY DRIVE

### Day 1 (morning after) — the pattern interrupt
Lead with the strongest replay angle available. If the webinar genuinely went deep, use "I revealed too much / I think I messed up" (see `references/angles.md` for the condition, do not use it if the content was thin). Frame the takedown as real and self-protective. Set the deadline and hint it could come even sooner.
- CTA: replay link.
- SMS: the same angle, condensed. "I might have revealed too much yesterday... replay's up but I might pull it early. Watch while you can: [link]"

### Day 2 — "did you watch it" + social proof of action
Light accountability plus the FOMO number: how many people already took action ("5 booked while still on the call, I barely pitched"). Stack 2-3 named case studies. Remind them it might come down sooner than promised.
- CTA: replay link. First soft offer mention goes in the P.S. only ("if you already watched and you're ready to talk, a few spots are open").
- SMS: the action number + watch tonight.

### Day 3 — TAKEDOWN DAY STACK (3 sends) — reveal and push the offer
This is the hinge. Everyone who'll watch has watched, so the offer comes forward now while the replay scarcity peaks.
- **Morning** — offer reveal with scarcity. "Replay comes down tonight. Here's what happened after: [action number]. I opened [N] spots. [N-x] left. Here's what happens on the call/what you get." Both links.
- **Early evening** — "pulling it down tonight. Last time I send this link." Heavier loss framing. Replay link, offer in P.S.
- **Night** — "last call." Two or three lines. Replay link for stragglers, offer link for fence-sitters. "This is the last email about this."
- Each of the three is paired with an SMS.
- **Then actually take the replay down.** The page should read "Replay Gone." A deadline you don't honor poisons every future send.
- Build note: the replay page should host the offer CTA directly below the video, so watching drops the viewer onto the booking calendar / checkout.

---

## PHASE 2 — OFFER FOMO (replay is gone)

Now there's one destination. The replay link is dead, so nothing competes with the CTA. Pure offer, pure scarcity, one email + one SMS per day.

### Day 4 — "which one are you?"
After a week of sharing, split the list in the reader's head: some watched and moved on, some booked and are already getting it built. Which are you? Restate what the call/offer is and the down-ticked spot count ("2 spots left, then I'm heads down on the pipeline").
- CTA: offer link only.
- SMS: condensed version with the spot count.

### Day 5 — "one more thing before I move on" / honest final
The last send. Radical honesty removes the salesy energy: most people won't act, no hard feelings, but a few self-selecting people will book and finally get it built. Stack the best case studies. Final number ("1 spot left") and the real cost of waiting ("next round opens after the next training").
- CTA: offer link only.
- SMS: best case studies condensed + final spot count + link.

If the cart window is longer than two days, extend Phase 2 by repeating the "which one are you / honest" logic with fresh proof and a tightening deadline. If the offer has a hard cart close, that close day gets its own 2-3 send stack (morning / hours-left / doors-closing), same as the takedown stack.

---

## CTA destination by offer type

- **Call / application / limited spots** → booking link, driven by the diminishing-spots countdown.
- **Cart / purchase** → checkout link, driven by a cart-close deadline plus any fast-action bonus expiry. Replace "spots left" language with "cart closes [time]" and give the close day its own stack.
- **Live event ticket** → registration/seats link, driven by seats remaining and the event date.

---

## Show vs No-Show — the tone overlay

The engine above is the same for both segments. If the ESP can't split them, run it blended (the proven swipe is mostly blended and works because it leads with replay and reveals the offer naturally). If it CAN split them, adjust tone only:

**Shows (attended):**
- Replay framing is "in case you want to rewatch" and "remember when I showed [X]." They were in the room; reward that.
- They already saw the offer live, so the Day-3 reveal lands as a reminder, not news. You can lean into the offer slightly faster.
- The "did you watch it" angle becomes "want to revisit any part of it."

**No-shows (registered, didn't attend):**
- Replay framing is "you missed it" / "did something come up?" Never guilt them; a judged no-show unsubscribes.
- The offer is genuinely new to them, so getting the replay watched matters more before the offer push. Keep them in Phase 1 logic a beat longer.
- By the takedown stack and Phase 2, treat them like everyone else. The hard close is the same for the whole list.

Keep a light "if you still haven't watched, the replay is [status]" line for the never-watched segment until the replay comes down. After that, the distinction is gone and everyone gets the offer push.
$roas_webinar_emails_references_post_webinar_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'references/pre-webinar.md', $roas_webinar_emails_references_pre_webinar_md$# Pre-Webinar Sequence — Fill the Room
...
$roas_webinar_emails_references_pre_webinar_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'assets/swipe-post-webinar.md', $roas_webinar_emails_assets_swipe_post_webinar_md$# SWIPE — Mike G. / Freedom Builderz post-webinar flow (PROVEN)

This is the real post-webinar sequence that doubled bookings on the back end: 5 booked live on the call, 5 more in the days after. Webinar ran Tuesday. Offer was 10 spots for free 1-on-1 calls. The CTA lived on the replay page itself, below the video.

Use this as a pattern-match for rhythm, FOMO mechanics, and angle construction. It is ONE client's voice and business. Adapt the angles, never copy the specifics. The transferable lessons are extracted in `references/angles.md`.

Note the cadence: one email + one SMS per day for the replay phase, then a 3-send STACK on the replay-takedown day (Friday), then one email + one SMS per day for the weekend offer push. Watch how the spot count ticks down: 10 offered → 5 booked live → 3 left (Fri) → 2 left (Sat) → 1 left (Sun).

---

## DAY 1 (Wed AM) — REPLAY DRIVE — "I revealed too much"

**EMAIL — Subject: I might have revealed too much...**

{{first_name}},

I had to email you to say sorry.

I had a couple people who came to the training yesterday say that I went way too deep into how to monetize online in 2026 and beyond.

...and as much as I don't side with the haters, they might be right.

I broke down the entire system behind 150+ online program launches... including the messaging framework, the offer structure, the funnel builds... all of it. Stuff (until now) I have only shared with our top level paid clients.

But honestly... I don't regret it.

Because if you've been sitting on expertise that could be an online program... you deserve to see exactly how this works.

So for that reason, I'm gonna let you watch the replay until Friday.

But I have to take the recording down on Friday at the latest... maybe sooner. Because if my competitors get their hands on the knowledge I shared, I'm going to be in serious trouble. So for those of you that have time today or tomorrow, here's the link: {{replay_link}}

Mike G.
Founder, Freedom Builderz

P.S. If you got value from this, my only ask: share my company with a friend who's looking to build an online course. Send them {{company_link}}.

**SMS:** {{first_name}}... I might have revealed too much at yesterday's training. A few people said I went way too deep. Replay is up but I might have to take it down before Friday... watch it while you can: {{replay_link}}

---

## DAY 2 (Thu AM) — REPLAY DRIVE — "did you watch it" + proof

**EMAIL — Subject: did you see what I sent yesterday?**

{{first_name}},

Quick question... did you see the replay I sent yesterday? If not... I get it. Life gets busy. But here's why I'm following up...

5 people booked calls with me while on the training. Not because I pitched hard. I barely pitched at all. Because they saw the full system... the case studies... the real numbers.

...A holistic health expert... $500K+ organically.
...A midwife... six figures in 6 weeks.
...A Wall Street trader with zero online presence... $100K+ in 30 days.

I meant what I said... I might have to pull the replay down sooner than Friday. So if you've been meaning to watch it... tonight would be a good night.

👉 {{replay_link}}

Mike G.

P.S. And if you already watched it and you're ready to talk about what a done-for-you build would look like... we have a few spots open this month: {{offer_link}}

**SMS:** {{first_name}}... did you see the replay yesterday? 5 people booked calls while still on the training. I barely even pitched. Replay's still up but I might pull it early... watch it tonight: {{replay_link}}

---

## DAY 3 (Fri) — REPLAY TAKEDOWN STACK — 3 sends — reveal the offer + scarcity

**EMAIL 1 (Fri AM) — Subject: I've got three if you want one...**

{{first_name}},

I've been sending you the replay from Tuesday's training... and the replay comes down tonight.

But whether you watched it or not... I want to tell you what happened after. 5 people booked calls with me while they were still on the training. And since then... more have come in.

So I want to be upfront... I opened up 10 spots this month for 1-on-1 calls. 7 are gone. 3 left.

Here's what happens on the call: I'll look at what you do... your expertise, your audience... and map out exactly what your online program build would look like. What you'd sell. Who you'd sell it to. How my team plugs in and builds the whole thing for you.

That's how a midwife went from zero to six figures in 6 weeks. That's how a Wall Street trader with no online presence did $100K+ in his first 30 days.

If it's the right fit, this could be the start of something great. If it's not a fit I'll tell you straight up.

3 spots left: {{offer_link}}

Mike G.

P.S. Last chance to watch the replay before I pull it tonight: {{replay_link}}

**SMS:** {{first_name}}... last reminder, the link I sent is being turned off tonight at midnight. This was the training where I offered 10 free 1-on-1 call spots to map out an entire program build for you. 3 spots left. Schedule: {{offer_link}}

**EMAIL 2 (Fri 5-6 PM) — Subject: pulling it down tonight.**

{{first_name}},

This is the last time I'm going to send you this link. {{replay_link}}

The replay comes down tonight. I told you I might pull it early and I meant it.

90 minutes. The full system behind 150+ program launches. Real case studies. Real numbers. I'm not running this training again anytime soon. And once the replay is gone... it's gone.

Watch it now: {{replay_link}}

Mike G.

P.S. If you already watched it and want to talk about working together... 2 spots left this month: {{offer_link}}

**SMS:** {{first_name}}... pulling the replay down tonight. This is the last time I'm sending this link: {{replay_link}}

**EMAIL 3 (Fri 9 PM) — Subject: last call.**

{{first_name}},

Replay comes down in a few hours. If you've been meaning to watch it... this is it: {{replay_link}}

And if you already watched it and you're still on the fence about booking a call... just book it. No pressure. I'll tell you straight up if it's a fit or not. 20 minutes. {{offer_link}}

This is the last email about this.

Mike G.

**SMS:** {{first_name}}... few hours left. Replay goes down at midnight. Last chance: {{replay_link}}

> Replay is actually taken down at midnight. The page now reads "Replay Gone." The scarcity was real and executed.

---

## DAY 4 (Sat AM) — OFFER FOMO — replay gone, call only

**EMAIL — Subject: quick question for you.**

{{first_name}},

I'll keep this short. Over the past week I shared more about how we build online programs than I ever have publicly. Some people watched and moved on. Some people booked calls and are already getting their programs built.

So here's my question... which one are you?

If there's even a small part of you that knows you're sitting on something that could be packaged into an online program... I want to talk to you. Not a pitch. Just a real conversation where I look at what you do and tell you exactly what the build would look like.

If it's a fit... great. If not... I'll tell you. But I only have 2 spots left this month. Once they're gone I'm heads down building for the clients already in the pipeline.

If you want one: {{offer_link}}

Mike G.

P.S. You don't need to have it all figured out before the call. That's literally what the call is for.

**SMS:** {{first_name}}... quick question. This past week I shared the full system behind 150+ launches. Some watched and moved on. Some booked calls and are getting their programs built. 2 spots left this month for a free 1-on-1 call. If you want one: {{offer_link}}

---

## DAY 5 (Sun AM) — OFFER FOMO — final push, 1 spot

**EMAIL — Subject: one more thing before I move on.**

{{first_name}},

This is the last email I'm sending about this. Most people who read these won't do anything. That's just how it goes. No hard feelings.

But a few people... the ones who know they're sitting on something... are going to book a call this week and finally get their program built.

A midwife did it and hit six figures in 6 weeks. A tattoo artist blew past her $25K goal on launch one. A Wall Street trader did $100K+ in his first 30 days with zero online presence.

They weren't more ready than you. They just stopped waiting and got on a call.

I have 1 spot left this month. After this... the next round won't open until after the next training.

If you want it: {{offer_link}}

Mike G.

P.S. If it's not a fit I'll tell you straight up. The call is 20 minutes. No pressure. But you do have to book it before the spot's gone.

**SMS:** {{first_name}}... last message about this. A midwife hit six figures in 6 weeks. A Wall Street trader did $100K+ in 30 days with zero online presence. It started with a call like the one I'm offering. 1 spot left. If you want it: {{offer_link}}
$roas_webinar_emails_assets_swipe_post_webinar_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.skill_library_resources (skill_key, file_path, content, content_type)
VALUES ('roas-webinar-emails', 'assets/swipe-pre-webinar.md', $roas_webinar_emails_assets_swipe_pre_webinar_md$# SWIPE — Mike G. / Freedom Builderz pre-webinar flow (PROVEN)
...
$roas_webinar_emails_assets_swipe_pre_webinar_md$, 'text/markdown')
ON CONFLICT (skill_key, file_path) DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type;

INSERT INTO public.template_skill_assignments (template_key, skill_key, is_enabled)
VALUES ('copywriter', 'roas-webinar-emails', true)
ON CONFLICT (template_key, skill_key) DO UPDATE SET is_enabled = true;
...
SELECT
  NULL, NULL, 'copywriter', r.skill_key, r.file_path, r.content, r.content_type, NULL
FROM public.skill_library_resources r
WHERE r.skill_key = 'roas-webinar-emails'
ON CONFLICT (agent_key, skill_key, file_path) WHERE user_id IS NULL AND org_id IS NULL DO UPDATE SET
  content = EXCLUDED.content,
  content_type = EXCLUDED.content_type,
  updated_at = now();

-- Backfill roas-webinar-emails to already-hired agents carrying roas-webinar-copy-package
INSERT INTO public.agent_skills (
  user_id, org_id, agent_key, skill_key, name, description, markdown_content, is_enabled, source
)
SELECT
...
CROSS JOIN public.skill_library library
WHERE marker.skill_key = 'roas-webinar-copy-package'
  AND library.skill_key = 'roas-webinar-emails'
  AND (marker.user_id IS NOT NULL OR marker.org_id IS NOT NULL)
  AND marker.source IN ('template', 'system', 'default')
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skills existing
...
JOIN public.skill_library_resources resource
  ON resource.skill_key = target.skill_key
WHERE target.skill_key = 'roas-webinar-emails'
  AND (target.user_id IS NOT NULL OR target.org_id IS NOT NULL)
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skill_resources existing
    WHERE existing.agent_key = target.agent_key
...
`Webinar Deck Bones` exists as a native editable 10-20 slide presentation, the complete offer stack is present, every unknown is visibly bracketed, and the mission subtask links to it.$roas_webinar_deck_body$,
  'agency_design',
  now()
)
ON CONFLICT (skill_key) DO UPDATE SET
...
-- Backfill dylans-super-voice to already-hired agents carrying roas-webinar-copy-package
INSERT INTO public.agent_skills (
  user_id, org_id, agent_key, skill_key, name, description, markdown_content, is_enabled, source
)
SELECT
...
CROSS JOIN public.skill_library library
WHERE marker.skill_key = 'roas-webinar-copy-package'
  AND library.skill_key = 'dylans-super-voice'
  AND (marker.user_id IS NOT NULL OR marker.org_id IS NOT NULL)
  AND marker.source IN ('template', 'system', 'default')
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skills existing
...
JOIN public.skill_library_resources resource
  ON resource.skill_key = target.skill_key
WHERE target.skill_key = 'dylans-super-voice'
  AND (target.user_id IS NOT NULL OR target.org_id IS NOT NULL)
  AND NOT EXISTS (
    SELECT 1 FROM public.agent_skill_resources existing
    WHERE existing.agent_key = target.agent_key
...
-- Backfill hired agent_skills copies from refreshed skill_library
...
WHERE s.skill_key = l.skill_key
  ...
  AND s.skill_key IN ('roas-market-research', 'roas-ad-kit', 'roas-ad-design', 'roas-ad-copy', 'roas-webinar-audit', 'roas-webinar-copy-package', 'roas-webinar-emails', 'roas-webinar-topics', 'roas-landing-page-copy', 'roas-video-ad-scripts', 'roas-image-brief', 'roas-funnel-design', 'roas-webinar-deck', 'instagram-carousel-design');
...
JOIN public.agent_skills AS owned ON owned.skill_key = library.skill_key
WHERE target.skill_key = library.skill_key
  ...
  AND target.skill_key IN ('roas-webinar-emails', 'roas-webinar-deck', 'roas-ad-copy');

DELETE FROM public.template_skill_assignments
WHERE template_key = 'ads_manager' AND skill_key = 'roas-ad-design';
...
WHERE resource.agent_key = skill.agent_key
  ...
  AND skill.skill_key = 'roas-ad-design'
  AND skill.agent_key IN ('ads_manager', 'blaze')
  AND skill.source IN ('template', 'system', 'default');

DELETE FROM public.agent_skills
WHERE skill_key = 'roas-ad-design'
  AND agent_key IN ('ads_manager', 'blaze')
  AND source IN ('template', 'system', 'default');

DELETE FROM public.template_skill_assignments WHERE skill_key = 'human-written-copy';
...
WHERE resource.agent_key = skill.agent_key
  ...
  AND resource.user_id IS NOT DISTINCT FROM skill.user_id
  AND resource.org_id IS NOT DISTINCT FROM skill.org_id
  AND skill.skill_key = 'human-written-copy'
  AND skill.source IN ('template', 'system', 'default');

DELETE FROM public.agent_skills
WHERE skill_key = 'human-written-copy'
  AND source IN ('template', 'system', 'default');
