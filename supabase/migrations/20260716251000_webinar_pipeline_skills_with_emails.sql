...
```
Deliver per environment: in a platform with native document artifacts (Vibey), register the markdown as a Doc artifact (`document_artifact`) with the title above — do not write to `/mnt/user-data/outputs/` inside the platform. In claude.ai / no native artifacts (fallback), save to `/mnt/user-data/outputs/` and present. Title the Doc `Copy Package`. Present the package first; keep per-unit files as attachments only if the platform supports them.
...
### Step 4 — Ship
Deliver per environment: in a platform with native document artifacts (Vibey), register the markdown as a Doc artifact (`document_artifact`) with the title above — do not write to `/mnt/user-data/outputs/` inside the platform. In claude.ai / no native artifacts (fallback), save to `/mnt/user-data/outputs/` and present. Title the Doc `Image Briefs`. Prompts are the deliverable; do not generate the images unless asked (and if asked, the text-on-texture ones still route to roas-ad-design).
...
WHERE s.skill_key = l.skill_key
  ...
  AND s.skill_key IN ('roas-market-research', 'roas-ad-kit', 'roas-ad-design', 'roas-webinar-audit', 'roas-webinar-copy-package', 'roas-webinar-emails', 'roas-webinar-topics', 'roas-landing-page-copy', 'roas-video-ad-scripts', 'roas-image-brief', 'roas-funnel-design', 'roas-webinar-deck', 'instagram-carousel-design');
