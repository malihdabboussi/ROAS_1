ALTER TABLE campaigns ADD COLUMN IF NOT EXISTS context JSONB DEFAULT '{}', ADD COLUMN IF NOT EXISTS resources JSONB DEFAULT '{}', ADD COLUMN IF NOT EXISTS current_priorities TEXT[] DEFAULT '{}';
...
CREATE INDEX IF NOT EXISTS idx_campaign_agents_campaign ON campaign_agents(campaign_id);
...
CREATE POLICY campaign_agents_own ON campaign_agents FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP TRIGGER IF EXISTS set_updated_at_campaign_agents ON campaign_agents;
CREATE TRIGGER set_updated_at_campaign_agents BEFORE UPDATE ON campaign_agents FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
ALTER TABLE mission_deliverables ADD COLUMN IF NOT EXISTS campaign_id UUID REFERENCES campaigns(id);
...
BEGIN
  ...
  IF NEW.status = 'todo' AND (OLD IS NULL OR OLD.status IS DISTINCT FROM 'todo') AND NEW.plan_id IS NOT NULL THEN
    UPDATE tasks SET title = COALESCE((SELECT content->>'title' FROM missions_plans WHERE id = NEW.plan_id), NEW.title), description = COALESCE((SELECT content->>'summary' FROM missions_plans WHERE id = NEW.plan_id), NEW.brief), status = 'todo', updated_at = NOW() WHERE mission_id = NEW.id;
  ELSIF NEW.status IN ('todo', 'in_progress', 'review', 'done', 'blocked') AND (OLD IS NULL OR OLD.status IS DISTINCT FROM NEW.status) THEN
    UPDATE tasks SET status = NEW.status, updated_at = NOW(), completed_at = CASE WHEN NEW.status = 'done' THEN NOW() ELSE NULL END WHERE mission_id = NEW.id;
  END IF;

  IF NEW.status IN ('failed', 'dead_letter') AND (OLD IS NULL OR OLD.status IS DISTINCT FROM NEW.status) THEN
    UPDATE tasks SET status = 'blocked', progress_notes = COALESCE(NEW.error, 'Mission failed'), updated_at = NOW() WHERE mission_id = NEW.id;
  END IF;
  ...
  IF NEW.progress_notes IS DISTINCT FROM OLD.progress_notes THEN
    UPDATE tasks SET progress_notes = NEW.progress_notes, updated_at = NOW() WHERE mission_id = NEW.id;
  END IF;
