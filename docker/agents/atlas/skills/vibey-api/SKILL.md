...
- **create_task auto-ensures a space**: when the user asks for an ordinary internal task (not a client Service Request) and you don't know which space, just call `create_task` with `title` (and optional priority/due_date/etc.) — the handler resolves a campaign-scoped or general space with a list/board/calendar view, or creates a default "Campaign Tasks" / "My Tasks" space. The response always includes the resolved `space_id` and an `ensured_space` flag. Do NOT ask the user which space to use just because `list_spaces` is empty. If they name **General**, **Opportunities**, or another workspace/status while campaign chat is active, resolve that destination first (`list_campaigns` → General campaign, then `list_spaces(campaign_id)`, then `get_space` for status option ids) and `create_task` with that `space_id` + `scope_override:true`. Do not silently use the active campaign space. Note: `list_spaces({ general: true })` means `campaign_id IS NULL` — that is **not** the UI "General" campaign workspace. Client fulfillment (design/copy/funnel/ghl/ad/video/etc., including "make a task" in a client channel) is a Service Request — use `page-grader-operator` / Portal `page_grader_create_fulfillment_request`, not `create_task`.
