...
`
    ...
    const planningMissionControl = [
      ...
      '      },',
      '      "outputContract": {',
      '        "artifact_kind": "agent_skill|document_artifact|presentation_artifact|brain_ingestion|ad_artifact|funnel_artifact|media_artifact",',
      '        "required_action": "exact vibey_backend action required to create the output",',
      '        "required_artifact_type": "agent_skill|doc|file|presentation|brain_memory|ad|funnel|image|video",',
      ...
      '- Use media_artifact for image or video outputs. For rendered media, required_action=process_media and required_artifact_type=image or video according to the final file. Never pair process_media with document_artifact or doc.',
  ...
  async callOpenClawForCommentDirective(
    ...
    const directiveMissionControl = [
      ...
      '- retry_subtask — { "type": "retry_subtask", "subtask_id": "<uuid>" } — for blocked, done, revision, pending, OR in_progress: aborts a live run if needed and re-queues execution with latest user comments in context',
      ...
      '',
      'MID-RUN STEERING (critical): If any subtask is in_progress and the user gives course-correction, constraints, focus, or "do X / use Y / avoid Z" guidance, you MUST return retry_subtask on that in_progress subtask so the live agent aborts and restarts with the new comment. Do NOT use note_only for mid-run steering.',
      'For in_progress work you want to stop and replace entirely: cancel_subtask then create_subtask is also valid.',
      'Use note_only ONLY for pure acknowledgment with no change requested (thanks, LGTM, emoji, "keep going").',
      'Respond with ONLY valid JSON (no markdown, no backticks):',
