...
export function expandIgOrganicVideoAdPlaybook(
  ...
  const subtask: MissionPlaybookPlanResult['subtasks'][number] = {
    ...
    intent: intent({
      ...
      ecology: `Load ig-organic-video-ad and follow it as the production authority. Use this kickoff exactly: ${scope}. ${copyInstruction} Produce exactly ${count} videos, one per selected scene. When reuse_when_available is selected, each clean preset video must be reused; only scenes without a preset use the direct Higgsfield MCP. Never substitute Google or Veo for Higgsfield, and never call generate_video for the final output. For every resolved clean source, call process_media with operation render_ig_story and the approved sticker payload. Only returned MP4s from process_media are final mission Deliverables. Complete Apple-style emoji, audio, full-frame visual QA, and Media registration requirements. For visual QA, call analyze_video with extract_frames true and transcribe false; do not require Deepgram unless a spoken-audio transcript was separately requested. Never route Higgsfield through Composio and never publish to Meta.`,
    }),
    outputContract: {
