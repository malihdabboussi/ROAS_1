...
function parsePaginatedJson(rawAt260: string): any[] {
  // oxlint-disable-next-line typescript/no-explicit-any
  const items: any[] = [];
  for (const line of rawAt260.split("\n")) {
    if (!line.trim()) {
      continue;
    }
...
function resolveLogin(
  name: string,
  email: string | null,
  apiByLoginAt357: Map<string, User>,
  nameToLoginAt358: Record<string, string>,
  emailToLoginAt359: Record<string, string>,
): string | null {
  if (email && emailToLoginAt359[email]) {
    return normalizeLogin(emailToLoginAt359[email]);
  }

  if (email && name) {
    const guessed = guessLoginFromEmailName(name, email, apiByLoginAt357);
    if (guessed) {
  ...
  if (email && email.endsWith("@github.com")) {
    ...
    if (apiByLoginAt357.has(login.toLowerCase())) {
      return normalizeLogin(login);
    }
  }

  const normalized = normalizeName(name);
  if (nameToLoginAt358[normalized]) {
    return normalizeLogin(nameToLoginAt358[normalized]);
  }

  const compact = normalized.replace(/\s+/g, "");
  if (nameToLoginAt358[compact]) {
    return normalizeLogin(nameToLoginAt358[compact]);
  }

  if (apiByLoginAt357.has(normalized)) {
    return normalizeLogin(normalized);
  }

  if (apiByLoginAt357.has(compact)) {
...
function guessLoginFromEmailName(
  name: string,
  email: string,
  apiByLoginAt409: Map<string, User>,
): string | null {
...
function parseReadmeEntries(
  ...
  const startAt442 = content.indexOf('<p align="left">');
  const endAt443 = content.indexOf("</p>", startAt442);
  if (startAt442 === -1 || endAt443 === -1) {
    return [];
  }
  const blockAt447 = content.slice(startAt442, endAt443);
  const entriesAt448: Array<{ display: string; html_url: string; avatar_url: string }> = [];
  const linked = /<a href="([^"]+)"><img src="([^"]+)"[^>]*alt="([^"]+)"[^>]*>/g;
  for (const match of blockAt447.matchAll(linked)) {
    const [, href, src, alt] = match;
    if (!href || !src || !alt) {
      continue;
    }
    entriesAt448.push({ html_url: href, avatar_url: src, display: alt });
  }
  const standalone = /<img src="([^"]+)"[^>]*alt="([^"]+)"[^>]*>/g;
  for (const match of blockAt447.matchAll(standalone)) {
    const [, src, alt] = match;
    if (!src || !alt) {
      continue;
    }
    if (entriesAt448.some((entry) => entry.display === alt && entry.avatar_url === src)) {
      continue;
    }
    entriesAt448.push({ html_url: fallbackHref(alt), avatar_url: src, display: alt });
  }
  return entriesAt448;
}
