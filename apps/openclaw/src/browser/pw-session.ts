...
export async function createPageViaPlaywright(opts: {
  ...
  if (targetUrl !== "about:blank") {
    assertAllowedBrowserNavigationUrl(targetUrl);
    await assertBrowserNavigationPassesSsrf(targetUrl, opts.ssrfPolicy);
    await page.goto(targetUrl, { timeout: 30_000 }).catch(() => {
      // Navigation might fail for some URLs, but page is still created
    });
    await assertBrowserCommittedNavigationPassesSsrf(targetUrl, page.url(), opts.ssrfPolicy);
  }
