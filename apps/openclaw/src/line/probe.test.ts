...
const { getBotInfoMock, MessagingApiClientMock } = vi.hoisted(() => {
  const getBotInfoMockAt3 = vi.fn();
  const MessagingApiClientMockAt4 = vi.fn(function () {
    return { getBotInfo: getBotInfoMockAt3 };
  });
  return { getBotInfoMock: getBotInfoMockAt3, MessagingApiClientMock: MessagingApiClientMockAt4 };
});
