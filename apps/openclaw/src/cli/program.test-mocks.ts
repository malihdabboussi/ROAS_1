...
export function installBaseProgramMocks() {
  ...
  vi.mock("../commands/configure.js", () => ({
    ...
    configureCommandFromSectionsArg: (sections: unknown, runtimeAt46: unknown) => {
      const resolved = Array.isArray(sections) ? sections : [];
      if (resolved.length > 0) {
        return configureCommandWithSections(resolved, runtimeAt46);
      }
      return configureCommand({}, runtimeAt46);
    },
  }));
