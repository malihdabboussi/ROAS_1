  ...
  await scanTranscriptFile({
    ...
    onEntry: (entry) => {
      ...
      if (entry.role === "assistant") {
        ...
        messageCounts.total += 1;
        const tsAt536 = entry.timestamp?.getTime();
        if (tsAt536 !== undefined) {
          const latencyMs =
            entry.durationMs ??
            (lastUserTimestamp !== undefined
              ? Math.max(0, tsAt536 - lastUserTimestamp)
              : undefined);
          if (
            latencyMs !== undefined &&
            ...
            const dayKey = formatDayKey(entry.timestamp ?? new Date(tsAt536));
