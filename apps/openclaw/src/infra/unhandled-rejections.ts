...
type UnhandledRejectionObserver = (
  reason: unknown,
  disposition: UnhandledRejectionDisposition,
) => void | Promise<void>;

const handlers = new Set<UnhandledRejectionHandler>();
const observers = new Set<UnhandledRejectionObserver>();
...
export function registerUnhandledRejectionObserver(
  observer: UnhandledRejectionObserver,
): () => void {
...
async function notifyUnhandledRejectionObservers(
  reason: unknown,
  disposition: UnhandledRejectionDisposition,
): Promise<void> {
  await Promise.allSettled(
    [...observers].map(async (observer) => {
      try {
        await observer(reason, disposition);
      } catch {
        // Observability must never affect runtime error policy.
      }
    }),
  );
}

function exitAfterObservers(
  reason: unknown,
  disposition: UnhandledRejectionDisposition,
  code: number,
): void {
  void notifyUnhandledRejectionObservers(reason, disposition).finally(() => {
    process.exit(code);
  });
}
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      void notifyUnhandledRejectionObservers(reason, "abort");
    ...
    if (isFatalError(reason)) {
      ...
      exitAfterObservers(reason, "fatal", 1);
      return;
    }

    if (isConfigError(reason)) {
      console.error("[openclaw] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
      exitAfterObservers(reason, "config", 1);
      return;
    }

    if (isTransientNetworkError(reason)) {
      void notifyUnhandledRejectionObservers(reason, "transient");
      console.warn(
    ...
    exitAfterObservers(reason, "unhandled", 1);
  });
}
