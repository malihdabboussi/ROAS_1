...
describe("gateway server agent", () => {
  ...
  test("agent dedupe survives reconnect", { timeout: 60_000 }, async () => {
    await withGatewayServer(async ({ port: portAt328 }) => {
      const dial = async () => {
        const wsAt330 = new WebSocket(`ws://127.0.0.1:${portAt328}`);
        await new Promise<void>((resolve) => wsAt330.once("open", resolve));
        await connectOk(wsAt330);
        return wsAt330;
      };
