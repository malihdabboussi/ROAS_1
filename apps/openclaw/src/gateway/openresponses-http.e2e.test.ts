...
describe("OpenResponses HTTP API (e2e)", () => {
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      expect(usageJson.usage).toMatchObject({
        input_tokens: 3,
        output_tokens: 5,
        total_tokens: 10,
  ...
  it("passes runtime lane to agent command", async () => {
    agentCommand.mockReset();
    ...
    const [optsLane] = agentCommand.mock.calls[0] ?? [];
    expect((optsLane as { lane?: string } | undefined)?.lane).toBe("chat:conversation-1");
    await ensureResponseConsumed(res);
  });
  ...
  it("streams OpenResponses SSE events", async () => {
    ...
    try {
      ...
      agentCommand.mockImplementationOnce(async (opts: unknown) => {
        const runId = (opts as { runId?: string } | undefined)?.runId ?? "";
        emitAgentEvent({ runId, stream: "assistant", data: { delta: "I'm on it." } });
        emitAgentEvent({
          runId,
          stream: "tool",
          data: {
        ...
        return { payloads: [{ text: "I'm on it. Final answer from tools." }] } as never;
      });

      const resToolBackfill = await postResponses(port, {
        stream: true,
        model: "openclaw",
        input: "read and answer",
      });
      expect(resToolBackfill.status).toBe(200);

      const toolBackfillText = await resToolBackfill.text();
      const toolBackfillEvents = parseSseEvents(toolBackfillText);
      const toolBackfillDeltas = toolBackfillEvents
        .filter((e) => e.event === "response.output_text.delta")
        .map((e) => {
          const parsed = JSON.parse(e.data) as { delta?: string };
          return parsed.delta ?? "";
        ...
        .join("");
      expect(toolBackfillDeltas).toBe("I'm on it. Final answer from tools.");

      agentCommand.mockReset();
      agentCommand.mockImplementationOnce(async (opts: unknown) => {
        const runId = (opts as { runId?: string } | undefined)?.runId ?? "";
        emitAgentEvent({ runId, stream: "assistant", data: { delta: "First streamed note." } });
        emitAgentEvent({
          runId,
          stream: "tool",
          data: {
        ...
        emitAgentEvent({ runId, stream: "assistant", data: { delta: " Final answer." } });
        return { payloads: [{ text: "Final answer.\n\nFirst streamed note." }] } as never;
      });

      const resNonPrefixFinal = await postResponses(port, {
        stream: true,
        model: "openclaw",
        input: "read and answer without duplicating",
      });
      expect(resNonPrefixFinal.status).toBe(200);

      const nonPrefixFinalText = await resNonPrefixFinal.text();
      const nonPrefixFinalEvents = parseSseEvents(nonPrefixFinalText);
      const nonPrefixFinalDeltas = nonPrefixFinalEvents
        .filter((e) => e.event === "response.output_text.delta")
        .map((e) => {
          const parsed = JSON.parse(e.data) as { delta?: string };
          return parsed.delta ?? "";
        ...
        .join("");
      expect(nonPrefixFinalDeltas).toBe("First streamed note. Final answer.");

      agentCommand.mockReset();
      agentCommand.mockImplementationOnce(async (opts: unknown) => {
        const typedOpts = opts as { runId?: string; abortSignal?: AbortSignal };
        const runId = typedOpts.runId ?? "";
        emitAgentEvent({
          runId,
          stream: "lifecycle",
          data: { phase: "error", error: "provider failed before usage" },
        });
        await new Promise((_, reject) => {
          typedOpts.abortSignal?.addEventListener("abort", () => reject(new Error("aborted")));
        });
      });

      const resLifecycleError = await postResponses(port, {
        stream: true,
        model: "openclaw",
        input: "fail before usage",
      });
      expect(resLifecycleError.status).toBe(200);
      const lifecycleErrorText = await resLifecycleError.text();
      const lifecycleErrorEvents = parseSseEvents(lifecycleErrorText);
      const failedEvent = lifecycleErrorEvents.find((e) => e.event === "response.failed");
      expect(failedEvent).toBeDefined();
      expect(failedEvent?.data).toContain('"status":"failed"');
      expect(lifecycleErrorEvents.some((e) => e.event === "response.completed")).toBe(false);
      expect(lifecycleErrorEvents.some((e) => e.data === "[DONE]")).toBe(true);
  ...
  it("supports previous_response_id continuation and validates conflicts", async () => {
    const port = enabledPort;
    agentCommand.mockReset();
    agentCommand.mockResolvedValueOnce({
      payloads: [{ text: "turn one" }],
      meta: { agentMeta: { providerResponseId: "provider-resp-1" } },
    } as never);

    const firstRes = await postResponses(port, {
      stream: false,
      model: "openclaw:beta",
      input: "first turn",
    });
    ...
    const firstResponseId = typeof firstJson.id === "string" ? firstJson.id : "";
    expect(firstResponseId).toMatch(/^resp_/);
    const [firstOpts] = agentCommand.mock.calls[0] ?? [];
    const firstSessionKey = String(
      (firstOpts as { sessionKey?: string } | undefined)?.sessionKey ?? "",
    );
    expect(firstSessionKey).toMatch(/^agent:beta:/);

    agentCommand.mockResolvedValueOnce({
      payloads: [{ text: "turn two" }],
      meta: { agentMeta: { providerResponseId: "provider-resp-2" } },
    } as never);
    const secondRes = await postResponses(port, {
      stream: false,
      model: "openclaw:beta",
      input: "second turn",
    ...
    const [secondOpts] = agentCommand.mock.calls[1] ?? [];
    const secondSessionKey = String(
      (secondOpts as { sessionKey?: string } | undefined)?.sessionKey ?? "",
    );
    expect(secondSessionKey).toBe(firstSessionKey);
    await ensureResponseConsumed(secondRes);

    agentCommand.mockResolvedValueOnce({
      payloads: [{ text: "turn three" }],
      meta: { agentMeta: { providerResponseId: "provider-resp-3" } },
    } as never);
    const providerLinkedRes = await postResponses(port, {
      stream: false,
      model: "openclaw:beta",
      input: "third turn",
      previous_response_id: "provider-resp-2",
    });
    expect(providerLinkedRes.status).toBe(200);
    const [thirdOpts] = agentCommand.mock.calls[2] ?? [];
    const thirdSessionKey = String(
      (thirdOpts as { sessionKey?: string } | undefined)?.sessionKey ?? "",
    );
    expect(thirdSessionKey).toBe(firstSessionKey);
    await ensureResponseConsumed(providerLinkedRes);

    const callsBeforeInvalid = agentCommand.mock.calls.length;

    const unknownRes = await postResponses(port, {
      stream: false,
      model: "openclaw:beta",
      input: "bad previous",
      previous_response_id: "missing-response-id",
    });
    expect(unknownRes.status).toBe(400);
    expect(agentCommand.mock.calls.length).toBe(callsBeforeInvalid);
    await ensureResponseConsumed(unknownRes);

    const mismatchAgentRes = await postResponses(port, {
      stream: false,
      model: "openclaw:main",
      input: "wrong agent",
      previous_response_id: firstResponseId,
    });
    expect(mismatchAgentRes.status).toBe(400);
    expect(agentCommand.mock.calls.length).toBe(callsBeforeInvalid);
    await ensureResponseConsumed(mismatchAgentRes);

    const mismatchHeaderRes = await postResponses(
      port,
      {
        stream: false,
        model: "openclaw:beta",
        input: "wrong session header",
        previous_response_id: firstResponseId,
      },
      { "x-openclaw-session-key": "agent:beta:forced-mismatch" },
    );
    expect(mismatchHeaderRes.status).toBe(400);
    expect(agentCommand.mock.calls.length).toBe(callsBeforeInvalid);
    await ensureResponseConsumed(mismatchHeaderRes);
  });

  it("forwards provider generation cost metadata on non-stream responses", async () => {
    const port = enabledPort;
    agentCommand.mockReset();
    agentCommand.mockResolvedValueOnce({
      payloads: [{ text: "hello" }],
      meta: {
        agentMeta: {
          providerResponseId: "gen-test-2",
          providerCost: 3.75,
          providerGenerationIds: ["gen-test-1", "gen-test-2"],
          providerGenerations: [
            {
              generationId: "gen-test-1",
              providerResponseId: "gen-test-1",
            ...
            },
            {
              ...
              model: "anthropic/claude-opus-4.6",
              providerCost: 2.5,
              usage: { input: 30, output: 40, total: 70 },
            },
          ],
        },
      },
    ...
    expect(json.metadata).toMatchObject({
      provider_response_id: "gen-test-2",
      provider_cost: 3.75,
      provider_generation_ids: ["gen-test-1", "gen-test-2"],
      provider_billing: "openrouter",
    });
    expect(json.metadata?.provider_generations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ generationId: "gen-test-1", providerCost: 1.25 }),
        expect.objectContaining({ generationId: "gen-test-2", providerCost: 2.5 }),
      ]),
    );
  });
