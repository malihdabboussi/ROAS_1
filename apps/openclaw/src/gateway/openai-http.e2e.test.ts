    ...
    try {
...

      {
        ...
        agentCommand.mockImplementationOnce(async (opts: unknown) => {
          ...
          return { payloads: [{ text: "I'm on it. Final answer." }] } as never;
        });

        const backfillRes = await postChatCompletions(port, {
          stream: true,
          model: "openclaw",
          messages: [{ role: "user", content: "hi" }],
        });
        ...
        const backfillData = parseSseDataLines(backfillText);
        const backfillChunks = backfillData
          .filter((d) => d !== "[DONE]")
          .map((d) => JSON.parse(d) as Record<string, unknown>);
        const backfillContent = backfillChunks
          .flatMap((c) => (c.choices as Array<Record<string, unknown>> | undefined) ?? [])
          .map((choice) => (choice.delta as Record<string, unknown> | undefined)?.content)
          .filter((v): v is string => typeof v === "string")
          .join("");
        expect(backfillContent).toBe("I'm on it. Final answer.");
      }
