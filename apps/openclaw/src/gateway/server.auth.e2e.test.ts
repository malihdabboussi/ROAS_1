...
describe("gateway server auth/connect", () => {
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { buildDeviceAuthPayload: buildDeviceAuthPayloadAt810 } =
      await import("./device-auth.js");
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../infra/device-identity.js");
    const { approveDevicePairing, getPairedDevice, listDevicePairing } =
      await import("../infra/device-pairing.js");
    const {
      GATEWAY_CLIENT_MODES: GATEWAY_CLIENT_MODESAt815,
      GATEWAY_CLIENT_NAMES: GATEWAY_CLIENT_NAMESAt815,
    } = await import("../utils/message-channel.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "openclaw-device-scope-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
    const client = {
      id: GATEWAY_CLIENT_NAMESAt815.TEST,
      version: "1.0.0",
      platform: "test",
      mode: GATEWAY_CLIENT_MODESAt815.TEST,
    };
    const buildDevice = (scopes: string[]) => {
      const signedAtMs = Date.now();
      const payload = buildDeviceAuthPayloadAt810({
        deviceId: identity.deviceId,
        clientId: client.id,
