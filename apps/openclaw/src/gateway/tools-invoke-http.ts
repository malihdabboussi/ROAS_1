...
import { classifyToolFailure } from "../agents/platform-failure.js";
import {
...
import { ToolInputError, jsonResult } from "../agents/tools/common.js";
import {
  preflightWorkflowToolCall,
  recordWorkflowToolResult,
} from "../agents/workflow-circuit-breaker.js";
...
function buildToolInvokeError(type: string, message: string): Record<string, unknown> {
  return {
    type,
    message,
    ...classifyToolFailure({ errorText: message }),
  };
}
...
export async function handleToolsInvokeHttpRequest(
  ...
  const workflowCircuit = { agentId, sessionKey };
  ...
  if (!tool) {
    ...
    if (!circuit.allowed) {
      sendJson(res, 409, {
        ok: false,
        error: {
          type: "workflow_circuit_open",
          message: circuit.contract.user_explanation.sentence,
          ...circuit.contract,
        },
    ...
    const message = `Tool not available: ${toolName}`;
    const error = buildToolInvokeError("not_found", message);
    recordWorkflowToolResult(
      {
        ...workflowCircuit,
        toolName,
  ...
  let toolArgs = args;
  const recordErrorResult = (error: Record<string, unknown>) => {
    recordWorkflowToolResult(
      {
        ...workflowCircuit,
        toolName,
  ...
  try {
    ...
    if (isToolInputError(err)) {
      const message = getErrorMessage(err) || "invalid tool arguments";
      const error = buildToolInvokeError("tool_error", message);
    ...
    const error = buildToolInvokeError("tool_error", "tool execution failed");
    recordErrorResult(error);
    sendJson(res, 500, {
      ok: false,
      error,
