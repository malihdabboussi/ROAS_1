...
const { resetWorkflowCircuitsForTests } = await import("../agents/workflow-circuit-breaker.js");
...
const expectToolInvokeErrorContract = (
  error: Record<string, unknown>,
  expected: { type: string; errorCode: string; errorClass: string },
) => {
  ...
  const userExplanation = error.user_explanation as { sentence?: unknown };
  expect(String(userExplanation.sentence)).not.toMatch(/platform|internal/i);
};
...
describe("POST /tools/invoke", () => {
  ...
  it("denies sessions_spawn via HTTP even when agent policy allows", async () => {
    ...
    expectToolInvokeErrorContract(body.error, {
      type: "not_found",
  ...
  it("maps tool input errors to 400 and unexpected execution errors to 500", async () => {
    ...
    expectToolInvokeErrorContract(inputBody.error, {
      type: "tool_error",
      errorCode: "TOOL_VALIDATION_ERROR",
      errorClass: "validation_error",
    });
    ...
    expectToolInvokeErrorContract(crashBody.error, {
      type: "tool_error",
      errorCode: "TOOL_SYSTEM_FAULT",
      errorClass: "system_fault",
    });
  });

  it("opens the workflow circuit for repeated non-retryable HTTP tool failures", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [{ id: "main", default: true, tools: { allow: ["tools_invoke_test"] } }],
    ...
    expect(body.error).toMatchObject({
      ...
      workflow_circuit: {
        state: "open",
        reason: "non_retryable_equivalent_repeat",
      },
    });
    expect(body.error.user_explanation.sentence).not.toMatch(/platform|backend|internal/i);
  });
});
