...
export const agentHandlers: GatewayRequestHandlers = {
  agent: async ({ params, respond, context, client, isWebchatConnect }) => {
    ...
    if (requestedSessionKey) {
      ...
      const agentIdAt428 = resolveAgentIdFromSessionKey(canonicalSessionKey);
      const mainSessionKey = resolveAgentMainSessionKey({ cfg: cfgAt354, agentId: agentIdAt428 });
      if (storePath) {
