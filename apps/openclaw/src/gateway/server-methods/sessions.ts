...
export const sessionsHandlers: GatewayRequestHandlers = {
  ...
  "sessions.reset": async ({ params, respond }) => {
    ...
    const next = await updateSessionStore(storePath, (store) => {
      const { primaryKey } = migrateAndPruneSessionStoreKey({ cfg, key, store });
      const entryAt309 = store[primaryKey];
      oldSessionId = entryAt309?.sessionId;
      oldSessionFile = entryAt309?.sessionFile;
      const now = Date.now();
      const nextEntry: SessionEntry = {
        ...
        thinkingLevel: entryAt309?.thinkingLevel,
        verboseLevel: entryAt309?.verboseLevel,
        reasoningLevel: entryAt309?.reasoningLevel,
        responseUsage: entryAt309?.responseUsage,
        model: entryAt309?.model,
        contextTokens: entryAt309?.contextTokens,
        sendPolicy: entryAt309?.sendPolicy,
        label: entryAt309?.label,
        origin: snapshotSessionOrigin(entryAt309),
        lastChannel: entryAt309?.lastChannel,
        lastTo: entryAt309?.lastTo,
        skillsSnapshot: entryAt309?.skillsSnapshot,
