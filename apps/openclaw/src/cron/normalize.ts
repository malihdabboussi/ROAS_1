...
export function normalizeCronJobInput(
  ...
  if (options.applyDefaults) {
    ...
    const payloadAt409 = isRecord(next.payload) ? next.payload : null;
    const payloadKind =
      payloadAt409 && typeof payloadAt409.kind === "string" ? payloadAt409.kind : "";
    const sessionTarget = typeof next.sessionTarget === "string" ? next.sessionTarget : "";
    const isIsolatedAgentTurn =
      sessionTarget === "isolated" || (sessionTarget === "" && payloadKind === "agentTurn");
    const hasDelivery = "delivery" in next && next.delivery !== undefined;
    const hasLegacyDelivery = payloadAt409 ? hasLegacyDeliveryHints(payloadAt409) : false;
    if (!hasDelivery && isIsolatedAgentTurn && payloadKind === "agentTurn") {
      if (payloadAt409 && hasLegacyDelivery) {
        next.delivery = buildDeliveryFromLegacyPayload(payloadAt409);
        stripLegacyDeliveryFields(payloadAt409);
      } else {
