...
export async function modelsScanCommand(
  ...
  const _updated = await updateConfig((cfgAt300) => {
    const nextModels = { ...cfgAt300.agents?.defaults?.models };
    ...
    const existingImageModel = cfgAt300.agents?.defaults?.imageModel as
    ...
    const nextImageModel =
      selectedImages.length > 0
        ...
        : cfgAt300.agents?.defaults?.imageModel;
    const existingModel = cfgAt300.agents?.defaults?.model as
      | { primary?: string; fallbacks?: string[] }
      | undefined;
    const defaults = {
      ...cfgAt300.agents?.defaults,
      model: {
      ...
      models: nextModels,
    } satisfies NonNullable<NonNullable<typeof cfgAt300.agents>["defaults"]>;
    return {
      ...cfgAt300,
      agents: {
        ...cfgAt300.agents,
