...
import { downloadSessionStore } from "../config/sessions/storage-sync.js";
import {
...
function runAgentAttempt(params: {
  ...
  return runEmbeddedPiAgent({
    ...
    enabledToolkits: params.opts.enabledToolkits,
    disabledNativeActions: params.opts.disabledNativeActions,
    provider: params.providerOverride,
    model: params.modelOverride,
    runtimeCredentials: params.opts.runtimeCredentials,
    authProfileId,
    ...
    reasoningLevel: params.sessionEntry?.reasoningLevel as ReasoningLevel | undefined,
    onReasoningStream: params.sessionEntry?.reasoningLevel === "stream" ? () => {} : undefined,
    streamParams: params.opts.streamParams,
    contextTokensOverride: params.opts.contextTokensOverride,
...
export async function agentCommand(
  ...
  await downloadSessionStore({
    storePath: cfg.session?.store
      ? resolveStorePath(cfg.session.store, { agentId: sessionAgentId })
      : resolveDefaultSessionStorePath(sessionAgentId),
  ...
  try {
    ...
    const hasExternalSkillCatalog = (opts.skillCatalog?.entries?.length ?? 0) > 0;
    const needsSkillsSnapshot =
      hasExternalSkillCatalog || isNewSession || !sessionEntry?.skillsSnapshot;
    ...
    const skillsSnapshot = needsSkillsSnapshot
      ? buildWorkspaceSkillSnapshot(workspaceDir, {
          ...
          skillFilter,
          externalSkillCatalog: opts.skillCatalog,
        })
    ...
    if (opts.reasoningLevel) {
      ...
      entry.reasoningLevel = opts.reasoningLevel;
      sessionEntry = entry;
    }

    // Per-request model override from Responses API (highest priority)
    if (opts.modelOverride) {
      const slash = opts.modelOverride.indexOf("/");
      if (slash !== -1) {
        const overrideRef = normalizeModelRef(
          opts.modelOverride.slice(0, slash),
          opts.modelOverride.slice(slash + 1),
        );
        provider = overrideRef.provider;
        model = overrideRef.model;
      }
    }
    ...
    try {
      ...
      const fallbacksOverride = opts.disableModelFallbacks ? [] : effectiveFallbacksOverride;
    ...
    if (sessionStore && sessionKey) {
      await updateSessionStoreAfterAgentRun({
        cfg,
        contextTokensOverride: opts.contextTokensOverride ?? agentCfg?.contextTokens,
        sessionId,
        sessionKey,
        storePath,
        sessionStore,
