...
export async function gatewayStatusCommand(
  ...
  const { discovery, probed } = await withProgress(
    ...
    async () => {
      ...
      const [discoveryAt106, tunnelFirst] = await Promise.all([discoveryTask, tunnelTask]);

      if (!sshTarget && opts.sshAuto) {
        const user = process.env.USER?.trim() || "";
        const candidates = discoveryAt106
          .map((b) => {
      ...
      try {
        const probedAt153 = await Promise.all(
        ...
        return { discovery: discoveryAt106, probed: probedAt153 };
      } finally {
        if (tunnel) {
          try {
            await tunnel.stop();
