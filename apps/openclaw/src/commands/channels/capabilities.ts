  ...
  for (const report of reports) {
    ...
    if (report.channel === "slack" && report.slackScopes) {
      for (const entry of report.slackScopes) {
        ...
        const labelAt527 = entry.tokenType === "user" ? "User scopes" : "Bot scopes";
        if (entry.result.ok && entry.result.scopes?.length) {
          lines.push(`${labelAt527}${source}: ${entry.result.scopes.join(", ")}`);
        } else if (entry.result.error) {
          lines.push(`${labelAt527}: ${theme.error(entry.result.error)}`);
        }
      }
    }
    if (report.channel === "discord" && report.channelPermissions) {
      ...
      if (perms.error) {
        ...
        const labelAt541 = perms.channelId ? ` (${perms.channelId})` : "";
        lines.push(`Permissions${labelAt541}: ${list}`);
