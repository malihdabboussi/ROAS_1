...
export async function channelsAddCommand(
  ...
  if (useWizard) {
    ...
    let nextConfigAt68 = await setupChannels(cfg, runtime, prompter, {
      allowDisable: false,
    ...
    if (wantsNames) {
      for (const channel of selection) {
        ...
        const account = plugin?.config.resolveAccount(nextConfigAt68, accountId) as
          | { name?: string }
          | undefined;
        const snapshot = plugin?.config.describeAccount?.(account, nextConfigAt68);
        ...
        if (name?.trim()) {
          nextConfigAt68 = applyAccountName({
            cfg: nextConfigAt68,
            channel,
            accountId,
            name,
    ...
    await writeConfigFile(nextConfigAt68);
