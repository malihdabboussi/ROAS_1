...
const ONBOARDING_BOTTOM_CHANNELS = new Map<ChannelChoice, number>([
  ["telegram", 0],
  ["slack", 1],
]);

function sortChannelsForOnboarding<T extends { id: ChannelChoice }>(entries: T[]): T[] {
  return entries.toSorted((a, b) => {
    const rankA = ONBOARDING_BOTTOM_CHANNELS.get(a.id);
    const rankB = ONBOARDING_BOTTOM_CHANNELS.get(b.id);
...
function getOnboardingDefaultPriority(channel: ChannelChoice): number {
  return ONBOARDING_BOTTOM_CHANNELS.has(channel) ? 0 : 1;
}
...
function resolveQuickstartDefault(
  ...
  let best: { channel: ChannelChoice; priority: number; score: number } | null = null;
  for (const [channel, status] of statusByChannel) {
    if (status.quickstartScore == null) {
      continue;
    }
    const priority = getOnboardingDefaultPriority(channel);
    if (
      !best ||
      priority > best.priority ||
      (priority === best.priority && status.quickstartScore > best.score)
    ) {
      best = { channel, priority, score: status.quickstartScore };
    }
  }
  return best?.channel;
}
...
export async function setupChannels(
  ...
  const primerChannels = sortChannelsForOnboarding([
    ...corePrimer,
    ...installedPlugins
  ...
  const getChannelEntries = () => {
    ...
    const entries = sortChannelsForOnboarding(
