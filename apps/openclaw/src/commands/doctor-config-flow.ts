...
function normalizeIssuePath(pathAt29: PropertyKey[]): Array<string | number> {
  return pathAt29.filter((part): part is string | number => typeof part !== "symbol");
}
...
function resolvePathTarget(root: unknown, pathAt52: Array<string | number>): unknown {
  let current: unknown = root;
  for (const part of pathAt52) {
    if (typeof part === "number") {
...
function stripUnknownConfigKeys(config: OpenClawConfig): {
  ...
  for (const issue of parsed.error.issues) {
    ...
    const pathAt92 = normalizeIssuePath(issue.path);
    const target = resolvePathTarget(next, pathAt92);
    ...
    for (const key of issue.keys) {
      ...
      delete record[key];
      removed.push(formatPath([...pathAt92, key]));
    }
  }
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (unknown.removed.length > 0) {
    const lines = unknown.removed.map((pathAt558) => `- ${pathAt558}`).join("\n");
