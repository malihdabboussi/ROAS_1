...
describe("setupChannels", () => {
    ...
    const select = vi.fn(
      async ({
        ...
        if (message === "Select channel (QuickStart)") {
          ...
          expect(values.slice(-3)).toEqual(["telegram", "slack", "__skip__"]);
          expect(initialValue).toBe("whatsapp");
        }
        return "whatsapp";
      },
    );
  ...
  it("moves Telegram and Slack to the bottom of manual channel selection", async () => {
    const select = vi.fn(async ({ message, options }: { message: string; options: unknown[] }) => {
      if (message === "Select a channel") {
        const values = (options as Array<{ value: string }>).map((option) => option.value);
        expect(values.slice(-3)).toEqual(["telegram", "slack", "__done__"]);
      }
      return "__done__";
    });
    const multiselect = vi.fn(async () => {
