...
describe("logger helpers", () => {
  ...
  it("only logs debug when verbose is enabled", () => {
    const logVerboseAt39 = vi.spyOn(console, "log");
    setVerbose(false);
    logDebug("quiet");
    expect(logVerboseAt39).not.toHaveBeenCalled();

    setVerbose(true);
    logVerboseAt39.mockClear();
    logDebug("loud");
    expect(logVerboseAt39).toHaveBeenCalled();
    logVerboseAt39.mockRestore();
  });
