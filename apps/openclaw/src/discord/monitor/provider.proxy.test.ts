...
const {
  ...
  const proxyAgentSpyAt12 = vi.fn();
  const webSocketSpyAt13 = vi.fn();

  const GatewayIntentsAt15 = {
    Guilds: 1 << 0,
    GuildMessages: 1 << 1,
    MessageContent: 1 << 2,
    DirectMessages: 1 << 3,
  ...
  class GatewayPluginAt26 {}

  class HttpsProxyAgentAt28 {
    static lastCreated: HttpsProxyAgentAt28 | undefined;
    proxyUrl: string;
    constructor(proxyUrl: string) {
      ...
      HttpsProxyAgentAt28.lastCreated = this;
      proxyAgentSpyAt12(proxyUrl);
    }
  }

  return {
    GatewayIntents: GatewayIntentsAt15,
    GatewayPlugin: GatewayPluginAt26,
    HttpsProxyAgent: HttpsProxyAgentAt28,
    getLastAgent: () => HttpsProxyAgentAt28.lastCreated,
    proxyAgentSpy: proxyAgentSpyAt12,
    resetLastAgent: () => {
      HttpsProxyAgentAt28.lastCreated = undefined;
    },
    webSocketSpy: webSocketSpyAt13,
  };
});
