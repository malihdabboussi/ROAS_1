...
import { buildSlackMessagePayloads, type SlackBlock } from "./table-blocks.js";
...
async function postSlackMessageBestEffort(params: {
  ...
  const postMessage = (payload: typeof basePayload & Record<string, unknown>) =>
    params.client.chat.postMessage(
      payload as Parameters<WebClient["chat"]["postMessage"]>[0],
    );
  try {
    ...
    return postMessage(basePayload);
  }
}
...
export async function sendMessageSlack(
  ...
  if (opts.mediaUrl) {
    ...
    const payloads = markdownChunks.flatMap((markdown) =>
      buildSlackMessagePayloads(markdown, { chunkLimit, tableMode }),
    );
    if (!payloads.length && trimmedMessage) {
      payloads.push({ text: trimmedMessage });
    }
    for (const payload of payloads.length ? payloads : [{ text: "" }]) {
      const response = await postSlackMessageBestEffort({
        client,
        channelId,
        text: payload.text,
