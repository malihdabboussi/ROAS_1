...
import { editSlackMessage, readSlackMessages } from "./actions.js";
...
describe("editSlackMessage", () => {
  it("formats Markdown content as Slack mrkdwn before updating", async () => {
    const client = createClient();

    await editSlackMessage(
      "C1",
      "171234.567",
      "**Important:** see [docs](https://example.com) & ship",
    ...
    expect(client.chat.update).toHaveBeenCalledWith({
      channel: "C1",
      ts: "171234.567",
      text: "*Important:* see <https://example.com|docs> &amp; ship",
    });
  });
});
