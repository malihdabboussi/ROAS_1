...
export async function registerSlackMonitorSlashCommands(params: {
  ...
  const registerArgAction = (actionId: string) => {
    (
      ...
      const regAt602 = await getCommandsRegistry();
      const commandDefinition = regAt602.findCommandByNativeName(parsed.command, "slack");
      const commandArgs: CommandArgs = {
        values: { [parsed.arg]: parsed.value },
      };
      const prompt = commandDefinition
        ? regAt602.buildCommandTextFromArgs(commandDefinition, commandArgs)
