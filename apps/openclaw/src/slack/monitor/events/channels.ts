...
export function registerSlackChannelEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const enqueueChannelSystemEvent = (paramsAt18: {
    ...
    if (
      !ctx.isChannelAllowed({
        channelId: paramsAt18.channelId,
        channelName: paramsAt18.channelName,
        channelType: "channel",
      })
    ) {
      return;
    ...
    enqueueSystemEvent(`Slack channel ${paramsAt18.kind}: ${label}.`, {
      sessionKey,
      contextKey: `slack:channel:${paramsAt18.kind}:${paramsAt18.channelId ?? paramsAt18.channelName ?? "unknown"}`,
    });
  };
