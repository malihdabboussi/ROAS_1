...
export function registerSlackMemberEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const handleMemberChannelEvent = async (paramsAt11: {
    verb: "joined" | "left";
    event: SlackMemberChannelEvent;
    body: unknown;
  }) => {
    try {
      if (ctx.shouldDropMismatchedSlackEvent(paramsAt11.body)) {
        return;
      }
      const payload = paramsAt11.event;
      ...
      enqueueSystemEvent(`Slack: ${userLabel} ${paramsAt11.verb} ${label}.`, {
        sessionKey,
        contextKey: `slack:member:${paramsAt11.verb}:${channelId ?? "unknown"}:${payload.user ?? "unknown"}`,
      });
    } catch (err) {
      ctx.runtime.error?.(danger(`slack ${paramsAt11.verb} handler failed: ${String(err)}`));
    }
  };
