...
describe("URL image provider compression", () => {
  it("compresses fetched URL images before provider base64 handoff", async () => {
    const release = vi.fn(async () => {});
    const original = Buffer.alloc(8 * 1024 * 1024);
    const compressed = Buffer.alloc(1024);
    fetchWithSsrFGuardMock.mockResolvedValueOnce({
    ...
    resizeToJpegMock.mockResolvedValueOnce(compressed);

    const { extractImageContentFromSource } = await import("./input-files.js");
    const result = await extractImageContentFromSource(
      { type: "url", url: "https://example.com/large.jpg" },
      {
        allowUrl: true,
    ...
    expect(Buffer.from(result.data, "base64").byteLength).toBe(compressed.byteLength);
    expect(release).toHaveBeenCalledTimes(1);
  });
});
