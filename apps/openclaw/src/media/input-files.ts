...
export const DEFAULT_INPUT_IMAGE_MAX_BYTES = 15 * 1024 * 1024;
...
const PROVIDER_INPUT_IMAGE_MAX_BASE64_BYTES = 10 * 1024 * 1024;
const PROVIDER_INPUT_IMAGE_TARGET_BYTES = Math.floor(PROVIDER_INPUT_IMAGE_MAX_BASE64_BYTES * 0.7);
const PROVIDER_INPUT_IMAGE_MAX_SIDE = 2000;

function encodedBase64Bytes(buffer: Buffer): number {
  return Math.ceil(buffer.byteLength / 3) * 4;
}
...
async function compressImageForProvider(params: {
  buffer: Buffer;
  mimeType: string;
}): Promise<{ buffer: Buffer; mimeType: string; resized: boolean; base64Bytes: number }> {
  const originalBase64Bytes = encodedBase64Bytes(params.buffer);
  if (originalBase64Bytes <= PROVIDER_INPUT_IMAGE_MAX_BASE64_BYTES) {
    return {
      buffer: params.buffer,
      mimeType: params.mimeType,
      resized: false,
      base64Bytes: originalBase64Bytes,
    };
  }

  const qualities = [85, 75, 65, 55, 45, 35, 25];
  const sides = [PROVIDER_INPUT_IMAGE_MAX_SIDE, 1800, 1600, 1400, 1200, 1000, 800];
  let smallest: { buffer: Buffer; base64Bytes: number } | null = null;
  ...
  for (const side of sides) {
    for (const quality of qualities) {
      ...
      const outBase64Bytes = encodedBase64Bytes(out);
      if (!smallest || outBase64Bytes < smallest.base64Bytes) {
        smallest = { buffer: out, base64Bytes: outBase64Bytes };
      }
      if (out.byteLength <= PROVIDER_INPUT_IMAGE_TARGET_BYTES) {
        return {
          buffer: out,
          mimeType: "image/jpeg",
          resized: true,
          base64Bytes: outBase64Bytes,
        };
      }
    }
  }

  if (smallest && smallest.base64Bytes <= PROVIDER_INPUT_IMAGE_MAX_BASE64_BYTES) {
    return {
      buffer: smallest.buffer,
      mimeType: "image/jpeg",
      resized: true,
      base64Bytes: smallest.base64Bytes,
    };
  }

  const gotMb = ((smallest?.base64Bytes ?? originalBase64Bytes) / (1024 * 1024)).toFixed(2);
  throw new Error(`Image could not be reduced below 10MB provider limit (got ${gotMb}MB)`);
}
...
export async function extractImageContentFromSource(
  ...
  if (source.type === "url" && source.url) {
    ...
    const providerImage = await compressImageForProvider({
      buffer: result.buffer,
      mimeType: result.mimeType,
    });
    if (!limits.allowedMimes.has(providerImage.mimeType)) {
      throw new Error(`Unsupported image MIME type from URL: ${providerImage.mimeType}`);
    }
    return {
      type: "image",
      data: providerImage.buffer.toString("base64"),
