...
export async function textToSpeech(params: {
  ...
  for (const providerAt556 of providers) {
    const providerStart = Date.now();
    try {
      if (providerAt556 === "edge") {
        ...
        return {
          ...
          provider: providerAt556,
          outputFormat: edgeResult.outputFormat,
          voiceCompatible,
        };
      }

      const apiKey = resolveTtsApiKey(config, providerAt556);
      if (!apiKey) {
        lastError = `No API key for ${providerAt556}`;
        continue;
      }

      let audioBuffer: Buffer;
      if (providerAt556 === "elevenlabs") {
      ...
      return {
        ...
        latencyMs,
        provider: providerAt556,
        outputFormat: providerAt556 === "openai" ? output.openai : output.elevenlabs,
        voiceCompatible: output.voiceCompatible,
      };
    } catch (err) {
      lastError = formatTtsProviderError(providerAt556, err);
    }
  }
