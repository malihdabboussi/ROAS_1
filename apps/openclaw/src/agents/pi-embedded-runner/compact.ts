...
export type CompactEmbeddedPiSessionParams = {
  ...
  /** Per-request context window cap for compaction budgeting. */
...
export async function compactEmbeddedPiSessionDirect(
  ...
  if (log.isEnabled("debug")) {
    log.debug(
      `[compaction-diag] context runId=${runId} sessionKey=${params.sessionKey ?? params.sessionId} ` +
        `diagId=${diagId} trigger=${trigger} provider=${provider}/${modelId} ` +
        `contextWindow=${contextWindowInfo.tokens} contextSource=${contextWindowInfo.source} ` +
        `requestContextTokens=${params.contextTokensOverride ?? "none"}`,
    );
  }
  try {
  ...
  try {
    ...
    try {
      ...
      void uploadSessionTranscript({
        ...
        log.warn(`storage-sync upload failed after compaction: ${String(err)}`);
      });
    }
  } catch (err) {
    const reason = describeUnknownError(err);
