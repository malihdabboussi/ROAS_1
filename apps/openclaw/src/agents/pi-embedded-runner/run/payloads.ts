...
export function buildEmbeddedRunPayloads(params: {
  ...
  if (params.lastToolError) {
    ...
    if (shouldShowToolError) {
      ...
      const failure =
        params.lastToolError.failure ??
        classifyToolFailure({ errorText: params.lastToolError.error });
      const safeMessage = params.lastToolError.userSafeMessage ?? toUserSafeFailureMessage(failure);
      const warningText = failure.error_code === "WORKFLOW_CIRCUIT_OPEN"
        ? `⚠️ ${safeMessage}`
        : `⚠️ ${toolSummary} failed. ${safeMessage}`;
