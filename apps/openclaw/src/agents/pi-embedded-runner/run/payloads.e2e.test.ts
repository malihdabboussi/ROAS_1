...
describe("buildEmbeddedRunPayloads", () => {
  ...
  it("adds a fallback error when a tool fails and no assistant output exists", () => {
    ...
    expect(payloads[0]?.text).toContain("I cannot run that step right now");
  });
  ...
  it("adds tool error fallback when the assistant only invoked tools", () => {
    ...
    expect(payloads[0]?.text).toContain("try a different approach");
    expect(payloads[0]?.text).not.toContain("internal issue");
  });
  ...
  it("still shows mutating tool errors when messages.suppressToolErrors is enabled", () => {
    ...
    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.isError).toBe(true);
    expect(payloads[0]?.text).toContain("temporary issue");
  });
  ...
  it("shows recoverable tool errors for mutating tools", () => {
    ...
    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.isError).toBe(true);
    expect(payloads[0]?.text).toContain("need one more detail");
  });
  ...
  it("shows mutating tool errors even when assistant output exists", () => {
    ...
    expect(payloads[1]?.text).toContain("need one more detail");
  });
  ...
  it("surfaces workflow circuit open messages without a tool-failed prefix", () => {
    const payloads = buildPayloads({
      lastToolError: {
        toolName: "campaign_capability",
        error: "Workflow circuit open for presentation_render",
        mutatingAction: true,
        failure: {
          class: "SYSTEM_FAULT",
          retryable: false,
          ...
          retry_policy: {
            ...
            reason: "Repeated failures already occurred in this workflow class.",
          },
          correction: {
            summary: "Stop tool calls for this workflow class.",
          },
          fallback: null,
          agent_diagnosis: "The workflow circuit breaker blocked another attempt.",
          agent_instruction: "Stop using tools for this workflow class.",
          user_explanation: {
            intent: "offer_verified_recovery_options",
            sentence:
              "This export path is not working from chat right now. I can still help with verified alternatives.",
          },
          forbidden_user_framing: ["platform error", "internal issue"],
          observability: {
            fingerprint: "openclaw.workflow_circuit.presentation_render.test",
            report_level: "warn",
          },
        },
      },
    ...
    expect(payloads[0]?.isError).toBe(true);
    expect(payloads[0]?.text).toBe(
      "⚠️ This export path is not working from chat right now. I can still help with verified alternatives.",
    );
    expect(payloads[0]?.text).not.toContain("failed");
  });
