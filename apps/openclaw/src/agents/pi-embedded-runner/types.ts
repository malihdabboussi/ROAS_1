...
export type EmbeddedPiAgentMeta = {
  ...
  /** Effective context window size (tokens) resolved for the model used in this run. */
  contextWindowTokens?: number;
  /** Real LLM provider response ID (e.g. OpenRouter gen_xxx). */
  providerResponseId?: string;
  /** Real LLM provider generation IDs included in this run. */
  providerGenerationIds?: string[];
  /** Per-generation provider cost ledger included in this run. */
  providerGenerations?: ProviderGenerationSummary[];
  /** Actual USD cost reported by the LLM provider (e.g. OpenRouter usage.cost). */
  providerCost?: number;
};
