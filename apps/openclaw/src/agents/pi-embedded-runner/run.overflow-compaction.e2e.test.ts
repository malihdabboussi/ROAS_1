...
describe("overflow compaction in run loop", () => {
  ...
  it("emits structured recovery events while retrying overflow compaction", async () => {
    const overflowError = new Error("request_too_large: Request size exceeds model context window");
    ...
    const phases = onAgentEvent.mock.calls.map((call) => call[0]?.data?.phase);
    expect(phases).toEqual(["overflow_detected", "start", "end", "retrying", "recovered"]);
