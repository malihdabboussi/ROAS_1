...
function resolveCompactionMode(cfg?: OpenClawConfig): "off" | "default" | "safeguard" {
  const mode = cfg?.agents?.defaults?.compaction?.mode;
  if (mode === "off" || mode === "safeguard") {
    return mode;
  }
  return "default";
}
...
export function buildEmbeddedExtensionSetup(params: {
  cfg: OpenClawConfig | undefined;
  sessionManager: SessionManager;
  provider: string;
  modelId: string;
  ...
  const pruning = buildContextPruningExtension(params);
  if (pruning.additionalExtensionPaths) {
    extensionPaths.push(...pruning.additionalExtensionPaths);
  }
