...
import {
  ...
  resolveProviderCostFromAssistant,
  resolveProviderResponseIdFromAssistant,
  sumProviderGenerationCosts,
  type ProviderGenerationSummary,
...
function emitContextOverflowRecoveryEvent(
  params: Pick<RunEmbeddedPiAgentParams, "runId" | "sessionId" | "sessionKey" | "onAgentEvent">,
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const ctxInfo = resolveContextWindowInfo({
        ...
        requestContextTokens: params.contextTokensOverride,
      });
      ...
      const resolveAuthProfileFailoverReason = (foParams: {
        allInCooldown: boolean;
        message: string;
      }): FailoverReason => {
        if (foParams.allInCooldown) {
          return "rate_limit";
        }
        const classified = classifyFailoverReason(foParams.message);
        return classified ?? "auth";
      };

      const throwAuthProfileFailover = (foParams: {
        allInCooldown: boolean;
        message?: string;
        error?: unknown;
      }): never => {
        ...
        const message =
          foParams.message?.trim() ||
          (foParams.error ? describeUnknownError(foParams.error).trim() : "") ||
          fallbackMessage;
        const reason = resolveAuthProfileFailoverReason({
          allInCooldown: foParams.allInCooldown,
          message,
        });
        if (fallbackConfigured) {
          throw new FailoverError(message, {
            ...
            cause: foParams.error,
          });
        }
        if (foParams.error instanceof Error) {
          throw foParams.error;
        }
      ...
      const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
        if (runtimeCredentialForProvider?.accessToken) {
          ...
          authStorage.setRuntimeApiKey(model.provider, runtimeCredentialForProvider.accessToken);
      ...
      const providerGenerations: ProviderGenerationSummary[] = [];
      ...
      try {
        while (true) {
          ...
          if (attempt.providerGenerations?.length) {
            providerGenerations.push(...attempt.providerGenerations);
          }
          ...
          if (contextOverflowError) {
            ...
            emitContextOverflowRecoveryEvent(params, {
              phase: "overflow_detected",
              trigger: "overflow",
              source: contextOverflowError.source,
            ...
            if (
              ...
              emitContextOverflowRecoveryEvent(params, {
                ...
                willRetry: false,
                reason: compactResult.reason ?? "nothing to compact",
                diagId: overflowDiagId,
              });
            }
            ...
            if (!toolResultTruncationAttempted) {
              ...
              if (hasOversized) {
                ...
                emitContextOverflowRecoveryEvent(params, {
                  phase: "tool_result_truncation",
                  trigger: "tool_result_truncation",
                  source: contextOverflowError.source,
                  attempt: overflowCompactionAttempts,
                  maxAttempts: MAX_OVERFLOW_COMPACTION_ATTEMPTS,
                  outcome: "attempted",
                ...
                emitContextOverflowRecoveryEvent(params, {
                  ...
                  maxAttempts: MAX_OVERFLOW_COMPACTION_ATTEMPTS,
                  outcome: "failed",
                  reason: truncResult.reason ?? "unknown",
                  diagId: overflowDiagId,
                });
          ...
          const lastAssistantAny = lastAssistant as Record<string, unknown> | undefined;
          const lastProviderResponseId = resolveProviderResponseIdFromAssistant(lastAssistantAny);
          const providerGenerationIdList = providerGenerationIds(providerGenerations);
          const providerResponseId =
            providerGenerationIdList.at(-1) ?? lastProviderResponseId ?? undefined;
          const providerCost =
            sumProviderGenerationCosts(providerGenerations) ??
            resolveProviderCostFromAssistant(lastAssistantAny);
          const agentMeta: EmbeddedPiAgentMeta = {
            ...
            providerResponseId,
            providerGenerationIds: providerGenerationIdList.length
              ? providerGenerationIdList
              : undefined,
            providerGenerations: providerGenerations.length
              ? providerGenerations.slice()
              : undefined,
            providerCost,
          };
          ...
          return {
            ...
            meta: {
              ...
              aborted,
              systemPromptReport: attempt.systemPromptReport,
              stopReason: attempt.clientToolCalls ? "tool_calls" : undefined,
              pendingToolCalls: attempt.clientToolCalls
                ? attempt.clientToolCalls.map((tc) => ({
                    id: tc.toolCallId,
                    name: tc.name,
