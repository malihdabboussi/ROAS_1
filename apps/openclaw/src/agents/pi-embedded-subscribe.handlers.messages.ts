...
export function handleMessageUpdate(
  ...
  if (evtType === "toolcall_delta") {
    const contentIndex =
      typeof assistantRecord?.contentIndex === "number" ? assistantRecord.contentIndex : -1;
    const partial = assistantRecord?.partial as { content?: unknown[] } | undefined;
    const toolBlock =
      partial && Array.isArray(partial.content) && contentIndex >= 0
        ? (partial.content[contentIndex] as Record<string, unknown> | undefined)
        : undefined;
    const partialArgs =
      typeof toolBlock?.partialArgs === "string"
        ? toolBlock.partialArgs
        : typeof toolBlock?.n === "string"
          ? toolBlock.n
          : null;
    const toolName = typeof toolBlock?.name === "string" ? toolBlock.name : "";
    const toolCallId =
      typeof toolBlock?.id === "string"
        ? toolBlock.id
        : typeof toolBlock?.toolCallId === "string"
          ? toolBlock.toolCallId
          : "";
    if (partialArgs) {
      emitAgentEvent({
        runId: ctx.params.runId,
        stream: "tool_args_delta",
        data: {
          name: toolName,
          toolCallId,
          partialJson: partialArgs,
        },
      });
    }
    return;
...
export function handleMessageEnd(
  ...
  ctx.recordProviderGeneration(assistantMessage);
  ...
  let replyCleanedText = parsedText?.text ?? "";
  let replyMediaUrls = parsedText?.mediaUrls;
  let hasMedia = Boolean(replyMediaUrls && replyMediaUrls.length > 0);

  if (!replyCleanedText && !hasMedia) {
    ...
    if (rawCandidate) {
      ...
      replyCleanedText = parsedFallback.text ?? rawCandidate;
      replyMediaUrls = parsedFallback.mediaUrls;
      hasMedia = Boolean(replyMediaUrls && replyMediaUrls.length > 0);
    }
  }

  if (!ctx.state.emittedAssistantUpdate && (replyCleanedText || hasMedia)) {
    emitAgentEvent({
      runId: ctx.params.runId,
      stream: "assistant",
      data: {
        text: replyCleanedText,
        delta: replyCleanedText,
        mediaUrls: hasMedia ? replyMediaUrls : undefined,
      },
    });
    void ctx.params.onAgentEvent?.({
      stream: "assistant",
