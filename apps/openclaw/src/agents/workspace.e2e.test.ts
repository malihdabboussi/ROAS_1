...
describe("ensureAgentWorkspace", () => {
  it("rejects macOS user paths on non-macOS runtimes before mkdir", () => {
    expect(() => assertRuntimeWorkspacePath("/Users/dev/.openclaw/workspace", "linux")).toThrow(
      "Invalid workspace path for runtime: /Users/dev/.openclaw/workspace",
    );
  });
...
describe("loadWorkspaceBootstrapFiles", () => {
  it("loads identity files before tools so role content has budget priority", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");

    const files = await loadWorkspaceBootstrapFiles(tempDir);

    expect(files.slice(0, 5).map((file) => file.name)).toEqual([
      DEFAULT_AGENTS_FILENAME,
      DEFAULT_SOUL_FILENAME,
      DEFAULT_IDENTITY_FILENAME,
      DEFAULT_ROLE_FILENAME,
