...
describe("buildWorkspaceSkillSnapshot", () => {
  ...
  it("omits disable-model-invocation skills from the prompt", async () => {
    ...
    expect(snapshot.prompt).toContain("<id>disk:visible-skill</id>");
    expect(snapshot.prompt).not.toContain("SKILL.md");
  ...
  it("includes DB skill catalog metadata without paths or markdown bodies", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-"));
    await _writeSkill({
      dir: path.join(workspaceDir, "skills", "disk-skill"),
      name: "disk-skill",
    ...
    expect(snapshot.prompt).toContain("<id>disk:disk-skill</id>");
    expect(snapshot.prompt).toContain("<id>db:db-skill</id>");
    expect(snapshot.prompt).toContain("<name>db-skill</name>");
    expect(snapshot.prompt).toContain("<description>DB skill</description>");
    expect(snapshot.prompt).not.toContain("SKILL.md");
    expect(snapshot.prompt).not.toContain("<location>");
    expect(snapshot.externalSkills).toEqual([
      {
        id: "db:db-skill",
        skill_key: "db-skill",
        name: "db-skill",
