...
describe("workspace path resolution", () => {
  ...
  it("rejects directory paths before host read reaches raw fs.readFile", async () => {
    await withTempDir("openclaw-ws-", async (workspaceDir) => {
      ...
      await expect(readTool!.execute("ws-read-dir", { path: "docs" })).rejects.toThrow(
        /path is a directory/,
      );
    });
  });

  it("rejects URL-like paths before local host file tools resolve them", async () => {
    await withTempDir("openclaw-ws-", async (workspaceDir) => {
      const tools = createOpenClawCodingTools({ workspaceDir });
      const readTool = tools.find((tool) => tool.name === "read");
      ...
      await expect(
        readTool!.execute("ws-read-url", { path: "https://example.com/image.png" }),
      ).rejects.toThrow(/local workspace file path.*URL/i);
      await expect(
        writeTool!.execute("ws-write-url", {
          path: "https://example.com/output.txt",
          content: "x",
        }),
      ).rejects.toThrow(/local workspace file path.*URL/i);
      await expect(
        editTool!.execute("ws-edit-url", {
          path: "https://example.com/output.txt",
          oldText: "x",
          newText: "y",
        }),
...
describe("sandboxed workspace paths", () => {
  it("rejects directory paths before sandbox read reaches raw file IO", async () => {
    await withTempDir("openclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("openclaw-workspace-", async (workspaceDir) => {
        await fs.mkdir(path.join(sandboxDir, "docs"));
        const sandbox = {
          enabled: true,
        ...
        await expect(readTool!.execute("sbx-read-dir", { path: "docs" })).rejects.toThrow(
          /path is a directory/,
        );
      });
    });
  ...
  it("rejects URL-like paths before sandbox file tools resolve them", async () => {
    await withTempDir("openclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("openclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        await expect(
          readTool!.execute("sbx-read-url", { path: "https://example.com/image.png" }),
        ).rejects.toThrow(/local workspace file path.*URL/i);
        await expect(
          writeTool!.execute("sbx-write-url", {
            path: "https://example.com/output.txt",
            content: "x",
          }),
        ).rejects.toThrow(/local workspace file path.*URL/i);
        await expect(
          editTool!.execute("sbx-edit-url", {
            path: "https://example.com/output.txt",
            oldText: "x",
            newText: "y",
          }),
