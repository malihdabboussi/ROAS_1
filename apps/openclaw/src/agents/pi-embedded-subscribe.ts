...
export function subscribeEmbeddedPiSession(params: SubscribeEmbeddedPiSessionParams) {
  ...
  const providerGenerationKeys = new Set<string>();
  ...
  const recordProviderGeneration = (assistantMessage: AgentMessage) => {
    const usage = normalizeUsage(
      ((assistantMessage as { usage?: unknown }).usage ?? undefined) as UsageLike | undefined,
    );
    const generation = buildProviderGenerationFromAssistant(
      assistantMessage,
      hasNonzeroUsage(usage) ? usage : undefined,
    );
    if (!generation) {
      return;
    }
    const generationKey = generation.generationId ?? generation.providerResponseId;
    if (generationKey) {
      if (providerGenerationKeys.has(generationKey)) {
        return;
      }
      providerGenerationKeys.add(generationKey);
    }
  ...
  const ctx: EmbeddedPiSubscribeContext = {
    ...
    getUsageTotals,
    getProviderGenerations: () => providerGenerations.slice(),
