...
import {
  classifyToolFailure,
  extractToolFailureEnvelope,
  toUserSafeFailureMessage,
} from "./platform-failure.js";
...
export async function handleToolExecutionEnd(
  ...
  if (isToolError) {
    const contractedFailure = extractToolFailureEnvelope(sanitizedResult);
    const errorMessage = contractedFailure?.error || extractToolErrorMessage(sanitizedResult);
    const failure = contractedFailure ?? classifyToolFailure({ errorText: errorMessage });
    ctx.state.lastToolError = {
      toolName,
      meta,
      error: errorMessage,
      failure,
      userSafeMessage: toUserSafeFailureMessage(failure),
