...
function applyContextCap(
  ...
  if (cap && cap < info.tokens) {
    return { tokens: cap, source };
  }
  return info;
}
...
export function resolveContextWindowInfo(params: {
  ...
  const agentCapTokens = normalizePositiveInt(params.cfg?.agents?.defaults?.contextTokens);
  const agentCapped = applyContextCap(baseInfo, agentCapTokens, "agentContextTokens");
  const requestCapTokens = normalizePositiveInt(params.requestContextTokens);
  return applyContextCap(agentCapped, requestCapTokens, "requestOverride");
}
