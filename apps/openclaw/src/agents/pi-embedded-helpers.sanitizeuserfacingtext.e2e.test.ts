...
describe("sanitizeUserFacingText", () => {
  ...
  it("rewrites direct requests for user secrets", () => {
    expect(sanitizeUserFacingText("Please provide your OpenAI API key so I can continue.")).toBe(
      "I can't take API keys, tokens, or secrets in chat. I'll use the available Vibey tools and connected integrations instead.",
    );
  });

  it("redacts secret values in assistant text", () => {
    const result = sanitizeUserFacingText("Use OPENAI_API_KEY=sk-proj-aB2cD3eF4gH5iJ6kL7");
    expect(result).toContain("OPENAI_API_KEY=[REDACTED_SECRET]");
    expect(result).not.toContain("sk-proj-");
  });

  it("does not rewrite policy discussion about secret requests", () => {
    const text = "Agents should never ask users to provide API keys or tokens.";
    expect(sanitizeUserFacingText(text)).toBe(text);
  });
