...
describe("context-window-guard", () => {
  ...
  it("uses requestContextTokens as the effective run cap when below the model window", () => {
    const info = resolveContextWindowInfo({
      cfg: undefined,
      provider: "openrouter",
      ...
      defaultTokens: 200_000,
      requestContextTokens: 272_000,
    });
    const guard = evaluateContextWindowGuard({ info });

    expect(info.source).toBe("requestOverride");
    expect(info.tokens).toBe(272_000);
    expect(guard.shouldWarn).toBe(false);
    expect(guard.shouldBlock).toBe(false);
  });

  it("does not shrink the effective window when requestContextTokens exceeds the model window", () => {
    const info = resolveContextWindowInfo({
      cfg: undefined,
      provider: "openrouter",
      modelId: "openai/gpt-5.5",
      ...
      defaultTokens: 200_000,
      requestContextTokens: 1_050_000,
    });
  ...
  it("evaluates compaction guard thresholds against the selected request cap", () => {
    const info = resolveContextWindowInfo({
      cfg: undefined,
      provider: "openrouter",
      modelId: "small-selected-context",
      modelContextWindow: 1_050_000,
      defaultTokens: 200_000,
      requestContextTokens: 24_000,
    });
    const guard = evaluateContextWindowGuard({ info });
