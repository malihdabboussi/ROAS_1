...
export function formatBillingErrorMessage(provider?: string): string {
  ...
  if (providerName) {
    return `⚠️ ${providerName} returned a billing error. Vibey could not complete that provider call because the platform-managed provider balance or billing state needs attention.`;
  }
  return "⚠️ API provider returned a billing error. Vibey could not complete that provider call because the platform-managed provider balance or billing state needs attention.";
}
...
const SECRET_REQUEST_USER_MESSAGE =
...
const SECRET_POLICY_DISCUSSION_PATTERN =
...
function containsUserSecretRequest(input: string): boolean {
  if (!input || SECRET_POLICY_DISCUSSION_PATTERN.test(input)) {
    return false;
  }
...
export function isLikelyContextOverflowError(errorMessage?: string): boolean {
  ...
  // Billing/credit errors (e.g. OpenRouter 402 "requires more credits, or fewer
  // max_tokens") can match the broad CONTEXT_OVERFLOW_HINT_RE because they
  // contain "request", "token", and "limit" — triggering false-positive
  // compaction loops. Exclude them early.
...
export function sanitizeUserFacingText(text: string, opts?: { errorContext?: boolean }): string {
  ...
  const secretSafe = redactSecretsInText(stripped);
  const trimmed = secretSafe.trim();
  if (!trimmed) {
    return "";
  }
  if (containsUserSecretRequest(trimmed)) {
    return SECRET_REQUEST_USER_MESSAGE;
  }
  ...
  const withoutLeadingEmptyLines = secretSafe.replace(/^(?:[ \t]*\r?\n)+/, "");
...
const ERROR_PATTERNS = {
  ...
  billing: [
    ...
    /^\s*402\b.*(?:credits?|afford|billing|payment|balance)/i,
    "payment required",
