...
describe("buildBootstrapContextFiles", () => {
  ...
  it("keeps ROLE.md whole before truncating later tools content", () => {
    const roleContent = "r".repeat(3_684);
    const files = [
      makeFile({ name: "AGENTS.md", content: "a".repeat(8_259) }),
      makeFile({ name: "SOUL.md", path: "/tmp/SOUL.md", content: "s".repeat(4_036) }),
      makeFile({
        name: "IDENTITY.md",
        path: "/tmp/IDENTITY.md",
        content: "i".repeat(1_645),
      }),
      makeFile({ name: "ROLE.md", path: "/tmp/ROLE.md", content: roleContent }),
      makeFile({ name: "TOOLS.md", path: "/tmp/TOOLS.md", content: "t".repeat(13_663) }),
    ];

    const result = buildBootstrapContextFiles(files, { totalMaxChars: 30_000 });
    const role = result.find((file) => file.path === "/tmp/ROLE.md");
    const tools = result.find((file) => file.path === "/tmp/TOOLS.md");

    expect(role?.content).toBe(roleContent);
    expect(tools?.content).toContain("[...truncated, read TOOLS.md for full content...]");
  });
