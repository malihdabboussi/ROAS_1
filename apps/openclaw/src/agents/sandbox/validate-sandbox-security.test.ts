...
describe("getBlockedBindReason", () => {
  ...
  it("blocks /var because it covers Docker socket directories", () => {
    expect(getBlockedBindReason("/var:/var")).toEqual(expect.objectContaining({ kind: "covers" }));
  });
});
...
describe("validateBindMounts", () => {
  ...
  it("blocks parent mounts that would expose the Docker socket", () => {
    ...
    expect(() => validateBindMounts(["/var:/var"])).toThrow(/blocked path "\/var\/run"/);
  });
