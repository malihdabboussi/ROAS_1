...
const URL_SCHEME_PATH_RE = /^[a-zA-Z][a-zA-Z0-9+.-]*:\/\//;
const URL_VALUE_PATH_RE = /^(?:https?|data|file|blob|s3|gs):/i;
...
function isUrlLikePath(value: string): boolean {
  ...
  if (!trimmed || WINDOWS_DRIVE_PATH_RE.test(trimmed)) {
    return false;
  }
  return URL_SCHEME_PATH_RE.test(trimmed) || URL_VALUE_PATH_RE.test(trimmed);
}

function assertLocalWorkspaceFilePath(
  record: Record<string, unknown> | undefined,
  toolName: string,
): void {
  const filePath = typeof record?.path === "string" ? record.path : "";
  if (!isUrlLikePath(filePath)) {
    return;
  }
  throw parameterValidationError(
    `${toolName}: path must be a local workspace file path, not a URL. Use web_fetch for web URLs or pass asset_ref to Vibey media actions`,
  );
}
...
export function wrapToolParamNormalization(
  ...
  return {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      assertLocalWorkspaceFilePath(record, tool.name);
...
type ReadToolGuardOptions = {
  root?: string;
  statInputPath?: (filePath: string) => Promise<Pick<SandboxFsStat, "type"> | null>;
};

function expandLocalReadPath(filePath: string): string {
  if (filePath === "~") {
    return os.homedir();
  }
...
function resolveLocalReadPath(filePath: string, root: string): string {
  const expanded = expandLocalReadPath(filePath);
  return path.isAbsolute(expanded) ? expanded : path.resolve(root, expanded);
}

async function statLocalReadPath(
  filePath: string,
  root: string,
): Promise<Pick<SandboxFsStat, "type"> | null> {
  try {
    const stats = await fs.stat(resolveLocalReadPath(filePath, root));
    return { type: stats.isDirectory() ? "directory" : stats.isFile() ? "file" : "other" };
  } catch (error) {
...
async function assertReadTargetIsFile(
  record: Record<string, unknown> | undefined,
  options?: ReadToolGuardOptions,
): Promise<void> {
  ...
  const stat =
    options?.statInputPath !== undefined
      ? await options.statInputPath(filePath)
      : options?.root !== undefined
        ? await statLocalReadPath(filePath, options.root)
        : null;
  if (!stat) {
    return;
  }
  if (stat.type === "directory") {
    throw new Error(`read: path is a directory, not a file. Use ls instead (${filePath})`);
  }
  if (stat.type !== "file") {
    throw new Error(`read: path is not a regular file (${filePath})`);
  }
}
...
export function createSandboxedReadTool(params: SandboxToolParams) {
  ...
  return createOpenClawReadTool(base, {
    statInputPath: (filePath) => params.bridge.stat({ filePath, cwd: params.root }),
  });
}
...
export function createOpenClawReadTool(
  ...
  return {
    ...
    execute: async (toolCallId, params, signal) => {
      ...
      assertLocalWorkspaceFilePath(record, base.name);
      await assertReadTargetIsFile(record, options);
