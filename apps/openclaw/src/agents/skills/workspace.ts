...
import { buildDbSkillId, buildDiskSkillId } from "./types.js";
...
function escapeSkillPromptValue(value: string): string {
  return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

function normalizeExternalSkillCatalog(
  catalog: ExternalSkillCatalog | undefined,
): ExternalSkillCatalogEntry[] {
  if (!catalog || catalog.source !== "vibey_db" || !Array.isArray(catalog.entries)) {
    return [];
  }
  const byId = new Map<string, ExternalSkillCatalogEntry>();
  for (const entry of catalog.entries) {
    const skillKey = typeof entry.skill_key === "string" ? entry.skill_key.trim() : "";
    if (!skillKey) {
      continue;
    }
    const id = buildDbSkillId(skillKey);
    byId.set(id, {
      id,
      skill_key: skillKey,
      name: typeof entry.name === "string" && entry.name.trim() ? entry.name.trim() : skillKey,
...
function toDiskPromptEntry(skill: Skill): SkillPromptEntry {
  return {
    id: buildDiskSkillId(skill.name),
    name: skill.name,
    description: skill.description?.trim() || skill.name,
  };
}

function toExternalPromptEntry(entry: ExternalSkillCatalogEntry): SkillPromptEntry {
  return {
    id: entry.id,
    name: entry.name,
...
export function formatSkillsForPrompt(skills: Array<Skill | SkillPromptEntry>): string {
  if (skills.length === 0) {
    return "";
  }
  const entries = skills.map((entry) => {
    const promptEntry: SkillPromptEntry =
      "id" in entry
        ? entry
        : {
            id: buildDiskSkillId(entry.name),
            name: entry.name,
            description: entry.description?.trim() || entry.name,
          };
    return [
      "  <skill>",
      `    <id>${escapeSkillPromptValue(promptEntry.id)}</id>`,
      `    <name>${escapeSkillPromptValue(promptEntry.name)}</name>`,
      `    <description>${escapeSkillPromptValue(promptEntry.description)}</description>`,
      "  </skill>",
    ].join("\n");
  });
...
function loadSkillEntries(
  ...
  const workspaceWorkflowsDir = path.resolve(workspaceDir, "workflows");
  const workspaceWorkflows = loadSkills({
    dir: workspaceWorkflowsDir,
    source: "openclaw-workspace-workflows",
  });
  ...
  for (const skill of workspaceWorkflows) {
    merged.set(skill.name, skill);
  }
...
export function buildWorkspaceSkillSnapshot(
  ...
  opts?: {
    ...
    externalSkillCatalog?: ExternalSkillCatalog;
  },
): SkillSnapshot {
  ...
  const externalSkills = normalizeExternalSkillCatalog(opts?.externalSkillCatalog);
  const prompt = formatSkillsForPrompt([
    ...resolvedSkills.map(toDiskPromptEntry),
    ...externalSkills.map(toExternalPromptEntry),
  ]);
  ...
  return {
    ...
    skills: [
      ...
      ...externalSkills.map((entry) => ({ name: entry.name })),
    ],
    ...(skillFilter === undefined ? {} : { skillFilter }),
    resolvedSkills,
    ...(externalSkills.length > 0 ? { externalSkills } : {}),
...
export function buildWorkspaceSkillsPrompt(
  ...
  return formatSkillsForPrompt([
    ...promptEntries.map((entry) => toDiskPromptEntry(entry.skill)),
    ...externalSkills.map(toExternalPromptEntry),
  ]);
}
...
export function resolveSkillsPromptForRun(params: {
  ...
  if (params.entries && params.entries.length > 0) {
    const prompt = buildWorkspaceSkillsPrompt(params.workspaceDir, {
      ...
      externalSkillCatalog: params.externalSkillCatalog,
    });
    return prompt.trim() ? prompt : "";
  }
  const externalSkills = normalizeExternalSkillCatalog(params.externalSkillCatalog);
  if (externalSkills.length > 0) {
    return formatSkillsForPrompt(externalSkills.map(toExternalPromptEntry));
  }
  return "";
}
