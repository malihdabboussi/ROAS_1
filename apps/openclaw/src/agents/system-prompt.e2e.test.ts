...
describe("buildAgentSystemPrompt", () => {
  ...
  it("uses strict vibey prompt mode with policy-filtered tools only", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      promptMode: "vibey",
      toolNames: ["campaign_capability", "platform_capability"],
      skillsPrompt:
        "<available_skills>\n  <skill>\n    <name>widget-builder</name>\n  </skill>\n</available_skills>",
      extraSystemPrompt: "CAMPAIGN_CONTEXT: active",
      contextFiles: [{ path: "IDENTITY.md", content: "You are Viktor." }],
    });

    expect(prompt).toContain("## Tools");
    expect(prompt).toContain("campaign_capability");
    expect(prompt).toContain("platform_capability");
    expect(prompt).toContain("## Skills");
    expect(prompt).toContain("widget-builder");
    expect(prompt).toContain("## Vibey Context");
    expect(prompt).toContain("CAMPAIGN_CONTEXT: active");
    expect(prompt).toContain("# Project Context");
    expect(prompt).toContain("## IDENTITY.md");
