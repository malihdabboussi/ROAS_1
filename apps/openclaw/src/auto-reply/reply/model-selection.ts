...
export function resolveModelDirectiveSelection(params: {
  ...
  const resolveFuzzy = (paramsAt436: {
    provider?: string;
    fragment: string;
  }): { selection?: ModelDirectiveSelection; error?: string } => {
    const fragment = paramsAt436.fragment.trim().toLowerCase();
    if (!fragment) {
      return {};
    }

    const providerFilter = paramsAt436.provider
      ? normalizeProviderId(paramsAt436.provider)
      : undefined;
