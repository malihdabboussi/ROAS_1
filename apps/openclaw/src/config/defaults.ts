...
/**
 * Default `cacheRetention: "long"` for any `openrouter/anthropic/*` model entry.
 * Independent of local Anthropic auth profile — OpenRouter ships its own provider key.
 *
 * Consumed at request time by createOpenRouterCacheControlWrapper, which injects
 * top-level `cache_control` into the OpenRouter chat payload.
 */
function applyOpenRouterAnthropicCacheDefaults(cfg: OpenClawConfig): OpenClawConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
  const existingModels = defaults.models ?? {};
  const nextModels: Record<string, { params?: Record<string, unknown> } & Record<string, unknown>> =
    { ...existingModels };
  let modelsMutated = false;

  const ensureCacheRetention = (key: string): void => {
    if (!key.startsWith("openrouter/anthropic/")) {
      return;
    }
...
export function applyContextPruningDefaults(cfg: OpenClawConfig): OpenClawConfig {
  const baseCfg = applyOpenRouterAnthropicCacheDefaults(cfg);
  const defaults = baseCfg.agents?.defaults;
  if (!defaults) {
    return baseCfg;
  }

  const authMode = resolveAnthropicDefaultAuthMode(baseCfg);
  if (!authMode) {
    return baseCfg;
  }
