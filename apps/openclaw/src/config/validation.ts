...
function validateConfigObjectWithPluginsBase(
  ...
  const ensureRegistry = (): RegistryInfo => {
    ...
    for (const diag of registry.diagnostics) {
      let pathAt223 = diag.pluginId ? `plugins.entries.${diag.pluginId}` : "plugins";
      if (!diag.pluginId && diag.message.includes("plugin path not found")) {
        pathAt223 = "plugins.load.paths";
      }
      const pluginLabel = diag.pluginId ? `plugin ${diag.pluginId}` : "plugin";
      const message = `${pluginLabel}: ${diag.message}`;
      if (diag.level === "error") {
        issues.push({ path: pathAt223, message });
      } else {
        warnings.push({ path: pathAt223, message });
      }
    }
  ...
  const validateHeartbeatTarget = (target: string | undefined, pathAt270: string) => {
    if (typeof target !== "string") {
      return;
    }
    const trimmed = target.trim();
    if (!trimmed) {
      issues.push({ path: pathAt270, message: "heartbeat target must not be empty" });
      return;
    }
    ...
    issues.push({ path: pathAt270, message: `unknown heartbeat target: ${target}` });
  };
