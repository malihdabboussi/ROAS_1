...
function collectEnvRefPaths(value: unknown, pathAt217: string, output: Map<string, string>): void {
  if (typeof value === "string") {
    if (containsEnvVarReference(value)) {
      output.set(pathAt217, value);
    }
    return;
  }
  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      collectEnvRefPaths(item, `${pathAt217}[${index}]`, output);
    });
    return;
  }
  if (isPlainObject(value)) {
    for (const [key, child] of Object.entries(value)) {
      const childPath = pathAt217 ? `${pathAt217}.${key}` : key;
...
function collectChangedPaths(
  ...
  if (Array.isArray(base) && Array.isArray(target)) {
    ...
    for (let index = 0; index < max; index += 1) {
      const childPath = pathAt241 ? `${pathAt241}[${index}]` : `[${index}]`;
  ...
  if (isPlainObject(base) && isPlainObject(target)) {
    const keys = new Set([...Object.keys(base), ...Object.keys(target)]);
    for (const key of keys) {
      const childPath = pathAt241 ? `${pathAt241}.${key}` : key;
      const hasBase = key in base;
      const hasTarget = key in target;
...
function isPathChanged(pathAt287: string, changedPaths: Set<string>): boolean {
  if (changedPaths.has(pathAt287)) {
    return true;
  }
  let current = parentPath(pathAt287);
  while (current) {
...
function restoreEnvRefsFromMap(
  ...
  if (typeof value === "string") {
    if (!isPathChanged(pathAt303, changedPaths)) {
      const original = envRefMap.get(pathAt303);
  ...
  if (isPlainObject(value)) {
    ...
    for (const [key, child] of Object.entries(value)) {
      const childPath = pathAt303 ? `${pathAt303}.${key}` : key;
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfigAt529(): OpenClawConfig {
    try {
  ...
  async function readConfigFileSnapshotAt804(): Promise<ConfigFileSnapshot> {
    const result = await readConfigFileSnapshotInternal();
    return result.snapshot;
  }

  async function readConfigFileSnapshotForWriteAt809(): Promise<ReadConfigFileSnapshotForWriteResult> {
    const result = await readConfigFileSnapshotInternal();
    return {
  ...
  async function writeConfigFileAt820(cfg: OpenClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
  ...
  return {
    configPath,
    loadConfig: loadConfigAt529,
    readConfigFileSnapshot: readConfigFileSnapshotAt804,
    readConfigFileSnapshotForWrite: readConfigFileSnapshotForWriteAt809,
    writeConfigFile: writeConfigFileAt820,
  };
}
