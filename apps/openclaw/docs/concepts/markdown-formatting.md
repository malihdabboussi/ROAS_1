...
- **Slack:** eligible Markdown tables are sent as native Block Kit `table`
  blocks. The configured `markdown.tables` mode is still used as the readable
  fallback for oversized tables and text-only delivery paths such as file
  upload captions.
