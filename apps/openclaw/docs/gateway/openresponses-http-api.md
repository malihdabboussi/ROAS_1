...
`previous_response_id` is supported for chained OpenResponses turns.

- You can pass either:
  - a prior OpenClaw response id (`resp_*`), or
  - a prior provider response id (`provider_response_id` from response metadata).
- The Gateway resolves the prior response and reuses its session continuity.

Validation rules:

- If `previous_response_id` is not found, the endpoint returns `400 invalid_request_error`.
- If `previous_response_id` belongs to a different agent than the current request, the endpoint returns `400 invalid_request_error`.
- If both `previous_response_id` and `x-openclaw-session-key` are provided, they must point to the same session. A mismatch returns `400 invalid_request_error`.

Precedence:

1. If `previous_response_id` is valid, its session is used.
2. Otherwise, explicit `x-openclaw-session-key` is used.
3. Otherwise, `user`-derived stable session key is used.
4. Otherwise, request stays stateless (new random session key).
