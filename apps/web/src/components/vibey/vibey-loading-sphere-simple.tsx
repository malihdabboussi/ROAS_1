...
function useRingData(size: SizeVariant) {
...
function SimpleRing({
  ...
  smoothMouse?: { current: { x: number; y: number } }
}) {
  ...
  useFrame((frameState, delta) => {
    ...
    if (instancedMeshRef.current) {
      ...
      dotsData.forEach((dot, i) => {
        ...
        if (mouseInteractive && smoothMouse?.current) {
          const mx = smoothMouse.current.x
          const my = smoothMouse.current.y
          const dx = wavePositionVec.x - mx
          const dy = wavePositionVec.y - my
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist < 0.8 && dist > 0.001) {
            const strength = ((0.8 - dist) / 0.8) * 0.08
            wavePositionVec.x += (dx / dist) * strength
            wavePositionVec.y += (dy / dist) * strength
          }
        }
    ...
    if (meshRef.current) {
      ...
      particleData.forEach((particle, i) => {
        ...
        if (mouseInteractive && smoothMouse?.current) {
          const mx = smoothMouse.current.x
          const my = smoothMouse.current.y
          const dx = (finalX as number) - mx
          const dy = (finalY as number) - my
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist < 0.5 && dist > 0.001) {
            const strength = ((0.5 - dist) / 0.5) * 0.12
            finalX = (finalX as number) + (dx / dist) * strength
            finalY = (finalY as number) + (dy / dist) * strength
          }
        }
...
function SimpleVibeyScene({
  ...
  useFrame((_frameState, delta) => {
    if (mouseInteractive && mouseGroupRef.current) {
      const targetRX = -_frameState.pointer.y * 0.3
      const targetRY = _frameState.pointer.x * 0.3
      mouseGroupRef.current.rotation.x +=
        (targetRX - mouseGroupRef.current.rotation.x) * 2.0 * delta
      mouseGroupRef.current.rotation.y +=
        (targetRY - mouseGroupRef.current.rotation.y) * 2.0 * delta
      smoothMouseRef.current.x +=
        (_frameState.pointer.x * 1.0 - smoothMouseRef.current.x) * 3.0 * delta
      smoothMouseRef.current.y +=
        (_frameState.pointer.y * 1.0 - smoothMouseRef.current.y) * 3.0 * delta
    }
...
export function VibeyLoadingSphereSimple({
  ...
  const bloomRadius = 0.3 // Fixed at 0.5 to match full version (contained glow)
    ...
    >
      ...
      {/* {showBackground && (
        <div style={{
          ...
          width: '100%',
          height: '100%',
          background: 'radial-gradient(circle, rgba(13, 13, 13, 1) 0%, rgba(13, 13, 13, 1) 100%)',
      ...
      {/* Animation layer with transparent canvas */}
      <div
        style={{
          position: 'relative',
          zIndex: 1,
          width: '100%',
          height: '100%',
          ...(clipToCircle ? { borderRadius: '100%', overflow: 'hidden' } : {}),
        }}
      >
        <Canvas
          ...
          onCreated={(state) => {
            // Always transparent canvas - CSS background handles dark circle
            if (state.gl) {
        ...
        >
          ...
          {/* Bloom effect for all themes */}
          {canvasReady && (
            <EffectComposer multisampling={0}>
              <AnimatedBloom
                baseIntensity={bloomIntensity}
                threshold={bloomThreshold}
                radius={bloomRadius}
                state={state}
                onPulseChange={setBloomPulse}
              />
            </EffectComposer>
          )}
        </Canvas>
