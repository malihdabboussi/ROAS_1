...
import type { CanvasItemKind, CanvasOperation, WhiteboardEdge, WhiteboardNode } from '../types/whiteboard.types'
...
export function useCanvasSelectionActions({
  ...
  const setItemsLocked = useCallback((itemIds: string[], locked: boolean) => {
    setNodes((current) => current.map((node) => itemIds.includes(node.id)
      ? { ...node, draggable: !locked, data: { ...node.data, locked } }
      : node))
    commit(itemIds.map((itemId) => ({ op: 'update_item', item_id: itemId, patch: { locked } })))
  }, [commit, setNodes])

  const alignItems = useCallback((itemIds: string[], axis: 'horizontal' | 'vertical') => {
    const selected = nodesRef.current.filter((node) => itemIds.includes(node.id))
    if (selected.length < 2) return
    const positionKey = axis === 'horizontal' ? 'y' : 'x'
    const target = selected.reduce((sum, node) => sum + node.position[positionKey], 0) / selected.length
    const positions = new Map(selected.map((node) => [node.id, { ...node.position, [positionKey]: target }]))
    setNodes((current) => current.map((node) => positions.has(node.id)
      ? { ...node, position: positions.get(node.id)! }
      : node))
    commit(selected.map((node) => ({
      op: 'update_item',
      item_id: node.id,
  ...
  const distributeItems = useCallback((itemIds: string[], axis: 'horizontal' | 'vertical') => {
    ...
    const positions = new Map(selected.map((node, index) => [
      node.id,
    ...
    setNodes((current) => current.map((node) => positions.has(node.id)
      ? { ...node, position: positions.get(node.id)! }
      : node))
    commit(selected.map((node, index) => ({
      op: 'update_item',
      item_id: node.id,
      patch: positionKey === 'x'
