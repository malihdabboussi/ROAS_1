  ...
  return (
    <aside className="surface-card border-border absolute bottom-spacing-3 right-spacing-3 top-spacing-3 z-30 flex w-96 flex-col overflow-hidden rounded-spacing-3 border shadow-xl">
      <header className="border-border flex items-center gap-spacing-2 border-b px-spacing-3 py-spacing-2">
        <span className="badge-glass-blue flex h-8 w-8 items-center justify-center rounded-spacing-2"><Boxes className="h-4 w-4" /></span>
        ...
        <button type="button" className="button-ghost flex h-8 w-8 items-center justify-center" onClick={onClose} aria-label="Close resource library"><X className="h-4 w-4" /></button>
      </header>
      <div className="min-h-0 flex-1 overflow-y-auto p-spacing-3">
        {!summary ? (
          <p className="body-3 text-muted-foreground text-center">
            {failed ? CANVAS_VIEW_MESSAGES.resourcesUnavailable : CANVAS_VIEW_MESSAGES.resourcesLoading}
          </p>
        ) : groups.length === 0 ? (
          ...
          <section key={group.type} className="mb-spacing-4">
            <h3 className="body-4 text-muted-foreground mb-spacing-2 font-semibold uppercase">{RESOURCE_LABELS[group.type]}</h3>
            <div className="flex flex-col gap-spacing-1">
              {group.resources.map((resource) => (
                <button key={resource.id} type="button" className="button-glass-neutral body-3 w-full justify-start text-left" onClick={() => onSelect({ id: resource.id, title: resource.name, type: group.type })}>
                  {resource.name}
                </button>
              ))}
            </div>
