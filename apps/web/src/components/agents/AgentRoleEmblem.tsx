...
const SVG_SIZE_CLASS: Record<'sm' | 'lg', string> = {
  sm: 'h-4 w-4',
  lg: 'h-7 w-7',
}

export function AgentRoleEmblem({ roleKey, size }: { roleKey: string; size: 'sm' | 'lg' }) {
  const path = ROLE_SHAPES[roleKey] ?? ROLE_SHAPES.developer

  return (
    <span
      className={`badge-glass badge-glass-green flex shrink-0 items-center justify-center rounded-full p-0 ${SIZE_CLASS[size]}`}
    >
      <svg viewBox="0 0 80 80" className={SVG_SIZE_CLASS[size]} aria-hidden>
