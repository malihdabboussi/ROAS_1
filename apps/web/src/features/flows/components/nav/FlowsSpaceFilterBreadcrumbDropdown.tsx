...
export function FlowsSpaceFilterBreadcrumbDropdown({
  spaces,
  filterCampaignId,
  selectedSpaceId,
  onSelectSpace,
  ...
  const visibleSpaces = useMemo(() => {
    const filtered = filterCampaignId
      ? spaces.filter((space) => space.campaign_id === filterCampaignId)
      : spaces
    return filtered
  }, [filterCampaignId, spaces])
  ...
  const groupedSpaces = useMemo(
    () => groupFlowSpacesByCampaign(visibleSpaces, campaigns),
    [campaigns, visibleSpaces],
  )
  ...
  return (
    <div className="relative min-w-0" ref={rootRef}>
      <button
        ...
        aria-expanded={open}
        aria-label={FLOWS_UI.filterBySpace}
      ...
      >
        <span className="truncate">{selectedSpace?.title ?? FLOWS_UI.allSpaces}</span>
        <ChevronDown
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-spacing-1 py-spacing-2">
            <button
              ...
              className={cn(
                ...
                !selectedSpaceId && 'nav-glass-selected-purple',
              )}
            >
              <span className="min-w-0 flex-1 truncate font-medium">{FLOWS_UI.allSpaces}</span>
              {!selectedSpaceId ? <Check className="icon-xs text-primary shrink-0" /> : null}
            </button>
