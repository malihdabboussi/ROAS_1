...
describe('Team2DetailView', () => {
  ...
  it('opens the canonical chat filtered to the selected agent without duplicating the team floor', () => {
    render(
      <Team2DetailView
        agent={agent}
        infoPanelTab="info"
    ...
    expect(screen.getByText('Agents grid remains visible').parentElement?.className).toContain(
      'hidden',
    )
  ...
  it('restores the team floor when AI chat is minimized', () => {
    render(
      <Team2DetailView
        agent={agent}
        infoPanelTab="info"
    ...
    act(() => useShellStore.getState().minimizeChatDrawer())
