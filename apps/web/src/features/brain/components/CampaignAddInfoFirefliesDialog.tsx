...
import { formatCampaignInfoMeetingTime } from './campaign-add-info-helpers'

interface CampaignAddInfoFirefliesDialogProps {
  importingMeetingId: string | null
  loading: boolean
  onImportTranscript: (transcript: FirefliesTranscript) => void
  onOpenChange: (open: boolean) => void
  open: boolean
  transcripts: FirefliesTranscript[]
}

export function CampaignAddInfoFirefliesDialog({
  importingMeetingId,
  loading,
  onImportTranscript,
  onOpenChange,
  open,
  transcripts,
}: CampaignAddInfoFirefliesDialogProps) {
  return (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="typo-caption text-muted-foreground">
                            {formatCampaignInfoMeetingTime(transcript.date)}
                          </p>
                        </div>
                        <button
                          type="button"
