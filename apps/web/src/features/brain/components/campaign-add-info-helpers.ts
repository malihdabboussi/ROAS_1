...
export function detectCampaignInfoLinkType(url: string): {
  supported: boolean
  platform?: string
} {
...
export function formatCampaignInfoMeetingTime(iso?: string | number): string {
...
export function insertCampaignInfoTextAtPosition(
  base: string,
  position: number,
  text: string,
): string {
...
export function getCampaignInfoMeetingId(meeting: MeetingIdentity): string {
  return String(
    meeting.id || meeting.recording_id || meeting.call_id || meeting.url || meeting.title,
  )
}

export function sortCampaignInfoFathomMeetings<T extends MeetingTimestamp>(meetings: T[]): T[] {
