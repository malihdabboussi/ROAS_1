    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        emailRedirectTo: (() => {
          ...
          if (promoCode) cbParams.set('promo', promoCode)
          const rp = getRedirectPath()
          if (rp !== '/home') cbParams.set('redirect', rp)
          const qs = cbParams.toString()
          return `${window.location.origin}/callback${qs ? `?${qs}` : ''}`
        })(),
  ...
  if (WAITLIST_MODE) {
    return (
      <AuthOrbShell quotes={SIGNUP_QUOTES}>
        <p className="body-2 text-muted-foreground text-center">Redirecting…</p>
      </AuthOrbShell>
    )
  }
