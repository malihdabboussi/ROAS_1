...
export class DocumentIngestionService {
  ...
  async ingest(
    ...
    const evidenceBrainId =
      this.evidenceIngestion && process.env.BRAIN_EVIDENCE_CHUNKS === '1'
        ? await this.resolveDefaultBrainId(supabase, input.ownerId)
        : null
  ...
  private async resolveDefaultBrainId(supabase: SupabaseClient, ownerId: string): Promise<string> {
    const { data: existing } = await this.repository.findDefaultUserBrainId(supabase, ownerId)
    if (existing?.id) return existing.id

    const { data: created, error } = await this.repository.createDefaultUserBrain(
      supabase,
      ownerId,
    )
    if (created?.id) return created.id
    throw new Error(
      `Failed creating or finding default brain: ${error?.message ?? 'unknown error'}`,
    )
  }
...
Be strict. Only flag genuine relationships, not vague topic overlap.`
    ...
    for (const candidate of candidates.slice(0, 3)) {
      try {
        ...
        const result = await this.embedding.callGemini(prompt, undefined, {
          userId: ownerId,
          orgId: billing.orgId,
        })
