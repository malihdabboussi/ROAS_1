...
import {
  EmbeddingService,
  type BrainGeminiBillingContext,
 ...
 *
 * Extracts meaningful memories from user-agent conversations (ONLY conversations).
 * 1. Validate source type (conversations only)
 * 2. Format messages and call Gemini extraction
 * 3. Significance gate (filter < 0.6)
 * 4. Dedup via content_hash, embed, save directly to memories
...
export class ConversationProcessingService {
  ...
  async processConversation(
    ...
    const billing = { userId: ownerId, orgId: data.org_id }

    const usageTotals: GeminiTokenUsage = { inputTokens: 0, outputTokens: 0, totalTokens: 0 }
    ...
    const extraction = await this.embedding.callGeminiWithUsage(prompt, undefined, billing)
    this.addUsage(usageTotals, extraction.usage)
    ...
    try {
      ...
      return { status: 'error', reason: 'LLM returned invalid JSON', usage: usageTotals }
    }

    if (!Array.isArray(extracted) || extracted.length === 0) {
      return {
        status: 'skipped',
        reason: 'No significant memories found',
        memories_created: 0,
        usage: usageTotals,
      }
    }
    ...
    // ── Step 4: Dedup, embed, save directly ────────────────────────────
    ...
    for (const item of significant) {
      ...
      const vectorResult = await this.embedding.getEmbeddingWithUsage(item.content, { billing })
      this.addUsage(usageTotals, vectorResult.usage)
    ...
    this.logger.log(
      `Processed conversation: ${savedMemories.length} saved, ${duplicateCount} dupes`,
    )
...
${conversationText}`
  ...
  private async resolveOrCreateCampaignBrainId(
    ...
    const { data: fallback, error: fallbackError } = await supabase
      ...
      .maybeSingle()
    if (fallbackError)
      throw new BadRequestException(`Could not load campaign brain: ${fallbackError.message}`)
