...
import type { AgentCapability, ResolvedAgentPolicy } from '../agent-policy.types'
...
export class AgentPolicyActionDecisionService {
  resolvePolicyBackedActionDecision(
    action: Action,
    policy: ResolvedAgentPolicy,
    roleDomains: readonly Domain[],
    isSystemAgent: boolean,
  ): PolicyDecision {
    const teamGrants = this.extractActionDomains(policy.grants)
    const agentAllowExtra = this.extractActionDomains(policy.overrides.allow_extra)
    const agentDeny = this.extractActionDomains(policy.overrides.deny)

    // Protected system agents are engineering-owned and never gated by
    // user-toggleable team policy. Legacy builders keep their existing runtime
    // bypass until the builder path is explicitly retired.
    if (isSystemAgent) {
    ...
    return resolveActionPolicy(action, {
      roleDomains: usesActionDomainPolicy ? [] : roleDomains,
      teamGrants,
      agentAllowExtra,
      agentDeny,
    })
  ...
  resolveActionContractDecision(action: Action, agentKey: string): PolicyDecision | null {
  ...
  resolveSystemDomainDecision(action: Action, agentKey: string): PolicyDecision | null {
