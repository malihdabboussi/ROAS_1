export const SOURCE_OF_TRUTH_INSTRUCTIONS = [
  ...
  'Brain items marked "trained" are rules the user taught their Brain on purpose. Apply them ahead of general best practice, cite them as the user\'s own rule, and say so plainly when a trained rule conflicts with other evidence or with what you would otherwise recommend. Low mastery means the rule is new or lightly evidenced, so hedge accordingly.',
