...
const PROMPTS = [
  ...
  textPrompt(
    ...
    [
      ...
      '- For questions about what the user thinks, believes, prefers, or decided about one topic, use synthesize_user_brain_topic.',
  ...
  textPrompt(
    'vibey_synthesize_user_brain_topic',
    'Answer what the authenticated user thinks about a topic from cited User Brain evidence.',
    [
      'Use this workflow for questions such as "What do I think about webinars?" or "How have my views on pricing changed?"',
      '',
      'Workflow:',
      "1. Call synthesize_user_brain_topic with a concise topic and the user's full question.",
      '2. Read coverage before drafting. If context_sufficient is false, state the gap and ask for a narrower topic or more source material.',
      '3. Draft the short answer from synthesis.short_answer_basis, then organize supporting beliefs, decisions, preferences, frameworks, stories, and perspectives.',
      '4. Cite evidence refs such as [E1] after every material claim and preserve source titles when useful.',
      '5. Surface meaningful tensions or changes only when the returned evidence supports them.',
      '',
      "Do not invent a belief, chronology, contradiction, or source. Do not treat retrieval score as certainty about the user's current view.",
    ].join('\n'),
  ),
  textPrompt(
