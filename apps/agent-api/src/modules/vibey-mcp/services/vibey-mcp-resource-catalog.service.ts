...
const RESOURCES = [
  ...
  markdownResource(
    ...
    [
      ...
      '',
      'UI General workspace:',
      '1. Call `list_campaigns` with `mode: "accessible"`.',
      '2. Select the unique campaign with `config.system_kind="general"`.',
      '3. Call `list_spaces` with `campaign_id` from that campaign.',
      '4. Do not use `general=true` for the UI General workspace. `general=true` returns only uncampaigned Spaces where `campaign_id is null`.',
      '',
      'Documents:',
  ...
  markdownResource(
    ...
    [
      ...
      '- User beliefs, preferences, decisions, or thinking about one topic: `synthesize_user_brain_topic`.',
  ...
  markdownResource(
    ...
    'User Brain Topic Synthesis Workflow',
    'How to answer what the authenticated user thinks about a topic from cited evidence.',
    [
      '# User Brain Topic Synthesis',
      '',
      'Use `synthesize_user_brain_topic` when the user asks what they think, believe, prefer, decided, learned, or repeatedly said about one topic.',
      '',
      'The tool searches several topic angles and returns:',
      '- `synthesis.short_answer_basis` for the most direct answer evidence.',
      '- grouped evidence refs for beliefs, decisions, preferences, frameworks, stories, perspectives, and synthesized context.',
      '- `coverage` with query counts, source counts, sufficiency, and confidence.',
      '- `gaps` when the Brain lacks enough direct evidence.',
      '- `evidence` records keyed as `E1`, `E2`, and so on, with excerpts and source metadata.',
      '',
      'Response standard:',
      '1. Lead with a concise answer supported by the short-answer refs.',
      '2. Add themes or sections only when they help answer the question.',
      '3. Put evidence refs such as `[E1]` immediately after supported claims.',
      '4. Distinguish recurring beliefs from one-off stories or historical decisions.',
      '5. If `coverage.context_sufficient` is false, say what is missing rather than filling gaps from general knowledge.',
      '',
      "Do not invent a source, belief, contradiction, or evolution. Retrieval relevance is not proof that every excerpt expresses the user's current position.",
    ].join('\n'),
  ),
  markdownResource(
