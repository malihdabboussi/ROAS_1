...
export const VIBEY_API_ACTION_DOCS: Record<string, VibeyActionDoc> = {
  ...
  search_campaign_brain: {
    section: 'Campaign',
    description:
      'Searches the campaign brain (ns_memories on the campaign-scoped ns_brains row) for durable client research, approved decisions, onboarding intake, strategy notes, and ROAS-brain package knowledge. Requires query. Prefer campaign chat scope, or pass campaign_id / campaign_name when the chat is on General or another campaign (cross-scope read allowed). Passing campaign_id or campaign_name binds this portal conversation so CONNECTIONS shows that client; it does not lock the whole Slack DM identity. Never target General. The response includes canonical_source, as_of, evidence, and brain_context. Use it to interpret current reporting, tasks, meetings, or Slack evidence; do not substitute Brain for those mutable owners.',
    parameters:
  ...
  list_tasks: {
    section: 'Tasks',
    description:
      'Lists task rows backed by space_items with filters applied before limit. For one Space, pass space_id. For "my tasks", "what should I do today", or current-user work across the organization, omit space_id and pass assigned_to_me:true; the backend resolves the session user, searches every accessible Space, and removes each Space\'s closed statuses. Never retry a failed assigned_to_me query without that ownership filter. Optional filters: status, category, filters, assignee_id, parent_item_id, search, include_closed, fields, include_count, sort_by, sort_direction, limit.',
    parameters:
      '```json\n{"action":"list_tasks","label":"Checking my priorities","data":{"assigned_to_me":true,"fields":"summary","include_count":true,"sort_by":"due_date","limit":30}}\n```\n```json\n{"action":"list_tasks","label":"Reviewing tasks","data":{"space_id":"UUID","fields":"summary","limit":30}}\n```\n```json\n{"action":"list_tasks","label":"Checking fixed bugs","data":{"space_id":"UUID","status":"in_review","category":"bugs","fields":"summary","include_count":true,"limit":10}}\n```',
  },
  get_task: {
    section: 'Tasks',
    description:
  ...
  get_campaign_main_dashboard: {
    section: 'Analytics',
    description:
      'Returns the aggregated campaign dashboard in one call: executive KPIs (leads, conversion, open rate, engagement, reach, visitors), per-channel summaries (funnels / emails / ads / social), unified daily timeseries, contribution breakdown by source, and threshold-based alerts. Same numbers the user sees on the Studio > Dashboard > Overview tab. The response identifies campaign_reporting as canonical_source, includes an as_of timestamp and evidence, and leaves brain_context null. Start here for any "how is my campaign doing" question, then use search_campaign_brain only to interpret durable decisions and conversation context. For organization-wide health, first list accessible campaigns, call this once per eligible active client campaign, and distinguish missing or partial data from a real KPI alert.',
    parameters:
