...
export const VIBEY_API_ACTION_DOCS: Record<string, VibeyActionDoc> = {
  ...
  process_media: {
    ...
    parameters:
      '```json\n{"action":"process_media","label":"Analyzing video","data":{"operation":"probe","asset_ref":{"kind":"external_asset","url":"https://..."},"url":"https://..."}}\n```\nOperations: trim{url,start_seconds,duration_seconds} | concat{asset_refs:[{url}],inputs:[{url}]} | convert{url,output_format} | extract_audio{url} | add_audio{video_url,audio_url,replace} | resize{url,resolution} | compose{asset_refs:[{url}],inputs:[{url,trim_start?,trim_duration?}],audio_url?,output_format?,resolution?} | audio_effect{url,effect,...params} | silence_remove{url,noise_db?,min_silence_duration?} | color_grade{url,brightness?,contrast?,saturation?,gamma?,hue?,temperature?,preset?} | blur{url,strength?,mode?} | vignette{url,angle?} | sharpen{url,amount?} | denoise{url,strength?,mode?} | reverse{url,include_audio?} | loop{url,count} | speed{url,factor} | overlay{url,overlay_url,position?,scale?,start_time?,end_time?,x?,y?} | crop{url,width,height,x?,y?} | text_overlay{url,text,font_size?,font_color?,background_color?,position?,start_time?,end_time?} | subtitle_burn{url,subtitle_content?,subtitle_url?} | transition{url,url2,transition?,duration?} | chroma_key{url,background_url,color?,similarity?,blend?} | split_screen{asset_refs:[{url}],inputs:[{url}],layout?} | thumbnail{url,timestamp,output_format?} | frame_extract{url,interval_seconds?,max_frames?,output_format?} | waveform{url,width?,height?,color?,output_format?} | probe{url}\nPositions: top-left|top-right|bottom-left|bottom-right|center|top|bottom\nTransitions: fade|fadeblack|fadewhite|dissolve|wipeleft|wiperight|wipeup|wipedown|slideleft|slideright|circlecrop|rectcrop|radial|smoothleft|smoothright|pixelize\nAudio effects: reverb|echo|fade_in|fade_out|volume|pitch|normalize|bass_boost|speed\nColor presets: vintage|lighter|darker|increase_contrast|negative|cross_process\nLayouts: horizontal|vertical',
  },
  list_mcp_servers: {
    section: 'MCP',
    description:
