...
function buildImportantPatterns(sections: Set<string>, availableActions: Set<string>): string {
  ...
  if (sections.has('Tasks')) {
    ...
    patterns.push(
      `**My Tasks is a cross-Space, fail-closed query**: for "my tasks", "what should I focus on today", "what is on top today", or current-user work, call \`list_tasks\` once with \`assigned_to_me: true\` and no \`space_id\`. The backend resolves the current user across every accessible Space and removes each Space's closed statuses. Never retry without \`assigned_to_me\`, never substitute unassigned or delegated work, and never widen to all tasks when this query fails. For a daily brief, call \`list_calendar_events\` for today's window separately and combine only the verified results; Brain explains context but is not a task source.`,
    )
    patterns.push(
