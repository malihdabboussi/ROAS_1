...
describe('agent capability source drift guardrail', () => {
  ...
  it('keeps named mission playbooks materializable from agent-facing docs', () => {
    ...
    expect(ACTION_SCHEMAS.create_mission?.descriptions?.playbook_id).toContain('client-lifecycle')
    ...
    expect(VIBEY_API_ACTION_DOCS.create_mission.description).toContain('client-lifecycle')
