...
import type {
  SpaceSemanticAsset,
  SpaceSemanticEdgeInput,
  SpaceSemanticEdgeType,
...
export class SpaceStructuralEdgeBuilderService {
  ...
  private async resolveViewSourceId(
    ...
    const data = await this.repository.findSpaceSchema(supabase, spaceId)
    const schema = this.record(data?.schema)
  ...
  private async resolveSourceTypeForSourceId(
    ...
    return this.repository.findSemanticObjectSourceType(supabase, sourceId, spaceId)
  }
