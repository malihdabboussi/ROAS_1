import { SocialResearchScrapeCreatorsServiceBase07 } from './social-research-scrapecreators-service-07.base'
...
export abstract class SocialResearchScrapeCreatorsServiceBase08 extends SocialResearchScrapeCreatorsServiceBase07 {

  protected async fetchYtPagedContent(
    handle: string,
    userId: string,
  ...
  ): Promise<Omit<SocialContentItem, 'outlier_score'>[]> {
    const cutoff = Date.now() - REELS_WINDOW_DAYS * 86_400_000
    const rawItems: Omit<SocialContentItem, 'outlier_score'>[] = []
    let token: string | undefined
    let pages = 0

    while (pages < MAX_PAGES) {
      if (pages > 0) await wait(PAGE_DELAY_MS)
      const page = await fetchPage(handle, userId, orgId, channelId, token)
      pages++
      let stoppedByDate = false
      for (const item of page.items) {
        const takenAt = new Date(item.taken_at).getTime()
        if (takenAt < cutoff) {
          stoppedByDate = true
          break
        }
      ...
      if (stoppedByDate || !page.nextToken || page.items.length === 0) break
  ...
  protected async fetchYtVideos(
    ...
    if (!resolvedChannelId) {
      try {
        const profile = await this.fetchYtProfile(handle, userId, orgId)
        resolvedChannelId = profile.user_id
      } catch {
        resolvedChannelId = null
      }
    }
    const [videos, shorts] = await Promise.all([
      this.fetchYtPagedContent(
        handle,
        userId,
        orgId,
        resolvedChannelId,
        this.fetchYtVideosOnePage.bind(this),
      ),
      this.fetchYtPagedContent(
        handle,
        userId,
        orgId,
        resolvedChannelId,
        this.fetchYtShortsOnePage.bind(this),
      ),
    ])
    const byId = new Map<string, Omit<SocialContentItem, 'outlier_score'>>()
    for (const item of [...videos, ...shorts]) {
      if (item.media_id) byId.set(item.media_id, item)
    }
    const merged = [...byId.values()]
    const scored = computeOutlierScores(merged.map((m) => ({ ...m, outlier_score: 0 })))
    scored.sort((a, b) => b.outlier_score - a.outlier_score)
    return scored
  }

  protected async fetchYtPostInfo(
    postUrl: string,
    userId: string,
    orgId?: string | null,
    ...
    const details = unwrapUpstreamPayload(body) as YtVideoDetails
    const videoId = details?.id ?? details?.videoId
    const lengthSeconds =
      details?.lengthInSeconds ??
      details?.lengthSeconds ??
      (details?.durationMs != null ? Math.round(details.durationMs / 1000) : null)
    const mapped: SocialRichPostInfo = {
      caption: details?.title ?? null,
      post_description: details?.description ?? null,
      like_count: details?.likeCountInt ?? details?.likeCount ?? null,
      comment_count: details?.commentCountInt ?? details?.commentCount ?? null,
      play_count: details?.viewCountInt ?? null,
      video_duration: lengthSeconds,
      owner_username: details?.channel?.handle?.replace(/^@/, '') ?? null,
      owner_full_name: details?.channel?.title ?? null,
      owner_follower_count: null,
      owner_is_verified: false,
      owner_profile_pic: details?.channel?.thumbnail ?? null,
      ...
      tagged_users: [],
      has_audio: true,
      post_genre: details?.genre ?? null,
      keyword_names: details?.keywords ?? [],
    }
    if (!videoId) return this.emptyRichPostInfo()
    return mapped
  }

  protected async fetchYtTranscript(
    postUrl: string,
    userId: string,
    orgId?: string | null,
  ): Promise<string | null> {
    try {
      ...
      const payload = unwrapUpstreamPayload(body) as YtTranscriptResponse
      if (
        typeof payload?.transcript_only_text === 'string' &&
        payload.transcript_only_text.trim()
      ) {
        return payload.transcript_only_text.trim()
      }
  ...
  protected twitterNormalizedHandle(handle: string): string {
    return handle.replace(/^@/, '').trim()
  }

  protected twitterProfileQuery(handle: string): Record<string, string> {
    return { handle: this.twitterNormalizedHandle(handle) }
  }

  protected twitterTweetMedia(t: TwitterTweetItem): NonNullable<TwitterTweetItem['media']> {
    return t.media ?? t.legacy?.extended_entities?.media ?? t.legacy?.entities?.media ?? []
  }

  protected twitterTweetId(t: TwitterTweetItem): string {
    return String(t.rest_id ?? t.legacy?.id_str ?? t.id_str ?? '')
  }
}
