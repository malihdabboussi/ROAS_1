...
export async function analyzeSlackTeamMessages(input: {
  ...
  for (let offset = 0; offset < input.messages.length; offset += 250) {
    ...
    const transcript = input.messages
      ...
      .join('\n')
    const prompt = [
      'Analyze recent Slack messages to detect every high-confidence operational signal supported by the supplied window.',
      `Requested loop: ${input.loopKind}. Return up to ${remaining} high-confidence signals.`,
      'Delivery ranking happens after detection. Do not omit a credible unanswered question, client risk, commitment, or durable memory merely because another item seems more important.',
      ...
      'For briefing-oriented signals, explain business impact, urgency, novelty, confidence, and actionability so a later selector can rank them.',
