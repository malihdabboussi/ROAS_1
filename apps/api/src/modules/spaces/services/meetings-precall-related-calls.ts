/** Related-call matching for Agenda: Fathom/manual call rows ↔ calendar events. */

import type { PrecallAgendaEventLike } from './meetings-precall-prep.helpers'
...
export type AgendaRelatedCall = {
  ...
  /** Fathom recording id when external automation stamped the call. */
...
/** Fathom recording id stamped on the call by external automation. */
export function resolveAgendaExternalRecordingId(
  custom: Record<string, unknown> | null | undefined,
): string | null {
  const automation = custom?.external_automation
  if (!automation || typeof automation !== 'object') return null
  const meetingId = (automation as Record<string, unknown>).meeting_id
  if (typeof meetingId === 'string' && meetingId.trim()) return meetingId.trim()
  if (typeof meetingId === 'number' && Number.isFinite(meetingId)) return String(meetingId)
  return null
}
...
export function toAgendaRelatedCall(input: {
  ...
  return {
    ...
    external_recording_id: resolveAgendaExternalRecordingId(custom),
...
/**
 * Post-gate reward so the recording-bearing duplicate wins the event over a
 * recording-less twin (mirrors findBestExistingCallForEvent's recording bias).
 */
export const RELATED_CALL_RECORDING_BONUS = 25
...
export function scoreRelatedCallMatch(
  ...
  const recordingBonus = call.has_recording ? RELATED_CALL_RECORDING_BONUS : 0
  return emailHits * 15 + Math.round(titleSim * 20) + timeBonus + recordingBonus
}
...
/** Score every event×call pair (recording-aware) for exclusive assignment. */
export function buildRelatedCallCandidates(
...
): RelatedCallMatchCandidate[] {
  const candidates: RelatedCallMatchCandidate[] = []
  for (const event of events) {
    for (const call of calls) {
      ...
      const score = scoreRelatedCallMatch(event, {
        ...
        has_recording: Boolean(
          resolveAgendaRecordingUrl(custom) ?? resolveAgendaExternalRecordingId(custom),
        ),
      })
      if (score <= 0) continue
