import { SocialResearchScrapeCreatorsServiceBase09 } from './social-research-scrapecreators-service-09.base'
...
export abstract class SocialResearchScrapeCreatorsServiceBase10 extends SocialResearchScrapeCreatorsServiceBase09 {

  protected async fetchTwitterTweet(
    postUrl: string,
    userId: string,
    orgId?: string | null,
    ...
    const payload = unwrapUpstreamPayload(body) as TwitterTweetDetailsResponse
    const tweetId = this.twitterTweetId(payload)
    if (!tweetId) return this.emptyRichPostInfo()
    const author = this.twitterTweetAuthor(payload)
    const entities = this.twitterTweetEntities(payload)
    return {
      caption:
        ...
        payload.text ??
        null,
      like_count: payload.legacy?.favorite_count ?? payload.favorite_count ?? null,
      comment_count: payload.legacy?.reply_count ?? payload.reply_count ?? null,
      play_count: this.twitterTweetViewCount(payload),
      video_duration: null,
      owner_username: author.screen_name,
      owner_full_name: author.name,
      owner_follower_count: author.followers_count,
      owner_is_verified: author.is_verified,
      owner_profile_pic: author.profile_pic,
      owner_post_count: author.statuses_count,
      audio_name: null,
      audio_artist: null,
      is_original_audio: false,
      is_paid_partnership: false,
      tagged_users: entities.mentions,
      has_audio: this.twitterDetectVideo(payload),
      retweet_count: payload.legacy?.retweet_count ?? payload.retweet_count ?? null,
      quote_count: payload.legacy?.quote_count ?? null,
      bookmark_count: payload.legacy?.bookmark_count ?? null,
      post_lang: payload.legacy?.lang ?? null,
      post_source: this.twitterPostSourceLabel(payload.source),
      hashtag_names: entities.hashtags,
      mention_handles: entities.mentions,
      link_urls: entities.urls,
    }
  }

  protected async fetchTwitterTranscript(
    postUrl: string,
    userId: string,
    orgId?: string | null,
  ): Promise<string | null> {
    try {
      ...
      const payload = unwrapUpstreamPayload(body) as TwitterTranscriptResponse
      if (
  ...
  protected emptyRichPostInfo(): SocialRichPostInfo {
    return {
      caption: null,
      like_count: null,
      comment_count: null,
