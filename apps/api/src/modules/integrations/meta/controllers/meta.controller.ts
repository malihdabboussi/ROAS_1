...
export class MetaController {
  constructor(
    ...
    private readonly metaEligibility: MetaIntegrationsEligibilityService,
  ) {}
  ...
  async connect(
    ...
    const allowed = await this.metaEligibility.isEligible(user.id, orgId)
    if (!allowed) {
      throw new HttpException(
        { success: false, error: 'Meta Ads is available for Admin and Enterprise accounts only.' },
        HttpStatus.FORBIDDEN,
      )
    }

    const validation = StartMetaOAuthSchema.safeParse(body)
  ...
  async callback(@Query('code') code: string, @Query('state') state: string, @Res() res: Response) {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Meta OAuth failed'
