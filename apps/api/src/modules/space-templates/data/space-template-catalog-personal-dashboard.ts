...
const fathomMeetingLog: SpaceTemplateAutomationSeed = {
  ...
  actions: [
    { type: 'change_status', status: 'processing' },
    {
      ...
      prompt_template: [
        'Process this Fathom meeting on the existing Personal Dashboard task. Do not create a duplicate.',
        DRAFT_ONLY_RULE,
        'Title: {{trigger.title}}',
        'Recording: {{trigger.url}}',
        'Attendees: {{trigger.attendees}}',
        'Summary: {{trigger.summary}}',
        'Transcript: {{trigger.transcript_text}}',
        'Action items: {{trigger.action_items}}',
        'Set a clear purpose-first title, recording_url, call_date, attendees, and status on this task.',
        'Create a meeting-log document with purpose, decisions, commitments, open questions, and transcript link.',
        'Suggest concrete follow-up tasks separately. Draft outreach; never send it.',
      ].join('\n'),
    },
    {
