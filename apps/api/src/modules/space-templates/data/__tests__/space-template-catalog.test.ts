...
describe('SPACE_TEMPLATE_CATALOG', () => {
  ...
  it('combines daily work and meeting operations in one role-neutral personal dashboard', () => {
    ...
    expect(
      dashboard?.schema.views.find((view) => view.id === 'all-meetings')?.visible_fields,
    ).toEqual([
      'title',
      'call_kind',
      'campaign_name',
    ...
    expect(dashboard?.schema.fields).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ id: 'campaign_name', name: 'Client Workspace', type: 'text' }),
        expect.objectContaining({ id: 'space_title', name: 'Campaign Space', type: 'text' }),
