...
export class BrainEvidenceIngestionService {
  constructor(
    ...
    private readonly brainEvidenceRepository: BrainEvidenceRepository,
  ) {}

  async writeServiceEvidenceChunks(
    input: BrainEvidenceChunkInput,
  ): Promise<{ chunks_inserted: number; episode_id: string | null }> {
    return this.writeEvidenceChunks(this.brainEvidenceRepository.getServiceClient(), input)
  }
  ...
  async writeEvidenceChunks(
    ...
    if (process.env.BRAIN_EVIDENCE_CHUNKS !== '1') {
      return { chunks_inserted: 0, episode_id: null }
    }
    ...
    for (let index = 0; index < input.chunks.length; index++) {
      ...
      const { error } = await this.brainEvidenceRepository.upsertEvidenceChunk(supabase, {
  ...
  private async upsertEpisode(
    ...
    const { data, error } = await this.brainEvidenceRepository.upsertEpisode(supabase, {
      brain_id: input.brainId,
      source_type: input.sourceType,
      source_id: sourceId,
      source_title: input.sourceTitle ?? null,
