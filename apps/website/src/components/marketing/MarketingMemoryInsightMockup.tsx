...
export type MarketingMemoryInsightVariant = 'dark' | 'light'

export function MarketingMemoryInsightMockup({
  variant = 'dark',
}: {
  variant?: MarketingMemoryInsightVariant
}) {
  ...
  return (
    <div
      className={cn(
        'pointer-events-auto w-full min-w-0 max-w-[20rem] overflow-hidden rounded-lg backdrop-blur-xl md:rounded-xl',
        isLight
          ? 'border border-color-glass bg-color-surface shadow-lg'
          : 'card-glass',
      )}
      style={
        isLight
    ...
    >
      ...
      <div
        className={cn(
          'md:px-spacing-4 md:py-spacing-3 flex items-center justify-between px-2 py-1.5',
          isLight ? 'border-b border-color-glass' : 'border-b border-white/5',
        )}
      >
      ...
      <div className="md:p-spacing-4 space-y-2 p-2 md:space-y-4">
        <p
          className={cn(
            'text-[9px] font-medium leading-tight md:text-[13px] md:leading-relaxed',
          ...
          <div>
            ...
            <div
              className={cn(
                ...
                isLight ? 'bg-color-subtle' : 'bg-white/5',
              )}
            >
              <div
        ...
        <div
          className={cn(
            'space-y-2 border-t pt-1.5 md:space-y-3 md:pt-2',
            isLight ? 'border-color-glass' : 'border-white/5',
          )}
        >
          <div className="flex items-center gap-1 md:gap-2">
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  isLight ? 'bg-[var(--border-glass)]' : 'bg-white/10',
                )}
              />
            </div>
        ...
        <div
          className={cn(
            'flex items-center justify-between border-t pt-1.5 md:pt-2',
            isLight ? 'border-color-glass' : 'border-white/5',
          )}
        >
          <span className="text-muted-foreground text-[8px] font-semibold uppercase tracking-wider md:text-[10px] md:tracking-widest">
          ...
          <span
            className={cn(
              'text-[9px] font-medium md:text-[11px]',
              isLight ? 'text-[var(--text-primary)]' : 'text-white',
            )}
          >
            Call Summary
