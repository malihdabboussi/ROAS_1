...
export default async function FeatureDetailPage({ params }: Props) {
  ...
  const loadMarketingAgentShowcase =
    slug === 'your-team' || slug === 'missions' || slug === 'autopilot' || slug === 'spaces'
  ...
  const missionDetailLibraryAgents =
    slug === 'missions' || slug === 'spaces' ? marketingAgentLibrary : undefined
  const missionDetailVibeyPortraitUrl =
    slug === 'missions' || slug === 'spaces' ? marketingVibeyPortrait : undefined
  ...
  const showcaseBlocks = page.showcase.blocks.map((b) => {
    ...
    if (
      slug === 'spaces' &&
      (b.mockupKind === 'space-views' ||
        b.mockupKind === 'space-docs' ||
        b.mockupKind === 'space-channels' ||
        b.mockupKind === 'space-automations')
    ) {
      block = {
        ...block,
        agentLibraryAgents: marketingAgentLibrary ?? [],
    ...
    if (hrShowcaseData != null && b.title === 'Spin up any agent in plain English') {
  ...
  return (
    <FeaturePageLayout>
        ...
        <section className="section-padding relative">
          <AnimateOnScroll>
            <div className="site-container">
              <div className="glass-card border-section mx-auto max-w-3xl rounded-2xl border p-10 text-center">
                <h2 className="h2 mb-4 tracking-tight text-white">{page.articlePromo.title}</h2>
                <p className="text-text-muted body-2 mb-8 leading-relaxed">{page.articlePromo.subtitle}</p>
                <Link
                  href={page.articlePromo.ctaHref}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="chip-glass-secondary body-2 group inline-flex items-center gap-2 rounded-xl px-8 py-3 font-semibold"
                >
                  {page.articlePromo.ctaLabel}
      ...
      {/* Standalone Video Showcase — commented out
