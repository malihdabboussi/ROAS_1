...
const OUT = path.join(
  REPO_ROOT,
  'supabase/migrations/20260716120000_webinar_pipeline_skills.sql',
)
...
const EXTRA_ASSIGNMENTS: Array<{ templateKey: string; skillKey: string }> = [
  { templateKey: 'copywriter', skillKey: 'dylans-super-voice' },
]
...
async function main(): Promise<void> {
  ...
  for (const skill of SKILLS) {
    ...
    for (const { relPath, absPath } of await walkResources(dir)) {
      ...
      if (isBinary) {
        ...
        for (const agentKey of skill.templateKeys) {
          ...
          lines.push(
            `   WHERE user_id IS NULL AND org_id IS NULL AND agent_key = '${agentKey}'`,
          )
          lines.push(
            `     AND skill_key = '${skill.skillKey}' AND file_path = '${relPath}'`,
          )
          lines.push(`);`)
          lines.push('')
        }
    ...
    for (const templateKey of skill.templateKeys) {
      ...
      lines.push(`)`)
      lines.push(`SELECT`)
      lines.push(`  NULL, NULL, '${templateKey}', skill_key, name, description, markdown_content, true, 'system'`)
      lines.push(`FROM public.skill_library`)
      lines.push(`WHERE skill_key = '${skill.skillKey}'`)
      lines.push(`ON CONFLICT (agent_key, skill_key) WHERE user_id IS NULL AND org_id IS NULL DO UPDATE SET`)
      lines.push(`  name = EXCLUDED.name,`)
      lines.push(`  description = EXCLUDED.description,`)
      lines.push(`  markdown_content = EXCLUDED.markdown_content,`)
      ...
      lines.push(`ON CONFLICT (agent_key, skill_key, file_path) WHERE user_id IS NULL AND org_id IS NULL DO UPDATE SET`)
      lines.push(`  content = EXCLUDED.content,`)
      lines.push(`  content_type = EXCLUDED.content_type,`)
      lines.push(`  updated_at = now();`)
      lines.push('')
