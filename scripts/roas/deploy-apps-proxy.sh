...
if rg -q 'REPLACE_WITH_ROAS_KV_NAMESPACE_ID' "${CONFIG}"; then
  echo "Edit workers/apps-proxy/wrangler.toml:" >&2
...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  cd "${WORKER_DIR}"
  printf '%s' "${SUPABASE_URL}" | "${WRANGLER[@]}" secret put SUPABASE_URL --config wrangler.toml
  printf '%s' "${SUPABASE_SERVICE_ROLE_KEY}" | "${WRANGLER[@]}" secret put SUPABASE_SERVICE_ROLE_KEY --config wrangler.toml
  printf '%s' "${INTERNAL_API_TOKEN}" | "${WRANGLER[@]}" secret put INTERNAL_API_TOKEN --config wrangler.toml
  printf '%s' "${WORKER_SECRET}" | "${WRANGLER[@]}" secret put WORKER_SECRET --config wrangler.toml
else
