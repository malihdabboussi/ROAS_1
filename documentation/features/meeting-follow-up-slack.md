...
- **2026-08-29:** Campaign-status questions bypass broad Brain preload before scope validation. Without a resolved client campaign, Pixel immediately streams a normal user-visible request to select or name one specific campaign; that guidance is not recorded as a failed run and invokes neither model nor tools. With a resolved campaign, the campaign route explicitly reads live reporting, Campaign Brain, and open campaign tasks from that same campaign. Every final answer appends a deterministic evidence receipt with the reporting owner, exact as-of timestamp, Campaign Brain result count, and open-task count; missing or failed evidence is labeled unavailable rather than inferred.
