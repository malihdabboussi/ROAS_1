...
**Last Modified:** 2026-08-29 (all action-intelligence phases production-verified)
...
Production acceptance on 2026-08-29 passed those gates: the pinned daily-focus chat returned one current action, exact Fathom provenance, and the bounded future calendar after reload; the named live-campaign path kept dashboard, Campaign Brain, and task evidence on one canonical campaign; and New Chat replaced stale July review items with current assigned call and agent tasks. API, web, and agent-runtime deployments were verified against their merge SHAs. No task state, due date, assignee, Slack message, or external campaign was mutated during acceptance.
...
- **2026-08-29:** Daily-focus wording such as “What should I focus on today?” and “What's on top today?” always composes canonical My Tasks with today's calendar, even when the user does not repeat the word “meetings.” Explicit task-only questions remain task-only, and calendar-only questions remain calendar-only.
...
- **2026-08-29:** A first-action follow-up is no longer rendered as another agenda inventory. Pixel retrieves the same bounded My Tasks and calendar evidence without Brain fan-out, skips every stale or owner-review item, and deterministically returns exactly the first current My Tasks focus item plus schedule context. It does not wait on a writer or turn one request into several recommendations.
- **2026-08-29:** New Chat suggestions and “In your court” request `focus=current` from the canonical task rollup. The API removes `action_lifecycle.review_state=needs_review` rows before applying the result limit, so old review work cannot displace current tasks on the homepage; the full My Tasks board still retains those rows for explicit Done or Keep open review.
...
- **2026-09-01:** Explicit Slack commands such as “Run post-call flow” are classified as meeting workflow requests and instructed to invoke `request_slack_follow_up_confirm` in active DM mode. They bypass generic task creation, canonical task-source lookup, and Service Request clarification.
- **2026-09-01:** The Auto chat pipeline excludes those explicit post-call commands from quoted-title task provenance lookup, even when the meeting name is quoted and the message contains “meeting” or “call.”
...
- **2026-08-29:** When a campaign-status sentence contains one complete Space title, that explicit title wins before fuzzy phrase matching. Other Spaces sharing client words such as “MultiFamily Strategy” therefore cannot make the exact requested Space ambiguous; sentences containing multiple complete Space titles remain fail-closed.
