...
**Last Modified:** 2026-08-29 (daily-focus performance route, source-of-truth roadmap, canonical call-task assignment, inline delegation review, follow-up refresh, and thread-aware Slack recording retrieval)
...
| --- | --- | --- | --- |
| 1. Canonical call actions and daily focus | Implemented and browser-verified | Provider or manual call commitments become canonical assigned tasks. My Tasks is a cross-Space signed-in-user query, New Chat surfaces the same queue, and operational chat combines that queue with live calendar events without treating Brain as the task store. Narrow agenda prompts bypass semantic Brain preload and model-led tool planning: the server runs the existing validated `list_tasks` and `list_calendar_events` actions in parallel, then deterministically renders every verified record without a model evidence cap. Calendar follow-ups preserve and accurately label their requested window: today, tomorrow, upcoming/next few days, and “after today” do not collapse back to today's events. | `apps/api/src/modules/meetings/repositories/meeting-provider-actions.repository.ts`; `apps/agent-api/src/modules/artifacts/services/artifact-my-tasks.helper.ts`; `apps/agent-api/src/modules/chat/services/chat-operational-agenda.util.ts`; `apps/agent-api/src/modules/chat/services/chat-operational-agenda-format.util.ts`; `apps/agent-api/src/modules/chat/services/chat-stream-execution.service.ts`; `apps/api/src/modules/home/services/next-moves.service.ts`; `apps/web/src/features/home/components/SuggestedNextMoves.tsx` |
| 2. Action lifecycle reconciliation | Planned | Continuously reconcile old, overdue, duplicated, and probably-completed work. Ask the owner whether ambiguous items are done, retain explicit completion evidence, and never create a second task during recap/review/delegation. | `apps/api/src/modules/meetings/repositories/meeting-workspace-state.repository.ts`; `apps/agent-api/src/modules/artifacts/services/artifact-tasks.service.ts`; `apps/web/src/features/home/components/MeetingActionItemsSection.tsx`; `apps/web/src/features/all-tasks/` |
| 3. Conversational evidence coverage | Planned | Link every task and decision to the call, transcript segment, Slack message/thread, or manual note that caused it. Measure weekly capture coverage, expose missing-source gaps, and let contextual follow-ups explain why a priority exists without changing the canonical task record. | `apps/api/src/modules/meetings/repositories/meeting-workspace-read.repository.ts`; `apps/api/src/modules/slack/repositories/slack-digest-evidence.repository.ts`; `apps/agent-api/src/modules/artifacts/services/artifact-conversation-search.service.ts`; `apps/agent-api/src/modules/artifacts/services/artifact-brain-search-actions.service.ts` |
| 4. Brain routing and provenance | Planned | Make Pixel resolve the requested entity and freshness requirement, query its canonical owner, then use Brain to add approved decisions, client knowledge, and cross-source context. Every answer identifies provenance and freshness; missing client scope fails closed instead of falling back to General or another client. | `apps/agent-api/src/modules/artifacts/services/artifact-brain-read-actions.service.ts`; `apps/agent-api/src/modules/chat/services/channel-chat-campaign-bind.ts`; `apps/agent-api/src/modules/chat/services/named-client-campaign-bind.ts`; `.vibey/brain-routing.md`; `.vibey/context-map.md` |
| 5. Live campaign intelligence | Planned | A campaign-status question resolves client/campaign scope, retrieves current reporting/ad metrics with an as-of timestamp, then cross-references Brain decisions and recent call/Slack evidence. Brain stores the durable interpretation and evidence pointer, not a stale copy of today's spend or ROAS. | reporting integration and campaign metric adapters; `apps/agent-api/src/modules/artifacts/services/artifact-brain-search-campaign-brain.test.ts`; campaign binding services above |
| 6. Proactive surfacing and quality | Planned | Home, New Chat, All Tasks, Meetings, and approved Slack loops surface one deduplicated action lifecycle. Track accepted, edited, dismissed, completed, stale, and falsely surfaced items so extraction and ranking improve from real operator feedback. | `apps/api/src/modules/home/services/next-moves.service.ts`; `apps/web/src/features/home/components/SuggestedNextMoves.tsx`; `apps/api/src/modules/spaces/repositories/slack-team-loop.repository.ts`; task and meeting read models above |

Phase 2 acceptance starts with an idempotent reconciliation job and explicit stale-task review states. Phase 3 requires source IDs and evidence excerpts on every conversationally derived task before coverage can be measured. Phase 4 must define one routing contract that returns `{ canonical_source, as_of, evidence, brain_context }`. Phase 5 cannot be marked complete until a signed-in campaign-status browser test proves live metrics retrieval and client-safe Brain cross-reference. Phase 6 requires product telemetry and false-positive review before any new proactive delivery is activated.
