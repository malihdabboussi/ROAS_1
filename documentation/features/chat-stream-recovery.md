...
20. Status returns a stable recovery code when the live run has a bridge error. If Redis has expired, status checks the latest durable runtime row so interrupted and context-limited turns remain recoverable after refresh.
21. If Redis is unavailable or expired without a durable recovery failure, the browser falls back to the existing DB polling recovery path.
22. Provider billing and insufficient-balance failures are classified as `provider_billing` before they reach the web client, so billing exhaustion does not look like a retryable transport interruption.
23. Subscription credential failures are classified as first-class request errors. Missing OpenAI Codex auth prompts the admin reconnect action; missing Claude Subscription auth stops retrying and explains the required admin connection.
24. `ChatService.processMessage` returns the terminal status it actually reached, so the queued internal executor writes `done`, `failed`, `failed_recoverable`, or `cancelled` from the same source of truth as the live stream.
25. When `CHAT_RUN_CHECKPOINTS_ENABLED` is on, Agent API writes compact `agent_runtime_run_checkpoints` rows at coarse tool batches, recoverable failures, and final success. OpenClaw context compaction state arrives through stream events rather than Agent API checkpoint retries.
26. Context-window failures are recovered inside OpenClaw. The runner emits `response.compaction` phases, compacts the transcript or truncates oversized tool results, retries the same run, and only returns `context_window_exceeded` if recovery cannot shrink the next model request.
27. Ordered tool/generation blocks are idempotent by `tool_call_id` or matching active label/action, so replayed or repeated start events update the same visible row instead of rendering duplicate tool calls.
28. Mission Worker can run the Railway chat autoscaler, which watches BullMQ chat queue pressure and updates the shared Railway Agent API replica count through Railway's GraphQL API.
...
- Auto uses Claude Opus 5 with medium reasoning and a supported 300,000-token request window. Economy uses GPT-5.6 Terra with low reasoning and a supported 272,000-token request window. Explicit Power uses Fable 5 with medium reasoning and a supported 300,000-token request window. Sonnet 4.6 is the stable fallback for every strategy. Sonnet 5 and GPT-5.6 Sol remain certified experimental choices rather than production defaults.
...
Transport failures auto-recover first. Redis resume is attempted for healthy active runs, then DB polling recovery runs if resume cannot complete. Explicit bridge interruptions are terminal for the current response: the browser does not advance past the error cursor, status exposes the recovery reason, and partial text without `metadata.duration_ms` is never accepted as a completed answer solely because it is visible.

If the answer still cannot recover, the banner says the work is safe and shows one `Continue response` action. The action refreshes the canonical thread, stops any orphaned active run, and starts one hidden continuation turn grounded in the exact latest visible user request, partial assistant output, and original attachments. A failed continuation keeps the control visible with a retry message instead of clearing the recovery state. The same behavior is restored after refresh from the durable runtime row, including after the live Redis recovery window expires.
...
- 2026-07-28: Made interrupted-turn state durable and user-recoverable across open-tab, race-window, and refresh paths. Partial visible text now requires a completion marker or healthy inactive status; worker bridge failures persist as `failed_recoverable` while the Redis safety lock remains active for orphan cancellation; status exposes only the stable recovery code; and failed continuation attempts keep the action visible.
