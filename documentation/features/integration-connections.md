...
50. Minimizing a Home Agenda occurrence hides its time, title, account, and badges behind a thin restore row. The occurrence is also stored in the owner’s profile preferences, independent of whether Fathom is connected yet. A matching Fathom webhook stops before transcript fetch, Brain import, Meetings automation, or Page Grader sync; restoring the row removes that exclusion. Exact calendar event id wins when Fathom supplies one, with normalized title plus a nearby scheduled start as the fallback.
51. Home Agenda exposes **New call** even when no calendar is connected. It creates an impromptu call item and live workspace in the user’s existing Meetings / Personal Dashboard, opens the same persistent meeting chat and note surface used for scheduled calls, and records the server start time plus optional participant emails. Fathom matching considers both scheduled calendar placeholders and these manual call placeholders within the existing ±10-minute identity window, then attaches the recording, transcript, recap, and provider actions to the workspace instead of creating a duplicate call.
...
- Agenda minimize is an ingestion exclusion, not a visual-only preference. The UI rolls back if the server cannot persist the exclusion so backend processing and the visible state cannot disagree.
- Impromptu calls use the canonical meeting workspace rather than a second scratchpad. The manual placeholder owns the start time, title, participant context, and persistent conversation before recording exists; Fathom enriches that same identity afterward.
