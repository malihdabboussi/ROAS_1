...
Production also exposes an IG organic video-ad launcher. The user can filter by lifestyle vs industry-adjacent footage, optionally narrow to an industry pack (trades & home services, health/fitness, insurance/professional, real estate/mortgage, coaching, social/influencer), then select one or many scenes. They can reuse clean preset footage to avoid generation credits, or request fresh Higgsfield footage. Industry-adjacent scenes currently generate via Higgsfield until clean presets are uploaded. Copy is an explicit first stage: Lux either proposes sticker copy from the campaign and source research or renders user-supplied copy verbatim after approval. Submitting the exact-copy form records `copy_approved: true`, so that click is the approval and an unattended mission can continue without asking for the same confirmation again. Agent-written copy still stops for human approval. The mission preserves its source research ids, selected scene ids, footage fit, industry pack, source strategy, music strategy, and one of the approved Apple-style emoji glyphs (`👇`, `⏰`, `✅`, `🚨`, `🙌`). The `ig-organic-video-ad` skill is available to Lux and Vibey so the same playbook can be started from Production or conversational channels. New footage uses the direct Higgsfield MCP; preset footage and Pillow-rendered stickers remain the credit-saving default.
...
An Ads Research mission cannot treat a document-only market scan as complete. Blaze must save at least 12 usable visual references across at least 3 topic or advertiser searches. Each saved reference preserves the creative thumbnail or snapshot when available, advertiser, format, angle, source URL, relevance, and extracted pattern. Recommendations map back to those references. If the provider cannot supply or save that evidence, the market-research step blocks with the real limitation instead of advancing to approval.
...
### 2026-07-27 - Industry-adjacent scene packs

Six industry packs with three Higgsfield-ready scenes each: trades & home services (merged), health & fitness, insurance & professional, real estate & mortgage, coaching, and social/influencer. Auto-service was removed. Production and Quick Missions filter lifestyle vs industry-adjacent. The skill prefers industry-adjacent for matching educator verticals. Clean presets are not uploaded yet.
...
Ads Research concept selection now completes the real human review gate and creates a linked Meta Ads Launch mission. The approval receipt, selected concepts, recording/design routes, source mission, and source deliverables remain traceable across both missions. Paid Ads consolidates Analyze, Research, and Launch into one tab while preserving legacy Ads Research schema data.
...
Run Research and Rerun Research now provision the campaign's agency team before opening Blaze, eliminating the missing-agent chat failure that restored the kickoff prompt without a response. The Ads Research playbook and Blaze market-research skill now require at least 12 mission-linked visual references across at least 3 searches before the human gate can approve the run.
