...
- Scheduled `observe_slack_team` actions power Pixel Team loops. They paginate every Slack channel Pixel belongs to, inspect the configured lookback window, require source channel/timestamp evidence, and detect Person Brain facts, repeated workflows, unanswered questions, and client-risk signals. Disabled is Off; Shadow creates proposals; Active can compound a managed Person Brain and can send an unanswered-question DM only when the target person is also Active. Daily limits, quiet hours, selected people/channels, Flow run history, and the enabled toggle provide the safety controls.
