...
- `request_slack_follow_up_confirm` DMs meeting follow-ups (admin MVP defaults to `dylan@dylanvanas.com`) after `agent_suggest_tasks`, stores a pending payload on the call item, and on Slack `reaction_added` (`white_check_mark`) marks follow-ups confirmed in ROAS and acknowledges the result in the Slack thread.
