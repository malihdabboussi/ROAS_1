...
- Empty **docked** chat drawer uses a ClickUp Brain–inspired empty state: active-agent hero (Pixel + lamp for the default agent), perpetual marquee capability starters under the hero (pause/expand on hover; seed the composer; clearing text restores the scroller), and verb action pills above the input (Find / Research / Create / …). Capability chips = what to deliver; action pills = how to approach. Full `/home?chat=new` still owns the Home template fan / “For you” extras.
