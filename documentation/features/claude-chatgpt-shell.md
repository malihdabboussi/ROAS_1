...
5. Rail lists Home, Team, Programs, Brain, More (flyouts). Home expands to Home / Inbox / Meetings / My Tasks. Chat history lives inside the AI drawer. Slack-origin rows show a channel badge via `ConversationChannelIcon` / `metadata.source`.
...
- Page work-area collapse applies on every shell route. The page body moves beyond the right edge while chat expands; **Show page** slides it back from right to left. Dragging the outer AI drawer to the viewport edge enters the same collapsed state.
...
- **AI Chats** is the top-left PanelLeft control in the shell top bar (opens/collapses the drawer). The HQ rail under the ROAS logo is logo → Home / Team / Programs / Brain / More only — no under-logo AI Chats pill.
- Chat history lives inside the drawer beside the panel and can be resized separately. Default leading icons are off (Claude/ChatGPT-style text list); filter can still show agent / channel / status marks. History rows use `body-2` titles with `py-spacing-2` / `gap-spacing-2` breathing room and compact relative ages (`19m` / `1d` / `1mo`) on the right; the overflow menu replaces that age on hover.
...
- Chat menu conversation toolbar is Search + filter (defaults: active status, flat list, no leading icons), with **New chat** full-width beneath. Filter options expand inline on click (Type / Status / Last activity / Group by / Icon). Icon chooses agent avatar, channel logo, in-progress/done status, or none; selecting Agent also loads all-agent conversations in the shell drawer. Reset to defaults restores the default filter state. Global Studio search is the centered top-bar Search / Cmd-K only. Agent picker stays in the main chat header (`ShellNewChatAgentBar`). The chat panel **X** closes the drawer.
...
- Home defaults to full-width Agenda + triage Inbox cards. The Home flyout also provides full-page Inbox, Meetings, and My Tasks routes. New Chat (history-toolbar New / `?chat=new`) remains the full former Home hero: greeting, composer, templates, recommendations.
