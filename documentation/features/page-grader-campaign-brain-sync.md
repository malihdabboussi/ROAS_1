...
- **2026-07-20:** Personal Campaigns showed multiple Sakha/Multifamily copies because Page Grader sync (always `org_id` null) filtered mapped campaign lookups to personal-only, missed org campaigns, and created duplicates. Import now loads mapped campaigns by id with ownership/org-member checks, prefers existing org campaigns for the same PG client, and uses the campaign’s org for ingest. Soft-deleted the personal duplicates.
