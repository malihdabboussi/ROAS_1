...
6. New campaigns keep their package `content_hash` empty until deterministic ingest succeeds. Campaign Knowledge rows index in bounded batches of six so large client packages finish within the API request window without unbounded embedding traffic.
