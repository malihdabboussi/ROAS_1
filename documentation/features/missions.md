...
- **Research and campaign brand before THE PLAN**: Webinar Fulfillment runs Blaze's Market Research immediately after post-call strategy and before Reed creates THE PLAN. The step records platform service/action and source links, inventories campaign logos, product imagery, and headshots, extracts verified website branding, and creates or updates the active Campaign Theme. Every Theme field carries a `confirmed`, `inferred`, or `not found` evidence status. An integration may be called unavailable only after a real failed tool attempt with the returned error recorded. THE PLAN consumes the completed research document and Theme instead of rerunning or guessing at provider availability.
...
- **Universal client-writing voice**: every agent-owned Webinar Fulfillment step carries one shared client-writing rule. Any client update, Slack message, recap, approval request, marketing asset, spoken script, slide line, on-image line, or client-facing text created by a fixed or dynamically added step must load `dylans-super-voice` as its only voice authority. `human-written-copy` and `dylans-voice` cannot be combined with it. New employee agents receive Dylan Super Voice as a default library skill, and the migration backfills existing scoped and system agents.
- **Split webinar creative production**: Validate Messaging statics are saved first as `WEB#6 — Validate Messaging Statics` and enhanced with editable `generate_visual_html`. Photographic and illustrative concepts flow through `WEB#7 — Image Briefs` into a separate `generate_image` task. Each visual step loads the active Campaign Theme and uses its verified colors, fonts, logo, brand voice, design settings, headshots, and product imagery. A human creative gate reviews both lanes before Blaze combines approved copy, Theme, and image assets with `create_ad`; copy-only ad shells are rejected. `WEB#8 — Media Plan` waits for those native ads plus the funnel and deck.
...
- **Presentation bundle schema is complete**: `presentation_files` and `presentation_assets` use the same current campaign-access model, so deck builders can persist editable HTML/CSS bundles instead of relying on a legacy `generated_html` row.
...
- 2026-07-17: Made Dylan Super Voice the exclusive voice authority for all client-facing writing in Webinar Fulfillment, including client updates and Slack approval messages, and seeded it as a universal default for employee agents.
