...
While an agent subtask is running, persisted partial output appears as live work in both the subtask workspace and its execution activity. This makes progress recoverable after a refresh or missed realtime broadcast. Structured completion envelopes render only their user-facing `content` or `summary`; incomplete JSON is withheld while it streams, so internal artifact manifests and assertion metadata never appear as raw activity text. A tool-created deliverable whose normalized title and agent match the active subtask appears immediately, before the final completion payload adds its durable subtask link. Once that deliverable exists, the active status reads `Finalizing` until verification completes.
...
Mission drills into the canonical playbook list inside the shared **Create** selector, while lower-frequency creation actions drill into a separate **More** submenu. Choosing a playbook opens its campaign-and-Space confirmation step instead of a second generic Mission picker. New guided Missions are named `<playbook> — <Space>` so simultaneous runs remain distinguishable in chat, Mission Control, and Outputs; Mission receipts also use a dedicated rocket treatment instead of the generic document tile.

Conversation Outputs retain the source message id. An Output's overflow menu or context menu can **Open** it or **Show in chat**, which scrolls to the exact originating receipt. The artifact pane exposes a visible resize grip. When a Mission or subtask detail pane is narrower than 760 pixels, it switches to an **Overview / Activity** selector and mounts only the selected surface instead of squeezing both columns.

Shell page controls separate visibility from layout. Chat shows **Show page** only while the page is closed. An open Mission pane owns one trailing **More**, **Expand/Collapse**, and **Close Page** group; Expand/Collapse changes only the pane layout, while Close Page changes only its visibility.

Chat launches missions from the shared **Create** catalog; the former standalone composer rocket is removed. The empty-chat quick-start row is generated from that same active Create catalog instead of maintaining a second Deep Search/Task/media list, so Mission and every supported creation action seed the identical prompt and tool contract from either entry point. The overflowing row is start-aligned inside its scroll viewport, keeping Mission and the earliest catalog items inside the work surface instead of centering them beneath the docked menu. The shared quick-start component owns the Mission action directly instead of delegating it through the composer-seeding callback, and routes the canonical `create-mission` item by its stable catalog id rather than optional presentation metadata. The quick-start component owns its Mission open state and renders its modal host locally, avoiding module, context, and parent-prop identity boundaries in blank Home and empty active chats. The host preloads client Spaces when it mounts instead of refreshing the Space store during the opening click, so the composer cannot remount and reset the dialog. Active populated chat surfaces provide React-tree launcher state to Create menus and slash commands. It prefills an attached campaign/Space when available but always requires the user to confirm the campaign and Space before execution. On Home, the visible `?conv=` route is the source-conversation authority; a persisted chat-store id from the prior conversation cannot attach a blank Home launch to a hidden chat. When the first action in a blank chat is a Mission, it creates and opens a campaign/Space-scoped conversation before creating the Mission, then records that conversation as the source and inserts the receipt there. Submitting dismisses the launcher immediately with a background-start toast; mission creation continues asynchronously and inserts the durable receipt when the API returns its id, so a slow scope lookup cannot block the conversation or prevent another mission from being started. Receipt persistence reuses the permission lookup that already proved the conversation exists, then performs a deterministic-id upsert; the happy path therefore requires one permission read and one message write instead of a duplicate conversation read plus separate existence and insert requests. Mission receipts remain in the originating transcript, where the shared Mission artifact status polls the persisted Mission and deliverable APIs until a terminal state and shows the current status plus output count. Final-output cards reuse that same status surface, so neither rendering path can remain on its original `Started` metadata after background work completes. Receipts also appear under Outputs in the conversation summary. Selecting either surface opens the interactive Mission workspace in a resizable third pane, so the chat remains available while the user reviews activity, answers human gates, comments, and opens deliverables. The chat's conversation-details menu and summary toggle stay together at the top-right of the second pane; the third pane owns one trailing expand/collapse/close group, with no duplicate shell toggle. Generated documents, PDFs, DOCX files, media, projects, and widgets use the same in-app artifact pane instead of opening browser tabs or becoming inert cards.
...
The **Client Lifecycle** playbook is the guided agency operating path from onboarding through ongoing optimization. It first audits the selected client's Client Brain and live campaign context against Company Brain methodology, then stops for human confirmation before treating onboarding context as authoritative. It reuses existing Avatar and Offer records, requires approval of their exact ids before strategy, binds those ids into canonical campaign context, produces the client strategy and campaign roadmap, and asks the operator which existing production playbooks to launch as child Missions. After that approval, a deterministic Canvas stage reads the current campaign revision and converts the approved roadmap into connected stages, canonical resource cards, and actionable placeholders without replacing unrelated Canvas work. Launch readiness then reconciles child Mission deliverables, native Space tasks, campaign Canvas, and Page Grader Work Requests as distinct canonical records before explicit launch authorization. The lifecycle finishes with an optimization cadence. Later Mission steps and normal client chat both filter Offer and Avatar context through those approved ids instead of silently selecting recent or first-available records; unresolved approved ids remain visible as unresolved and never fall back to another record. Client chat injects this canonical campaign asset block alongside Theme and Brain context, so direct conversation and Mission execution share the same campaign fundamentals. Mission execution also sends the current step request and campaign id through the same Brain retrieval path used by chat, so automatic Client/Campaign Brain and Company Brain grounding is scoped to the work being performed. The playbook never duplicates Avatar or Offer records and never publishes, spends, or sends messages without its human gates. It is available both from the canonical Create > Mission catalog and directly under Create > More, while conversational lifecycle requests use the same playbook id and source-conversation contract.

Mission Details exposes one **Manage mission** menu instead of making operators learn separate execution controls. For Missions with a source conversation, Continue, Extend, Retry from a step, Restart from a stage, Skip or remove a step, Replace a step, and Branch from here reopen that conversation and prefill an operation-specific request. These visual actions do not mutate Mission state directly: Pixel first inspects the persisted Mission, explains the affected steps and downstream impact, and gets the user's confirmation before invoking the existing manager action contract. Mission-manager read actions remain immediate, while context attachment, extension, editing, cancellation, retry, reassignment, replanning, and approval are classified as explicit-intent operations. The same scrollable menu retains deterministic in-place extensions and linked child-playbook launches.
...
The preview groups destination, Copy, downloads, expand/collapse, and close in the document header. Native Space docs also expose `Export to Google Docs`; the first export creates and opens the Google Doc in a tab opened during the same click (so the browser cannot treat it as a blocked popup) and persists its file identity on the Space item, while later clicks are a real `Open Google Doc` link. Text documents expose `Download as Markdown` and `Print as PDF` from the Copy split menu. Presentation entities expose HTML, PDF, and PowerPoint downloads; non-presentation entities keep PDF, Markdown, and JSON downloads. Direct file actions remain available for file-backed deliverables. Toolbar icon help uses the portaled tooltip component so nested task modals do not clip tooltip text.
...
Customer Brain pattern analysis is one such bounded workflow. Its first pass uses the normal Auto route, currently Sonnet 4.6 at medium reasoning, with the relevant recent memories, belief and perspective summaries, discriminator axes, organization, and offers. It sends that compact packet with `tool_choice: none` and an 8,192-token output ceiling, so neither client tools nor native Vibey tools are exposed for the routine JSON decision.

If Sonnet requests an existing belief or perspective's older evidence, or proposes a challenge, resolution, archive, or perspective synthesis, the worker resolves only the referenced supporting memories, capped at 80. A second tool-free Opus 5 pass reviews the compact packet, initial decision, and targeted older evidence at medium reasoning. Opus must return a complete final decision with no unresolved evidence request; otherwise the worker refuses to write. The returned JSON still passes ID, evidence, distinct-customer, duplicate-belief, and write-threshold validation before Brain state changes.

Full-library user and agent worldview synthesis uses the normal Auto/Sonnet route while retaining Brain retrieval tools. Its workflow starts from INDEX, recently changed pages, and compact belief/perspective lists, then searches only the exact older belief or tension needed for an uncertain lifecycle decision. Automatic pattern-analysis events are debounced for five minutes, coalesced per Brain, and held to one active pass at a time; an explicit manual analysis bypasses the debounce. Customer Avatar synthesis and ambiguous high-impact pattern review use Opus 5. Fable 5 is not selected by any automatic Brain workflow.
...
- 2026-08-29: Replaced the generic Change the plan chat action with explicit Extend, Restart from a stage, Skip or remove a step, and Replace a step controls. Each control returns to the source conversation with its exact impact and confirmation requirements, while the bounded menu remains usable when every lifecycle action and child playbook is available.
- 2026-08-29: Injected canonical campaign assets into normal client chat and made approved Offer/Avatar ids authoritative across chat and Mission context, including fail-closed unresolved-id reporting instead of substituting another record.
- 2026-08-29: Made the active global client filter authoritative for normal Quick Mission launch choices and defaults, while preserving explicit parent-Mission Space context for extensions. The campaign-and-Space menu now portals into its owning Mission dialog instead of the page body, so Radix treats option selection as an internal interaction and preserves the chosen Space.
- 2026-08-28: Added Client Lifecycle to the shared Create/Chat Mission catalog and deterministic Mission Worker registry. The flow audits Client Brain against Company Brain methodology, reuses approved Avatar and Offer records, gates strategy, production selection, and launch authorization, and continues production through linked child Missions rather than duplicating those playbooks.
- 2026-08-28: Replaced the narrow Extend entry point with Manage mission. Continue, plan changes, step retries, and branches now return to the Mission's source conversation with a precise prompt and keep all mutations behind Pixel's existing confirmation-protected mission manager actions.
...
- 2026-08-18: Tightened the turn-level static-ad routing guard so pasted conversations with distant want/need and ad-account language no longer force the production-type clarification card.

- 2026-08-13: Moved Mission receipt polling into the shared artifact card and reused it from final-output cards after production showed completed quick-launch receipts remaining on their persisted `Started` fallback even though the Mission pane was already `Done`.
...
- 2026-08-26: Stabilized shared page controls so chat owns Show Page only while closed, Mission headers own a single More/Expand/Close Page group, and Mission expansion uses mutually exclusive positioning instead of conflicting relative and absolute states.
...
- 2026-08-13: Moved empty-chat Mission state and its host into the quick-start component itself after production showed even a direct parent callback did not reach the rendered quick-start instance.
- 2026-08-13: Passed blank Home's Mission open state directly between the composer, trigger, and modal host after production `aria-expanded` stayed false and proved the trigger was receiving the context default across a client-chunk boundary.
- 2026-08-13: Made Mission routing depend on the stable `create-mission` catalog id and exposed dialog expansion semantics on its trigger after production showed that the Mission item continued to fall through while ordinary catalog items seeded the composer.
- 2026-08-13: Replaced the module-global Quick Missions singleton with a launcher provider inside each chat surface after exact-production testing proved that normal quick starts hydrated while the singleton transition did not reach the modal consumer.
- 2026-08-13: Moved the Quick Missions host from the detached dashboard shell into the blank Home and active global chat surfaces after exact-production testing proved that the compiled shell host was not part of the live interactive tree.
- 2026-08-13: Made the shared empty-chat quick-start row own Mission routing directly and added a real host/modal integration test after production showed the generic composer-selection callback still did not open the dialog.
- 2026-08-13: Replaced the lossy `window` Quick Missions event with shared reactive launcher state after production proved that the current deployed button could dispatch without the persistent shell host observing it.
- 2026-08-13: Moved the Quick Missions host to the persistent dashboard shell and made blank-chat launches create and open their scoped source conversation before Mission creation, preserving the source link and durable receipt from the first action.
...
- 2026-07-29: Added a pre-generation chat selection gate that presents the same three Static Ad Production lanes as the Builder, then asks for the Static Ad Book family and formats before any visual is rendered.
...
- 2026-07-26: Routed Chat playbook aliases through the deterministic registry, added hard media-contract consistency preflight, and kept transient renewal timeouts inside the active execution lease.
