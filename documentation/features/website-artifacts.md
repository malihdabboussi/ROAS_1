...
The design lane uses `anthropic/claude-opus-4.8`; routine builder execution remains unchanged. Every enabled template that owns `funnel-builder` or `website-builder` receives the design skill, and existing Designer, Lux, and designer-role registry agents are backfilled. Canonical Designer/Lux runtimes may use the browser for visual review; custom designer-role runtimes receive that exception only when they own `funnel-site-design`. Canonical Pixel (`vibey`, the CEO the user chats with) may use the same browser to open a live funnel, screenshot 1440px desktop and 390px mobile, submit a labeled test lead, and inspect the confirmation page. The library `pixel` key is a separate designer runtime. All other denied tools remain denied. The old platform-managed `funnel-page-design` skill is retired because it overlaps this workflow; user-authored skill copies and variants are preserved.
...
- `openclaw-gateway.visual-review.test.ts` covers browser review access for canonical Designer/Lux/Pixel (`pixel` and `vibey`) and trained custom designer-role runtimes while preserving the deny policy for untrained or non-designer agents.
- `pixel-live-page-clickthrough.test.ts` covers Pixel's live-funnel test-lead contract, desktop/mobile viewport QC, and the production browser control path.
...
- **2026-08-18** — Unlocked the browser for the real Pixel runtime (`vibey`) and required 1440/390 screenshots. Reason: the 2026-08-14 click-through work allowed browser on the library `pixel` key, so chat Pixel still fetched HTML, skipped mobile/desktop visual QA, and never produced a screenshot card.
