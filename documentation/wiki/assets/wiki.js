...
;(function () {
  ...
  if (cb) {
    try {
      var saved = localStorage.getItem('roas-wiki-tech')
      if (saved !== null) cb.checked = saved === '1'
    } catch (e) {}
    var applyTech = function () {
      document.body.classList.toggle('show-tech', cb.checked)
      try {
        localStorage.setItem('roas-wiki-tech', cb.checked ? '1' : '0')
      } catch (e) {}
    }
    cb.addEventListener('change', applyTech)
    applyTech()
  }
  ...
  sections.forEach(function (s) {
    if (tabs.indexOf(s.tab) < 0) tabs.push(s.tab)
  })
  ...
  if (multi) {
    ...
    tabs.forEach(function (t) {
      ...
      b.type = 'button'
      b.dataset.tab = t
      var count = sections.filter(function (s) {
        return s.tab === t
      }).length
      b.innerHTML = t + '<span class="count">' + count + '</span>'
      b.addEventListener('click', function () {
        activate(t, null, true)
      })
      bar.appendChild(b)
    })
  ...
  var placeAside = function () {
    var h = nav ? nav.offsetHeight : 58
    aside.style.top = h + 'px'
    if (bar) bar.style.top = h + 'px'
    document.documentElement.style.setProperty(
      '--sticky',
      h + (bar ? bar.offsetHeight : 0) + 16 + 'px',
    )
  }
  placeAside()
  ...
  function renderOutline(active) {
    ...
    tabs.forEach(function (tab) {
      ...
      sections.forEach(function (s) {
        ...
        a.href = '#' + s.id
        a.textContent = headingText(s.h2)
        a.dataset.id = s.id
        a.addEventListener('click', function (e) {
          e.preventDefault()
          activate(s.tab, s.id, true)
        })
        aside.appendChild(a)
      })
    })
  ...
  function findSectionFor(id) {
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      for (var j = 0; j < s.els.length; j++) {
        if (s.els[j] === target || s.els[j].contains(target)) return s
      }
    }
    return null
  }
  ...
  function activate(tab, id, scroll) {
    ...
    if (id) {
      ...
      if (el) {
        ...
        if (scroll) {
          el.scrollIntoView({ block: 'start', behavior: scroll === 'instant' ? 'auto' : 'smooth' })
        }
      }
    } else if (scroll) {
  ...
  function markActive() {
    ...
    var best = null
    var offset = (nav ? nav.offsetHeight : 58) + 80
    sections.forEach(function (s) {
      if (multi && s.tab !== activeTab) return
      if (s.h2.getBoundingClientRect().top - offset <= 0) best = s.id
    })
    if (!best) {
      var first = sections.filter(function (s) {
        return !multi || s.tab === activeTab
      })[0]
      best = first ? first.id : null
    }
  ...
  window.addEventListener('hashchange', function () {
    ...
    var s = findSectionFor(h)
    if (s) activate(s.tab, h, true)
  })
})()
...
;(function () {
  ...
  box.innerHTML =
    '<form role="search"><input type="search" placeholder="Search the wiki  ( / )" aria-label="Search the wiki" autocomplete="off"></form><div class="search-results" hidden></div>'
  var spacer = nav.querySelector('.spacer')
  nav.insertBefore(box, spacer ? spacer.nextSibling : null)
  var input = box.querySelector('input'),
    panel = box.querySelector('.search-results')
  var idx = null,
    loading = false,
    waiting = []
  function load(cb) {
  ...
  var CAT = {
    ...
    decisions: 'Decisions and plans',
  }
  function crumb(r) {
    var slash = r.p.indexOf('/'),
      cat = slash > 0 ? CAT[r.p.slice(0, slash)] : null
  ...
  function run(q) {
    ...
    idx.forEach(function (r) {
      var h = r.h.toLowerCase(),
        t = r.t.toLowerCase(),
        pt = r.pt.toLowerCase(),
        score = 0
      for (var i = 0; i < terms.length; i++) {
        var term = terms[i],
          inH = h.indexOf(term) >= 0,
          inP = pt.indexOf(term) >= 0,
          n = 0,
          pos = t.indexOf(term)
        while (pos >= 0 && n < 50) {
          n++
    ...
    panel.innerHTML = hits
      .map(function (x) {
        ...
        var start = Math.max(0, p - 70),
          snip = t.slice(start, start + 180),
          e = esc(snip)
        terms.forEach(function (term) {
          e = e.replace(rx(term), '<mark>$1</mark>')
        })
        ...
        return (
          ...
          '"><span class="sr-page">' +
          esc(crumb(r)) +
          (r.tab !== 'Overview' ? ' · ' + esc(r.tab) + ' tab' : '') +
          '</span><span class="sr-head">' +
          esc(r.h) +
          '</span><span class="sr-snip">' +
          (start > 0 ? '… ' : '') +
          e +
          ' …</span></a>'
        )
